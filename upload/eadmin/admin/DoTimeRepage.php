...
function DoAutoUpAndDownInfo(){
	...
	while($r=$empire->fetch($sql))
	{
		...
		{
			$infor=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$r['classid']]['tbname']."_check where id='".$r['id']."'".do_dblimit_one());
			if(!$infor['id'])
			{
				continue;
			}
			...
			if($infor['isqf'])
			{
				$qfr=$empire->fetch1("select checktno from {$dbtbpre}enewswfinfo where id='".$r['id']."' and classid='".$r['classid']."'".do_dblimit_one());
				if($qfr['checktno']!='100')
				{
					continue;
				}
			}
			$empire->query("update {$dbtbpre}ecms_".$class_r[$r['classid']]['tbname']."_index set checked=1 where id='".$r['id']."'".do_dblimit_upone());
		...
		if($r['infodowntime']&&$r['infodowntime']<=$todaytime)
		{
			...
			if($pf)
			{
				...
				{
					$finfor=$empire->fetch1("select ".$pf." from {$dbtbpre}ecms_".$tbname."_data_".$infor['stb']." where id='".$r['id']."'".do_dblimit_one());
					$infor[$pf]=$finfor[$pf];
				}
				if($stf&&$stf==$pf)//存放文本
				{
			...
			$empire->query("update {$dbtbpre}ecms_".$class_r[$r['classid']]['tbname']."_index set checked=0,havehtml=0 where id='".$r['id']."'".do_dblimit_upone());
			$pubid=ReturnInfoPubid($r['classid'],$r['id']);
