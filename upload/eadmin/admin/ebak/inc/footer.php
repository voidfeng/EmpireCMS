...
if($p>=$tb[$btb[$t]])
{
	...
	echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=$nfile?t=$t&p=0&mydbname=$mydbname&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime&phome=ReDataF".hReturnEcmsHashStrHref(0).heformhash_get('ReDataF',1)."\">".$btb[$t-1].$fun_r['ReOneTableSuccess'];
	exit();
}
//进入下一个文件
$p++;
...
echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=$nfile?t=$t&p=$p&mydbname=$mydbname&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime&phome=ReDataF".hReturnEcmsHashStrHref(0).heformhash_get('ReDataF',1)."\">".$fun_r['ReOneDataSuccess'].Ebak_EchoReDataSt($btb[$t],$tbcount,$t,$tb[$btb[$t]],$p);
db_close();
$empire=null;
?>