...
function CreateClassCache($bclassid,$exp,$expjs,$expmodjs,$adminclass,$doall,$mid,$addminfocid,$oldmid,$oldaddminfocid,$userid){
	...
	$returnr['oldjsmod']='';
	$returnr['userjs']='';
	$num=$empire->gettotal("select count(*) as total from {$dbtbpre}enewsclass where bclassid='$bclassid'");
	if($num==0)
	{
		return $returnr;
	}
	...
	while($r=$empire->fetch($sql))
	{
		...
		{
			$classurl=$eagotourl;
			$eagotourl_onclick=' onclick="window.open(\''.$classurl.'\');return false;"';
		}
		*/
		//------ 管理栏目页面 ------
		$divonclick="";
		...
		if($r['islast'])
		{
			$img="<a href='#e' onclick=addi(".$r['classid'].")><img src='../../e/data/images/txt.gif' border=0></a>";
			$bgcolor="#ffffff";
...
function HaveNavClassCache($where){
	...
	$navcachenum=$empire->gettotal("select count(*) as total from {$dbtbpre}enewsclassnavcache where ".$where."".do_dblimit_cone());
	return $navcachenum;
}
...
if(stristr($enews,',doclass,'))
{
	...
	{
		$classfcfile='../../c/ecachepub/eclassfc/ListClass0.php';
		$classfcfile2='../../c/ecachepub/eclassfc/ListClass1.php';
...
if(stristr($enews,',doinfo,'))
{
	if(!HaveNavClassCache("navtype='listenews'"))
	{
		...
		$infofcfile='../../c/ecachepub/eclassfc/ListEnews.php';
...
if(stristr($enews,',douserinfo,'))
{
	if($userid)
	{
		...
		{
			$userinfofcfile='../../c/ecachepub/eclassfc/ListEnews'.$userid.'.php';
			WriteFiletext($userinfofcfile,AddCheckViewTempCode().$cacher['userenews']);
			$userinfojsfile='../../c/ecachepub/eclassfc/userclass'.$userid.'.js';
...
if(stristr($enews,',doinfo,'))
{
	if(!HaveNavClassCache("navtype='jsclass'"))
	{
		...
		$search_jsfile="../../c/ecachepub/eclassfc/searchclass.js";
