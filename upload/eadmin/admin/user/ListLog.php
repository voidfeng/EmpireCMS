...
function DelLog_all($loginid,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"log");
	$loginid=eCheckEmptyArray($loginid);
	$count=count($loginid);
	if(!$count)
	{
		printerror("NotDelLogid","history.go(-1)");
...
$query="select loginid,username,loginip,logintime,status,password,loginauth,ipport,userid,onepass from {$dbtbpre}enewslog";
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1">
  <tr> 
    <td height="32">位置：日志管理 &gt; <a href="ListLog.php<?=$ecms_hashur['whehref']?>">管理登录日志</a></td>
    ...
    <tr> 
      <td height="25"> <div align="center">时间从 
          ...
          <select name="show" id="show">
            ...
            <option value="2"<?=$show==2?' selected':''?>>登录IP</option>
          </select>
          <select name="status" id="status">
            <option value="0"<?=$status==0?' selected':''?>>所有状态</option>
            <option value="1"<?=$status==1?' selected':''?>>登录成功日志</option>
            <option value="2"<?=$status==2?' selected':''?>>登录失败日志</option>
			<option value="3"<?=$status==3?' selected':''?>>一次性密码登录日志</option>
          </select>
...
<form name="form2" method="post" action="ListLog.php" onsubmit="return confirm('确认要删除?');">
  ...
  <?=$ecms_hashur['form']?>
  <?php echo heformhash_get('DelLog_all'); ?>
    <tr class="header">
      <td width="7%"><div align="center"><a href="ListLog.php?orderby=0&mydesc=<?=$loginiddesc.$search2?>">ID</a></div></td>
      <td width="28%" height="25"><div align="center"><a href="ListLog.php?orderby=1&mydesc=<?=$usernamedesc.$search2?>">登录用户</a></div></td>
      <td width="20%"><div align="center"><a href="ListLog.php?orderby=2&mydesc=<?=$statusdesc.$search2?>">状态</a></div></td>
      <td width="17%" height="25"><div align="center"><a href="ListLog.php?orderby=3&mydesc=<?=$loginipdesc.$search2?>">登录IP</a></div></td>
      <td width="17%"><div align="center"><a href="ListLog.php?orderby=4&mydesc=<?=$logintimedesc.$search2?>">登录时间</a></div></td>
      <td width="11%" height="25"><div align="center">删除</div></td>
    </tr>
    <?php
	//formhash
	$efh=heformhash_get('DelLog',1);
  ...
  while($r=$empire->fetch($sql))
  {
	...
	$showuid=$r['userid']?' ('.$r['userid'].')':'';
	$showonepass=$r['onepass']?'<br><font color="#666666">一密：'.$r['onepass'].'</font>':'';
  ?>
    <tr bgcolor="#FFFFFF" id=log<?=$r['loginid']?>>
      ...
      <td height="25"><div align="center">[<a href="ListLog.php?enews=DelLog&loginid=<?=$r['loginid']?><?=$ecms_hashur['href'].$efh?>" onclick="return confirm('确认要删除此日志?');">删除</a> 
...
<form action="ListLog.php" method="post" name="dellogform" id="dellogform" onsubmit="return confirm('确认要删除?');">
  ...
  <?=$ecms_hashur['form']?>
  <?php echo heformhash_get('DelLog_date'); ?>
    <tr> 
      <td><div align="center">
