...
$tbname=$add['tbname'];
$tbname=eCheckEmptyArray($tbname);
$count=count($tbname);
//刷新所有表
if(!$count)
{
...
$url="../ecmschtml.php?enews=ReNewsHtml&classid=".$add['classid']."&from=".urlencode($add['from'])."&retype=".$add['retype']."&startday=".$add['startday']."&endday=".$add['endday']."&startid=".$add['startid']."&endid=".$add['endid']."&havehtml=".$havehtml."&reallinfotime=".time().$ecms_hashur['href'].$efh.$mphref;
