...
function ECMS_EditorPrintError($errorNumber,$fileUrl,$fileName,$customMsg,$fileno,$filesize){
	...
	{
		@include "../../../e/data/".LoadLang("pub/message.php");
...
function eTranMorePic($file,$file_name,$file_type,$file_size,$add,$userid,$username){
	...
	if($add['saveurl'])
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			eInsertFileTable($r['filename'],$r['filesize'],$r['filepath'],$username,$add['classid'],$r['filename'],$add['type'],$add['filepass'],$add['filepass'],$public_r['fpath'],0,$modtype,$fstb,$cid,$cid2);
			//缩略图
			if($add['getsmall'])
			{
				$sfiler=GetMySmallImg($add['classid'],$r['filename'],$r['insertfile'],$r['filepath'],$r['yname'],$add['swidth'],$add['sheight'],$r['name'],$add['filepass'],$add['filepass'],$userid,$username,$modtype,$fstb,$cid,$cid2);
	...
	else
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			$add['type']=(int)$add['type'];
			$add['filepass']=(int)$add['filepass'];
			eInsertFileTable($r['filename'],$r['filesize'],$r['filepath'],$username,$add['classid'],$file_name[$i],$add['type'],$add['filepass'],$add['filepass'],$public_r['fpath'],0,$modtype,$fstb,$cid,$cid2);
			//缩略图
			if($add['getsmall'])
			{
				$sfiler=GetMySmallImg($add['classid'],$r['filename'],$r['insertfile'],$r['filepath'],$r['yname'],$add['swidth'],$add['sheight'],$r['name'],$add['filepass'],$add['filepass'],$userid,$username,$modtype,$fstb,$cid,$cid2);
...
function TranFile($file,$file_name,$file_type,$file_size,$tranurl,$no,$classid,$type,$post,$userid,$username){
	...
	$type=(int)$type;
	$sql=eInsertFileTable($r['filename'],$r['filesize'],$r['filepath'],$username,$classid,$no,$type,$post['filepass'],$post['filepass'],$public_r['fpath'],0,$modtype,$fstb,$cid,$cid2);
	$fileid=$empire->lastid(eReturnFileTable($modtype,$fstb),'fileid');
	//导入gd.php文件
	...
	{
		GetMySmallImg($classid,$no,$r['insertfile'],$r['filepath'],$r['yname'],$post['width'],$post['height'],$r['name'],$post['filepass'],$post['filepass'],$userid,$username,$modtype,$fstb,$cid,$cid2);
	}
	//水印
