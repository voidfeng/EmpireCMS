...
if($ecms=='dotask')
{
	...
	$r=$empire->fetch1("select id,filename,lastdo,doweek,doday,dohour,dominute,userid,taskname from {$dbtbpre}enewstask where id=$id and isopen=1".do_dblimit_one());
	$file='../../e/tasks/'.$r['filename'];
...
elseif($ecms=='TodoTask')
{
	...
	$r=$empire->fetch1("select id,filename,taskname from {$dbtbpre}enewstask where id=$id".do_dblimit_one());
	$file='../../e/tasks/'.$r['filename'];
