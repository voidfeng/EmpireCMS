...
//菜单显示
$showfastmenu=$empire->gettotal("select count(*) as total from {$dbtbpre}enewsmenuclass where classtype=1".do_dblimit_cone());//常用菜单
$showextmenu=$empire->gettotal("select count(*) as total from {$dbtbpre}enewsmenuclass where classtype=3".do_dblimit_cone());//扩展菜单
$showshopmenu=stristr($public_r['closehmenu'],',shop,')?0:1;
//formhash
$efh=heformhash_get('exit',1);
...
<div align="center"><font color="#FFFFFF"><strong>常用</strong></font></div></td>
                    ...
                    <TD width="301"><div align="right"><font color="#ffffff">用户：<a href="user/EditPassword.php<?=$ecms_hashur['whehref']?>" target="main"><font color="#ffffff"><b><?=$loginin?></b></font></a>&nbsp;&nbsp;&nbsp;[<a href="#ecms" onClick="if(confirm('确认要退出?')){location.href='enews.php?enews=exit<?=$ecms_hashur['href'].$efh?>';}"><font color="#ffffff">退出</font></a>]&nbsp;&nbsp;</font></div></TD>
                  </TR>
                </TBODY>
              </TABLE>
              ...
              <TABLE class=flyoutMenu border=0>
                <TBODY>
                  <TR align=middle> 
					...
					{
					?>
                    <TD width="60" class="flyoutLink" onClick="JumpToMain('openpage/AdminPage.php?efileid=3<?=$ecms_hashur['ehref']?>');" onMouseOver="over(this)" onMouseOut="out(this)">管理评论</TD>
					<?php
					}
					?>
                    <TD width="60" class="flyoutLink" onClick="JumpToMain('sp/UpdgxSp.php<?=$ecms_hashur['whehref']?>');" onMouseOver="over(this)" onMouseOut="out(this)">更新碎片</TD>
					<TD width="60" class="flyoutLink" onClick="JumpToMain('special/UpdgxZt.php<?=$ecms_hashur['whehref']?>');" onMouseOver="over(this)" onMouseOut="out(this)">更新专题</TD>
					<?php
					if($r['dochangedata'])
					{
					?>
                    ...
                    <TD width="60" class="flyoutLink" onClick="window.open('http://www.phome.net/zx/pnews/');" onMouseOver="over(this)" onMouseOut="out(this)">版本更新</TD>
                  </TR>
                </TBODY>
              </TABLE>            </TD>
...
?>
  ...
  <?=$ecms_hashur['form']?>
  <?php echo heformhash_get('ChangeMoreportAdmin'); ?>
	&nbsp;&nbsp;
<?php
}
?>
