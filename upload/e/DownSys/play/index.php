...
function ViewOnlineKFen($showdown_r,$u,$userid,$classid,$id,$pathid,$r){
	global $level_r,$class_r,$dbtbpre,$public_r,$empire,$have_bak,$have_fen,$emod_pubr;
	$userid=(int)$userid;
	$classid=(int)$classid;
	$id=(int)$id;
	$pathid=(int)$pathid;
...
if($downgroup)
{
	...
	{
		//---------是否有历史记录
			$bakr=$empire->fetch1("select id,truetime from {$dbtbpre}enewsdownrecord where id='$id' and classid='$classid' and userid='".$user['userid']."' and pathid='$pathid' and online=1 order by truetime desc".do_dblimit_one());
...
else
{
	...
	eCheckStrType(5,$playerr['filename'],1);
	if($playerr['filename'])
	{
