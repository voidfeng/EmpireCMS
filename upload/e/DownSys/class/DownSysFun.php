...
function DownSoft($classid,$id,$pathid,$p,$pass){
	global $empire,$dbtbpre,$public_r,$level_r,$class_r,$emod_r,$ecms_config,$emod_pubr;
	//验证IP
	...
	$ok=1;
	$r=$empire->fetch1("select * from {$dbtbpre}ecms_".$tbname." where id='$id'".do_dblimit_one());
	...
	if($downgroup)
	{
		...
		//取得会员资料
		$u=$empire->fetch1("select ".eReturnSelectMemberF('*')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' and ".egetmf('rnd')."='$rnd'".do_dblimit_one());
		if(empty($u['userid']))
		{printerror("MustSingleUser","history.go(-1)",1);}
		if(empty($u['isot']))
		{printerror("MustSingleUser","history.go(-1)",1);}
		...
		{
			//---------是否有历史记录
			$bakr=$empire->fetch1("select id,truetime from {$dbtbpre}enewsdownrecord where id='$id' and classid='$classid' and userid='$userid' and pathid='$pathid' and online=0 order by truetime desc".do_dblimit_one());
	...
	if(strstr($emod_pubr['tbidsed'],','.$tid.','))
	{
		$ymdtotaled=eReturnYmdTotalf($r,1);
	}
    $usql=$empire->query("update {$dbtbpre}ecms_".$class_r[$classid]['tbname']." set totaldown=totaldown+1".$ymdtotaled." where id='$id'");
...
function QDownLoadFile($file){
	...
	$firstp=substr($file,0,3);
	$firstp2=substr($file,0,4);
	if($firstp=='/d/'||$firstp2=='/dd/')
	{}
	else
	{
		Header("Location:$firstfile");
...
function GetSofturl($classid,$id,$pathid,$p,$pass,$onlinetime,$onlinepass){
	global $empire,$dbtbpre,$public_r,$class_r,$emod_r,$level_r,$ecms_config,$emod_pubr;
	//验证IP
	...
	if($downgroup)
	{
		...
		{
			//---------是否有历史记录
		    $bakr=$empire->fetch1("select id,truetime from {$dbtbpre}enewsdownrecord where id='$id' and classid='$classid' and userid='$userid' and pathid='$pathid' and online=1 order by truetime desc".do_dblimit_one());
			if($bakr['id']&&(time()-$bakr['truetime']<=$public_r['redodown']*3600))
			{}
			else
			{
