...
$user=islogin();
$ddid=(int)$_GET['ddid'];
$r=$empire->fetch1("select * from {$dbtbpre}enewsshopdd where ddid='$ddid' and userid='".$user['userid']."'".do_dblimit_one());
if(empty($r['ddid']))
{
	printerror('此订单不存在','',1,0,1);
}
$shoppr=ShopSys_ReturnSet();
$addr=$empire->fetch1("select * from {$dbtbpre}enewsshopdd_add where ddid='$ddid'".do_dblimit_one());
//需要发票
$fp="否";
if($r['fp'])
{
...
//导入模板
include(ECMS_PATH.'e/template/ShopSys/ShowDd.php');
db_close();
$empire=null;
?>