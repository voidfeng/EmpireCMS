...
function ShopSys_ReturnSet(){
	...
	$shoppr=$empire->fetch1("select * from {$dbtbpre}enewsshop_set".do_dblimit_one());
	return $shoppr;
}
...
//加入购物车
function AddBuycar($classid,$id,$pn=1,$add=null){
	global $class_r,$empire,$dbtbpre,$public_r;
	$shoppr=ShopSys_ReturnSet();
	$classid=(int)$classid;
	$id=(int)$id;
	$pn=(int)$pn;
	if(empty($classid)||empty($id)||empty($class_r[$classid]['tbname'])||InfoIsInTable($class_r[$classid]['tbname']))
	{
	...
	//验证产品是否存在
	$infor=$empire->fetch1("select id,classid,pmaxnum,price,buyfen from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where id='$id'".do_dblimit_one());
...
function EditBuycar($add){
	...
	$record="!";
	$field="|";
	$productid=eCheckEmptyArray($add['productid']);
	$addatt=$add['addatt'];
	$num=$add['num'];
	$del=eCheckEmptyArray($add['del']);
	$delatt=$add['delatt'];
...
function ShopSys_BuycarInfoAdd($addatt){
	$addatt=eCheckEmptyArray($addatt);
	$count=count($addatt);
	if(!$count)
	{
		return '';
...
function ShopSys_GetPre($precode,$totalmoney,$user,$classids){
	...
	$prer=$empire->fetch1("select id,prename,precode,premoney,pretype,reuse,endtime,groupid,classid,musttotal,usenum,haveusenum from {$dbtbpre}enewsshop_precode where precode='$precode'".do_dblimit_one());
	if(!$prer['id'])
	{
		printerror("EmptyPreCode","history.go(-1)",1);
	}
...
function AddDd($add){
	...
	$edb_ir['ddno']=RepPostVar($add['ddno']);
	$edb_ir['truename']=dgdbe_rpstr($add['truename']);
	$edb_ir['oicq']=dgdbe_rpstr($add['oicq']);
	$edb_ir['msn']=dgdbe_rpstr($add['msn']);
	$edb_ir['mycall']=dgdbe_rpstr($add['mycall']);
	$edb_ir['phone']=dgdbe_rpstr($add['phone']);
	$edb_ir['email']=dgdbe_rpstr($add['email']);
	$edb_ir['address']=dgdbe_rpstr($add['address']);
	$edb_ir['zip']=dgdbe_rpstr($add['zip']);
	$edb_ir['signbuild']=dgdbe_rpstr($add['signbuild']);
	$edb_ir['besttime']=dgdbe_rpstr($add['besttime']);
	$edb_ir['bz']=dgdbe_rpstr($add['bz']);
	$edb_ir['fptt']=dgdbe_rpstr($add['fptt']);
	$edb_ir['fpname']=dgdbe_rpstr($add['fpname']);
	$edb_ir['fp']=(int)$add['fp'];
	$edb_ir['psid']=(int)$add['psid'];
	$edb_ir['payfsid']=(int)$add['payfsid'];
	$edb_ir['precode']=RepPostVar($add['precode']);
	//基本必填
	if(!$edb_ir['ddno'])
	{
		printerror("EmptyBuycar","history.go(-1)",1);
	...
	//必填项
	ShopSys_CheckDdMust($edb_ir,$shoppr);
	$mess="AddDdSuccess";
	$haveprice=0;
	$payby=0;
	//返回购物车存放格式
	...
	$alltotal=$buyr[2];
	$alltotalfen=$buyr[1];
	$buycar=dgdbe_rpstr($buyr[3]);
	$classids=$buyr['classids'];
	//配送方式
	$pr=array();
	if($shoppr['shoppsmust'])
	{
		$pr=$empire->fetch1("select pid,pname,price from {$dbtbpre}enewsshopps where pid='".$edb_ir['psid']."' and isclose=0");
		if(empty($pr['pid']))
		{
			printerror("NotPsid","history.go(-1)",1);
		}
	...
	{
		$payr=$empire->fetch1("select payid,payname,payurl,userpay,userfen from {$dbtbpre}enewsshoppayfs where payid='".$edb_ir['payfsid']."' and isclose=0");
		if(empty($payr['payid']))
		{
			printerror("NotPayfsid","history.go(-1)",1);
		}
	...
	if($userid)
	{
		...
		$user=$empire->fetch1("select ".eReturnSelectMemberF('userid,money,userfen,groupid')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' and ".egetmf('rnd')."='$rnd'".do_dblimit_one());
		if(!$user['userid'])
		{
	...
	if($edb_ir['precode'])
	{
		$prer=ShopSys_GetPre($edb_ir['precode'],$alltotal,$user,$classids);
	...
	$pr['pname']=dgdbe_rpstr($pr['pname']);
	$pr['price']=dgdbe_rpstr($pr['price']);
	$payr['payname']=dgdbe_rpstr($payr['payname']);
	$haveprice=(int)$haveprice;
	$payby=(int)$payby;
	$buycar=dgdbe_rpstr($buycar);
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsshopdd(ddno,ddtime,userid,username,outproduct,haveprice,checked,truename,oicq,msn,email,mycall,phone,address,zip,psid,psname,pstotal,alltotal,payfsid,payfsname,payby,alltotalfen,fp,fptt,fptotal,fpname,userip,signbuild,besttime,pretotal,ddtruetime) values('".$edb_ir['ddno']."','".$ddtime."','".$userid."','".$username."',0,'".$haveprice."',0,'".$edb_ir['truename']."','".$edb_ir['oicq']."','".$edb_ir['msn']."','".$edb_ir['email']."','".$edb_ir['mycall']."','".$edb_ir['phone']."','".$edb_ir['address']."','".$edb_ir['zip']."','".$edb_ir['psid']."','".$pr['pname']."','".$pr['price']."','".$alltotal."','".$edb_ir['payfsid']."','".$payr['payname']."','".$payby."','".$alltotalfen."','".$edb_ir['fp']."','".$edb_ir['fptt']."','".$fptotal."','".$edb_ir['fpname']."','".$ip."','".$edb_ir['signbuild']."','".$edb_ir['besttime']."','".$pretotal."','".$ddtruetime."');","ins");
	$ddid=$empire->lastid($dbtbpre.'enewsshopdd','ddid');
	$sqladd=$empire->updatesql("insert into {$dbtbpre}enewsshopdd_add(ddid,buycar,bz,retext) values('".$ddid."','".addslashes($buycar)."','".$edb_ir['bz']."','');","ins");
	//减库存
...
function ReturnBuycardd($shoppr){
	...
	for($i=0;$i<count($r)-1;$i++)
	{
		...
		//取得产品信息
		$productr=$empire->fetch1("select title,tprice,price,isurl,titleurl,classid,id,titlepic,buyfen,pmaxnum from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where id='$id'".do_dblimit_one());
		...
		$productr['price']=dgdbe_rpstr($productr['price']);
...
function ShopDdToPay($ddid){
	...
	//是否登陆
	$user_r=islogin();
	$r=$empire->fetch1("select ddid,payfsid,haveprice,checked,ddtime from {$dbtbpre}enewsshopdd where ddid='$ddid' and userid='".$user_r['userid']."'".do_dblimit_one());
	if(!$r['ddid'])
	{
		printerror("NotShopDdId","history.go(-1)",1);
	}
...
function ShopSys_qDelDd($add){
	...
	$r=$empire->fetch1("select ddid,outproduct,haveprice,checked,ddtime,havecutnum from {$dbtbpre}enewsshopdd where ddid='$ddid' and userid='".$user_r['userid']."'".do_dblimit_one());
	if(!$r['ddid'])
	{
		printerror("NotChangeShopDdid","history.go(-1)",1);
	}
...
function ShopSys_AddAddress($add){
	...
	//是否登陆
	$user_r=islogin();
	$edb_ir['addressname']=dgdbe_rpstr($add['addressname']);
	$edb_ir['truename']=dgdbe_rpstr($add['truename']);
	$edb_ir['oicq']=dgdbe_rpstr($add['oicq']);
	$edb_ir['msn']=dgdbe_rpstr($add['msn']);
	$edb_ir['email']=dgdbe_rpstr($add['email']);
	...
	$isdefault=$num?0:1;
	$sql=$empire->query("insert into {$dbtbpre}enewsshop_address(addressname,userid,truename,oicq,msn,email,address,zip,mycall,phone,signbuild,besttime,isdefault) values('".$edb_ir['addressname']."','".$user_r['userid']."','".$edb_ir['truename']."','".$edb_ir['oicq']."','".$edb_ir['msn']."','".$edb_ir['email']."','".$edb_ir['address']."','".$edb_ir['zip']."','".$edb_ir['mycall']."','".$edb_ir['phone']."','".$edb_ir['signbuild']."','".$edb_ir['besttime']."','".$isdefault."');");
	if($sql)
	{
...
function ShopSys_EditAddress($add){
	...
	$edb_ir['signbuild']=dgdbe_rpstr($add['signbuild']);
	$edb_ir['besttime']=dgdbe_rpstr($add['besttime']);
	if(!$addressid||!trim($edb_ir['addressname']))
	{
		printerror("EmptyAddress","history.go(-1)",1);
	}
	$sql=$empire->query("update {$dbtbpre}enewsshop_address set addressname='".$edb_ir['addressname']."',truename='".$edb_ir['truename']."',oicq='".$edb_ir['oicq']."',msn='".$edb_ir['msn']."',email='".$edb_ir['email']."',address='".$edb_ir['address']."',zip='".$edb_ir['zip']."',mycall='".$edb_ir['mycall']."',phone='".$edb_ir['phone']."',signbuild='".$edb_ir['signbuild']."',besttime='".$edb_ir['besttime']."' where addressid='$addressid' and userid='".$user_r['userid']."'");
	if($sql)
	{
...
function ShopSys_GetAddress($addressid){
	...
	$address_r=$empire->fetch1("select * from {$dbtbpre}enewsshop_address where userid='".$user_r['userid']."' and ".$where."".do_dblimit_one());
	return $address_r;
}
?>