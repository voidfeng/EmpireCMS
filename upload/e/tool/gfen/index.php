...
$r=$empire->fetch1("select ".eReturnSelectMemberF('userid,username')." from ".eReturnMemberTable()." where ".$where."".do_dblimit_one());
if(empty($r['userid']))
{
	Header("Location:$gotourl");
	exit();
...
else
{
	...
	$ipr=$empire->fetch1("select ip,addtime from {$dbtbpre}enewsgfenip where ip='$ip'".do_dblimit_one());
	if($ipr['ip'])
	{
		Header("Location:$gotourl");
		exit();
