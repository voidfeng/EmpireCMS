...
function do_dbquery_common($query,$dblink,$ecms=0){
	...
	{
		$sql=mysql_query($query,$dblink) or die($ecms_config['db']['showerror']==1?str_replace($GLOBALS['dbtbpre'],'***_',mysql_error($dblink).'<br>'.$query):'DbError');
	}
	return $sql;
}
...
class mysqlquery
{
	...
	function query($query,$ecms=''){
		global $ecms_config;
		$this->sql=mysql_query($query,return_dblink($query)) or die($ecms_config['db']['showerror']==1?str_replace($GLOBALS['dbtbpre'],'***_',mysql_error(return_dblink($query)).'<br>'.$query):'DbError');
		return $this->sql;
	}
	//执行mysql_query()语句2
	...
	function usequery($query){
		global $ecms_config;
		$this->sql=mysql_query($query,$GLOBALS['link']) or die($ecms_config['db']['showerror']==1?str_replace($GLOBALS['dbtbpre'],'***_',mysql_error($GLOBALS['link']).'<br>'.$query):'DbError');
		if($GLOBALS['linkrd'])
		{
	...
	function updatesql($query,$ecms=''){
		global $ecms_config;
		$this->sql=mysql_query($query,return_dblink_w()) or die($ecms_config['db']['showerror']==1?str_replace($GLOBALS['dbtbpre'],'***_',mysql_error(return_dblink_w()).'<br>'.$query):'DbError');
		return $this->sql;
	}
	//执行mysql_query()语句2(操作数据库)
	function updatesql2($query,$ecms=''){
		global $ecms_config;
		$this->sql=mysql_query($query,return_dblink_w());
		return $this->sql;
	}
	//执行pg_prepare()语句
	...
	{
		$this->sql=$this->query($query);
		$this->r=mysql_fetch_assoc($this->sql);
		return $this->r;
	}
	//执行mysql_fetch_row()
	function fetch_sz($sql)//此方法的参数是$sql就是sql语句执行结果
	{
		$this->r=mysql_fetch_row($sql);
		return $this->r;
	}
	...
	{
		$this->sql=$this->query($query);
		$this->r=mysql_fetch_row($this->sql);
		return $this->r;
	}
	//执行mysql_num_rows()
