...
function GetMemberFform($type,$f,$fvalue,$fformsize=''){
	...
	$file="../../e/data/html/memberfhtml.txt";
...
function AddMemberF($add,$userid,$username){
	...
	//字段是否重复
	$s=do_dbFieldRowList($dbtbpre.'enewsmemberadd');
	$b=0;
	...
	$s=do_dbFieldRowList($dbtbpre.'enewsmember');
	$b=0;
	while($r=$empire->fetch($s))
	{
		if($r['Field']==$add['f'])
	...
	//新增字段
	$asql=do_dbTableAddF($dbtbpre.'enewsmemberadd',$add['f'],$add['ftype'],$add['flen'],'');//pgsql
	//替换代码
	...
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsmemberf(f,fname,fform,fhtml,fzs,myorder,ftype,flen,fvalue,fformsize) values('".$add['f']."','".$add['fname']."','".$add['fform']."','".eaddslashes2($fhtml)."','".eaddslashes($add['fzs'])."','".$add['myorder']."','".$add['ftype']."','".$add['flen']."','".eaddslashes2($add['fvalue'])."','".$add['fformsize']."');","ins");
	$lastid=$empire->lastid($dbtbpre.'enewsmemberf','fid');
	if($asql&&$sql)
	{
		//操作日志
...
function EditMemberF($add,$userid,$username){
	...
	$field="".$add['f']." ".$type." NOT NULL".$def;
	$usql=do_dbTableEditF($dbtbpre.'enewsmemberadd',$add['f'],$add['oldf'],$add['ftype'],$add['flen'],'');//pgsql
	//替换代码
...
function DelMemberF($add,$userid,$username){
	...
	$usql=do_dbTableDelF($dbtbpre.'enewsmemberadd',$r['f']);//pgsql
...
function ChangeMemberForm($fid,$mtemp){
	global $empire,$dbtbpre;
	$fid=(int)$fid;
	$file="../../c/ecachemod/emodpub/memberform".$fid.".php";
...
function AddMemberForm($add,$userid,$username){
	...
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsmemberform(fname,ftemp,fzs,enter,mustenter,filef,imgf,tobrf,viewenter,searchvar,canaddf,caneditf,checkboxf) values('".$add['fname']."','".eaddslashes2($add['ftemp'])."','".addslashes($add['fzs'])."','$enter','$mustenter','$filef','$imgf','$tobrf','$viewenter','$searchvar','$canaddf','$caneditf','$checkboxf');","ins");
	$fid=$empire->lastid($dbtbpre.'enewsmemberform','fid');
	//生成表单页面
...
function AddMemberGroup($add,$userid,$username){
	...
	//处理
	$add['groupname']=hRepPostStr($add['groupname'],1,1);
	...
	$add['regps']=hRepPostStr($add['regps'],1,1);
	$add['loginps']=hRepPostStr($add['loginps'],1,1);
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsmembergroup(groupname,level,checked,favanum,daydown,msglen,msgnum,canreg,formid,regchecked,spacestyleid,dayaddinfo,infochecked,plchecked,regps,loginps) values('".$add['groupname']."','".$add['level']."','".$add['checked']."','".$add['favanum']."','".$add['daydown']."','".$msglen."','".$msgnum."','".$canreg."','".$formid."','".$regchecked."','".$spacestyleid."','".$add['dayaddinfo']."','".$add['infochecked']."','".$add['plchecked']."','".$add['regps']."','".$add['loginps']."');","ins");
	$groupid=$empire->lastid($dbtbpre.'enewsmembergroup','groupid');
	//更新缓存
	GetMemberLevel();
	if($sql)
	{
