...
function AddInfoVote($classid,$id,$add){
	...
	$num=$empire->gettotal("select count(*) as total from {$dbtbpre}enewsinfovote where pubid='$pubid'".do_dblimit_cone());
	$votename=eCheckEmptyArray($add['vote_name']);
	...
	if($num)	//修改
	{
		...
		$sql=$empire->query("update {$dbtbpre}enewsinfovote set title='".$add['vote_title']."',votenum='$t_votenum',votetext='$votetext',voteclass='$voteclass',doip='$doip',dotime='".$add['vote_dotime']."',tempid='$tempid',width='$width',height='$height',diyotherlink='$diyotherlink',infouptime='$infouptime',infodowntime='$infodowntime' where pubid='$pubid'".do_dblimit_upone());
	}
	else	//增加
	{
...
function UpdateInfoCopyids($classid,$id,$copyids){
	...
	if($num)	//修改
	{
		$empire->query("update {$dbtbpre}enewsinfovote set copyids='$copyids' where pubid='$pubid'".do_dblimit_upone());
	}
	else	//增加
	{
...
function ReturnCheckRetitle($add){
	...
	//已审核
	$num=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where title='".addslashes($title)."'".$where."".do_dblimit_cone());
	//未审核
	if(empty($num))
	{
		$num=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_check where title='".addslashes($title)."'".$where."".do_dblimit_cone());
	}
	return $num;
}
...
function DoPostInfoVar($add){
	...
	$add['newwfid']=(int)$add['newwfid'];
	$add['oldwfid']=(int)$add['oldwfid'];
	$add['isurl']=$add['titleurl']?1:0;
	//ftype
	eCheckStrType_fname($add['filename'],1);
	eCheckStrType_fname($add['newspath'],2);
	return $add;
}
...
function AddNews($add,$userid,$username){
	...
	$ccr=$empire->fetch1("select classid,modid,listdt,haddlist,sametitle,addreinfo,wburl,repreinfo from {$dbtbpre}enewsclass where classid='".$add['classid']."' and islast=1".do_dblimit_one());
	...
	$isqf=0;
	$dowfid=0;
	if($class_r[$add['classid']]['wfid']||$add['newwfid'])
	{
		if($doselfinfo['doeditqf']&&$add['newwfid'])
		{
			if($add['newwfid']>0)
			{
				$add['checked']=0;
		...
		else
		{
			...
			if(!$userisqf)
			{
				$add['checked']=0;
				$isqf=1;
				$dowfid=$class_r[$add['classid']]['wfid'];
			}
		}
	}
	...
	//索引表
	$sql=$empire->updatesql("insert into {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('".$add['classid']."','".$add['checked']."','".$newstime."','".$truetime."','".$lastdotime."','".$havehtml."');","ins");
	$id=$empire->lastid($dbtbpre.'ecms_'.$class_r[$add['classid']]['tbname'].'_index','id');
	$pubid=ReturnInfoPubid($add['classid'],$id);
	$infotbr=ReturnInfoTbname($class_r[$add['classid']]['tbname'],$add['checked'],$ret_r['tb']);
	//efz
	$newefzstb=PubAEInfoToFzData($class_r[$add['classid']]['tbname'],$add['classid'],$id,$newstime,$add['checked'],'',$add['efzid'],$add['delefzid'],0,0);
	$addefz_f='';
	$addefz_v='';
	...
	//主表
	$infosql=$empire->updatesql("insert into ".$infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard".$ret_r['fields'].") values('".$id."','".$add['classid']."','".$add['ttid']."','".$add['onclick']."',0,'".$add['totaldown']."','".$newspath."','".$filename."','".$userid."','".addslashes($username)."','".$add['firsttitle']."','".$add['isgood']."','".$add['ispic']."','".$add['istop']."','".$isqf."',0,'".$add['isurl']."','".$truetime."','".$lastdotime."','".$havehtml."','".$add['groupid']."','".$add['userfen']."','".addslashes($add['my_titlefont'])."','".addslashes($add['titleurl'])."','".$ret_r['tb']."','".$public_r['filedeftb']."','".$public_r['pldeftb']."','".addslashes($add['keyboard'])."'".$ret_r['values'].");","ins");
	//副表
	$finfosql=$empire->updatesql("insert into ".$infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$addefz_f.$ret_r['datafields'].") values('".$id."','".$add['classid']."','".$keyid."','".$add['dokey']."','".$add['newstempid']."','".$add['closepl']."',0,'".addslashes($add['infotags'])."'".$addefz_v.$ret_r['datavalues'].");","ins");
	//更新栏目信息数
	...
	if($isqf==1)
	{
		InfoInsertToWorkflow($id,$add['classid'],$dowfid,$userid,$username);
	}
	//更新附件表
	...
	//usersavelast
	AddInfoUserSaveLastSt($class_r[$add['classid']]['modid'],$userid,$username);
	//加入专题
	...
	{
		eInsertTags($add['infotags'],$add['classid'],$id,$newstime,$add['checked']);
	}
	//处理函数
	...
	{
		$prer=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']." where id<$id and classid='".$add['classid']."' order by id desc".do_dblimit_one());
		$epreid=$prer['id'];
	...
	//同时发布
	$copyclassid=eCheckEmptyArray($add['copyclassid']);
	...
	if($sql)
	{
		...
		$GLOBALS['public_diyr']['ecmsadderrorurl']=$ecmsfrom.$addecmscheck;
...
function EditNews($add,$userid,$username){
	...
	//索引表
	$index_checkr=$empire->fetch1("select id,classid,checked from {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']."_index where id='".$add['id']."'".do_dblimit_one());
	...
	$checkr=$empire->fetch1("select id,classid,userid,username,ismember,stb,newspath,filename,isqf,fstb,isgood,firsttitle,istop,ttid,eckuid,efz from ".$infotb." where id='".$add['id']."'".do_dblimit_one());
	...
	if($checkr['isqf'])
	{
		if(!$doselfinfo['doeditqf'])
		{
			$qfr=$empire->fetch1("select wfid,checktno from {$dbtbpre}enewswfinfo where id='".$add['id']."' and classid='".$add['classid']."'".do_dblimit_one());
			if($qfr['checktno']<100)
			{
				$qfwfr=$empire->fetch1("select wfid,canedit from {$dbtbpre}enewsworkflow where wfid='".$qfr['wfid']."'".do_dblimit_one());
	...
	$pubcheckr=$empire->fetch1("select copyids from {$dbtbpre}enewsinfovote where pubid='$pubid'".do_dblimit_one());
	...
	if($doselfinfo['doeditqf']&&$add['newwfid'])
	{
		if($add['newwfid']==-100)//取消
		{
			if($checkr['isqf'])
			{
				$empire->query("delete from {$dbtbpre}enewswfinfo where id='".$add['id']."' and classid='".$add['classid']."'");
				$empire->query("delete from {$dbtbpre}enewswfinfolog where id='".$add['id']."' and classid='".$add['classid']."'");
				$updatefile.=",isqf=0";
			}
		...
		{
			$havechwf=InfoChangeToWorkflow($add['id'],$add['classid'],$add['oldwfid'],$add['newwfid'],$userid,$username);
			if(!$checkr['isqf'])
			{
				$updatefile.=",isqf=1";
			}
	...
	else
	{
		...
		{
			$qfr=$empire->fetch1("select checktno from {$dbtbpre}enewswfinfo where id='".$add['id']."' and classid='".$add['classid']."'".do_dblimit_one());
	...
	if($newefzstb&&$newefzstb!='null')
	{
		$addefz_v=",efzstb='".$newefzstb."'";
	}
	//主表
	$sql=$empire->query("update ".$infotbr['tbname']." set classid='".$add['classid']."',ttid='".$add['ttid']."',onclick='".$add['onclick']."',totaldown='".$add['totaldown']."',firsttitle='".$add['firsttitle']."',isgood='".$add['isgood']."',ispic='".$add['ispic']."',istop='".$add['istop']."',isurl='".$add['isurl']."',lastdotime='".$lastdotime."',groupid='".$add['groupid']."',userfen='".$add['userfen']."',titlefont='".addslashes($add['my_titlefont'])."',titleurl='".addslashes($infourl)."',keyboard='".addslashes($add['keyboard'])."'".$updatefile.$ret_r['values']." where id='".$add['id']."'".do_dblimit_upone());
	//副表
	$stb=$checkr['stb'];
	$fsql=$empire->query("update ".$infotbr['datatbname']." set classid='".$add['classid']."',keyid='".$keyid."',dokey='".$add['dokey']."',newstempid='".$add['newstempid']."',closepl='".$add['closepl']."',infotags='".addslashes($add['infotags'])."'".$addefz_v.$ret_r['datavalues']." where id='".$add['id']."'".do_dblimit_upone());
	//取第一张图作为标题图片
	if($add['getfirsttitlepic']&&empty($add['titlepic']))
	{
	...
	//TAGS
	if($add['infotags']&&($add['infotags']<>$add['oldinfotags']||$index_checkr['checked']!=$newchecked))
	{
		eInsertTags($add['infotags'],$add['classid'],$add['id'],$newstime,$newchecked);
	}
	//是否改变审核状态
	...
	{
		$prer=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']." where id<".$add['id']." and classid='".$add['classid']."' order by id desc".do_dblimit_one());
		$epreid=$prer['id'];
...
function EditInfoSimple($add,$userid,$username){
	...
	//主表
	$infotb=ReturnInfoMainTbname($class_r[$add['classid']]['tbname'],$index_checkr['checked']);
	$checkr=$empire->fetch1("select id,classid,userid,username,ismember,stb,newspath,filename,isqf,fstb,isgood,firsttitle,istop,groupid,ttid,efz from ".$infotb." where id='".$add['id']."'".do_dblimit_one());
	if($doselfinfo['doselfinfo']&&($checkr['userid']<>$userid||$checkr['ismember']))//只能编辑自己的信息
	{
	...
	$indexsql=$empire->query("update {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']."_index set newstime='$newstime',lastdotime='$lastdotime'".$aqf." where id='".$add['id']."'".do_dblimit_upone());
	//主表
	$sql=$empire->query("update ".$infotbr['tbname']." set classid='".$add['classid']."',ttid='".$add['ttid']."',onclick='".$add['onclick']."',totaldown='".$add['totaldown']."',firsttitle='".$add['firsttitle']."',isgood='".$add['isgood']."',ispic='".$add['ispic']."',istop='".$add['istop']."',isurl='".$add['isurl']."',lastdotime='".$lastdotime."',titlefont='".addslashes($add['my_titlefont'])."',titleurl='".addslashes($infourl)."',title='".addslashes($add['title'])."',titlepic='".addslashes($add['titlepic'])."',newstime='".$newstime."' where id='".$add['id']."'".do_dblimit_upone());
	//副表
	$fsql=$empire->query("update ".$infotbr['datatbname']." set classid='".$add['classid']."',closepl='".$add['closepl']."'".$ret_r['datavalues']." where id='".$add['id']."'".do_dblimit_upone());
	//更新附件
	UpdateTheFileEdit($add['classid'],$add['id'],$checkr['fstb']);
	//是否改变审核状态
	if($index_checkr['checked']!=$newchecked)
...
function DelNews($id,$classid,$userid,$username){
	...
	$ccr=$empire->fetch1("select classid,modid,listdt,haddlist,repreinfo from {$dbtbpre}enewsclass where classid='$classid'".do_dblimit_one());
	if(!$ccr['classid'])
	{
		printerror("ErrorUrl","history.go(-1)");
	}
	//索引表
	$index_r=$empire->fetch1("select classid,checked from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_index  where id='$id'".do_dblimit_one());
	...
	$r=$empire->fetch1("select * from ".$infotb." where id='$id'".do_dblimit_one());
	...
	if($pf)
	{
		...
		{
			$finfor=$empire->fetch1("select ".$pf." from ".$infotbr['datatbname']." where id='$id'".do_dblimit_one());
			$r[$pf]=$finfor[$pf];
		}
	}
	//存文本
	...
	//efz
	DelInfoForFzData($class_r[$classid]['tbname'],$classid,$id,$index_r['checked'],$r['stb'],$r['efz'],'');
	...
	if($index_r['checked'])
	{
		...
		if($ccr['repreinfo'])
		{
			$prer=$empire->fetch1("select * from {$dbtbpre}ecms_".$tbname." where id<$id and classid='$classid' order by id desc".do_dblimit_one());
			$epreid=$prer['id'];
			GetHtml($prer['classid'],$prer['id'],$prer,1);
			//下一篇
			$nextr=$empire->fetch1("select * from {$dbtbpre}ecms_".$tbname." where id>$id and classid='$classid' order by id".do_dblimit_one());
			if($nextr['id'])
			{
				$epreid2=$nextr['id'];
...
function DelNews_all($id,$classid,$userid,$username,$ecms=0){
	...
	for($i=0;$i<$count;$i++)//删除信息文件
	{
		...
		DelInfoForFzData($tbname,$r['classid'],$id[$i],$efzchecked,$r['stb'],$r['efz'],'');
...
function EditMoreInfoTime($add,$userid,$username){
	...
	$infoid=eCheckEmptyArray($infoid);
	$count=count($infoid);
	$tbname=$class_r[$classid]['tbname'];
	...
	for($i=0;$i<$count;$i++)
	{
		...
		{
			//索引表
			$index_r=$empire->fetch1("select classid,checked from {$dbtbpre}ecms_".$tbname."_index where id='$doinfoid'".do_dblimit_one());
			if(!$index_r['classid'])
			{
				continue;
			}
...
function AddInfoToCopyInfo($classid,$id,$to_classid,$userid,$username,$usergroupr){
	...
	$classid=(int)$classid;
	$id=(int)$id;
	$cr=eCheckEmptyArray($to_classid);
	$count=count($cr);
	...
	$stf=$emod_r[$mid]['savetxtf'];
	//索引表
	$index_r=$empire->fetch1("select id,checked from {$dbtbpre}ecms_".$tbname."_index where id='$id'".do_dblimit_one());
	if(empty($index_r['id']))
	{
		return '';
	}
	...
	//副表
	$fr=$empire->fetch1("select ".ReturnSqlFtextF($mid)." from ".$infotbr['datatbname']." where id='$id'".do_dblimit_one());
	$r=array_merge($r,$fr);
	if($stf)//存放文本
	{
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$checked=(int)$checked;
		//索引表
		$empire->updatesql("insert into {$dbtbpre}ecms_".$tbname."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('".$newclassid."','".$checked."','".$r['newstime']."','".$r['truetime']."','".$r['truetime']."','".$r['havehtml']."');","ins");
		$l_id=$empire->lastid($dbtbpre.'ecms_'.$tbname.'_index','id');
		$infotbr=ReturnInfoTbname($tbname,$checked,$ret_r['tb']);
		//主表
		$empire->updatesql("insert into ".$infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard".$ret_r['fields'].") values('".$l_id."','".$newclassid."','".$r['ttid']."',0,0,0,'".$newspath."','".$filename."','".$r['userid']."','".StripAddsData($r['username'])."',0,0,'".$r['ispic']."',0,'".$isqf."',0,'".$r['isurl']."','".$r['truetime']."','".$r['truetime']."','".$r['havehtml']."','".$r['groupid']."','".$r['userfen']."','".StripAddsData($r['titlefont'])."','".StripAddsData($r['titleurl'])."','".$ret_r['tb']."','".$public_r['filedeftb']."','".$public_r['pldeftb']."','".StripAddsData($r['keyboard'])."'".$ret_r['values'].");","ins");
		//副表
		$empire->updatesql("insert into ".$infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$ret_r['datafields'].") values('".$l_id."','".$newclassid."','".$r['keyid']."','".$r['dokey']."','".$newstempid."','".$r['closepl']."',0,''".$ret_r['datavalues'].");","ins");
		//公共表
		...
		$empire->query("update ".$infotbr['tbname']." set filename='$filename'".$updateinfourl." where id='$l_id'".do_dblimit_upone());
		//处理函数
		DoMFun($class_r[$newclassid]['modid'],$newclassid,$l_id,1,0);
		//生成信息文件
		if($checked)
		{
			$addr=$empire->fetch1("select * from ".$infotbr['tbname']." where id='$l_id'".do_dblimit_one());
...
function EditInfoToCopyInfo($classid,$id,$userid,$username,$usergroupr){
	...
	for($i=1;$i<$count-1;$i++)
	{
		...
		//索引表
		$index_infor=$empire->fetch1("select classid,checked from {$dbtbpre}ecms_".$tbname."_index where id='$infoid'".do_dblimit_one());
		//返回表
		...
		if($stf)
		{
			...
			{
				$infor=$empire->fetch1("select stb,isurl,newspath,filename,efz from ".$update_infotb." where id='$infoid'".do_dblimit_one());
				if(!$infor['stb'])
				{
					continue;
				}
				...
				$infodr=$empire->fetch1("select ".$stf." from ".$update_infotbr['datatbname']." where id='$infoid'".do_dblimit_one());
				$r['newstext_url']=$infodr[$stf];
			}
			else
			{
				$infor=$empire->fetch1("select ".$stf.",stb,isurl,newspath,filename,efz from ".$update_infotb." where id='$infoid'".do_dblimit_one());
				if(!$infor['stb'])
				{
					continue;
				}
		...
		{
			//生成信息文件
			$addr=$empire->fetch1("select * from {$dbtbpre}ecms_".$tbname." where id='$infoid'".do_dblimit_one());
			GetHtml($addr['classid'],$addr['id'],$addr,1);
		}
		//更新动态缓存
		if($public_r['ctimeopen']&&$index_r['checked'])
...
function DelInfoToCopyInfo($classid,$id,$r,$userid,$username,$usergroupr,$pubcopyids){
	...
	for($i=1;$i<$count-1;$i++)
	{
		...
		//索引表
		$index_infor=$empire->fetch1("select checked from {$dbtbpre}ecms_".$tbname."_index where id='$infoid'".do_dblimit_one());
		//返回表
		$update_infotb=ReturnInfoMainTbname($tbname,$index_infor['checked']);
		//主表
		$infor=$empire->fetch1("select * from ".$update_infotb." where id='$infoid'".do_dblimit_one());
		if(!$infor['stb'])
		{
			continue;
		}
		...
		if($selectdataf)
		{
			$infodr=$empire->fetch1("select ".$selectdataf." from ".$update_infotbr['datatbname']." where id='$infoid'".do_dblimit_one());
		...
		//efz
		DelInfoForFzData($tbname,$infor['classid'],$infoid,$index_infor['checked'],$infor['stb'],$infor['efz'],'');
...
//增加信息更新用户最后保存
function AddInfoUserSaveLast($add,$userid,$username){
	...
	$userid=(int)$userid;
	$modid=(int)$class_r[$add['classid']]['modid'];
	$lasttitle=AddAddsData($add['title']);
	$lasttext=AddAddsData($add['newstext']);
	if(!$lasttitle||!$lasttext||!$modid)
	{
		return '';
	}
	if(!AddInfoUserSaveLastIsOpen($modid))
	{
		return '';
	}
	$time=time();
	$lastinfor=$empire->fetch1("select userid,lasttime from {$dbtbpre}enewsuser_save where userid='".$userid."'".do_dblimit_one());
	if(!$lastinfor['userid'])
	{
		$usql=$empire->query("insert into {$dbtbpre}enewsuser_save(userid,lasttitle,lasttext,lasttime,haveup) values('".$userid."','".addslashes($lasttitle)."','".addslashes($lasttext)."','".$time."',0);");
	}
	else
	{
		if($time-$lastinfor['lasttime']<$usavelast)
		{
			return '';
		}
		$usql=$empire->query("update {$dbtbpre}enewsuser_save set lasttitle='".addslashes($lasttitle)."',lasttext='".addslashes($lasttext)."',lasttime='".$time."',haveup=0 where userid='".$userid."'");
	}
}

//增加信息更新用户最后保存状态
function AddInfoUserSaveLastSt($modid,$userid,$username){
	...
	$usql=$empire->query("update {$dbtbpre}enewsuser_save set haveup=1 where userid='".$userid."'");
}
...
function TopNews_all($classid,$id,$istop,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		{
			$index_r=$empire->fetch1("select checked from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_index where id='$infoid'".do_dblimit_one());
			//返回表
			$infotb=ReturnInfoMainTbname($class_r[$classid]['tbname'],$index_r['checked']);
		}
		$donum++;
...
function CheckNews_all($classid,$id,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$infor=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_check where id='$infoid'".do_dblimit_one());
		...
		if($infor['ismember']&&$infor['userid'])
		{
			...
			if($cr['addinfofen'])
			{
				$finfor=$empire->fetch1("select haveaddfen from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_check_data where id='$infoid'".do_dblimit_one());
	...
	if($adddatar['causetext']&&$infoid)
	{
		if(!$infor['id'])
		{
			$infor=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where id='$infoid'".do_dblimit_one());
		}
...
function NoCheckNews_all($classid,$id,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		$infoid=(int)$id[$i];
		//主表
		$r=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where id='$infoid'".do_dblimit_one());
		if(!$r['id']||$r['isqf']==1)
		{
			continue;
		}
	...
	if($adddatar['causetext']&&$infoid)
	{
		if(!$r['id'])
		{
			$r=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_check where id='$infoid'".do_dblimit_one());
		}
...
function MoveNews_all($classid,$id,$to_classid,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		{
			//索引表
			$index_r=$empire->fetch1("select checked from {$dbtbpre}ecms_".$tbname."_index where id='".$id[$i]."'".do_dblimit_one());
			//返回表
			$infotb=ReturnInfoMainTbname($tbname,$index_r['checked']);
		}
		//主表
		$r=$empire->fetch1("select stb,classid,fstb,restb,id,isurl,filename,groupid,newspath,titleurl,title,ismember,userid,username,newstime,truetime,ttid,efz from ".$infotb." where id='".$id[$i]."'".do_dblimit_one());
		$haveefz=$r['efz'];
	...
	//efz
	PubFzinfoUpChangeMore($classid,$addids,$to_classid,$haveefz);
	//刷新列表
	ReListHtml($classid,1);
...
function CopyNews_all($classid,$id,$to_classid,$userid,$username){
	...
	while($index_r=$empire->fetch($dosql))
	{
		...
		$infotb=ReturnInfoMainTbname($tbname,$index_r['checked']);
		//主表
		$r=$empire->fetch1("select * from ".$infotb." where id='".$index_r['id']."'".do_dblimit_one());
		//返回表信息
		$infotbr=ReturnInfoTbname($tbname,$index_r['checked'],$r['stb']);
		//副表
		$finfor=$empire->fetch1("select ".ReturnSqlFtextF($mid)." from ".$infotbr['datatbname']." where id='".$r['id']."'".do_dblimit_one());
		...
		$checked=(int)$checked;
		//索引表
		$empire->updatesql("insert into {$dbtbpre}ecms_".$tbname."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('".$to_classid."','".$checked."','".$r['newstime']."','".$truetime."','".$truetime."','".$r['havehtml']."');","ins");
		$l_id=$empire->lastid($dbtbpre.'ecms_'.$tbname.'_index','id');
		$update_infotbr=ReturnInfoTbname($tbname,$checked,$ret_r['tb']);
		//主表
		$sql=$empire->updatesql("insert into ".$update_infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard".$ret_r['fields'].") values('".$l_id."','".$to_classid."','".$r['ttid']."',0,0,0,'".$newspath."','".$filename."','".$userid."','".$username."',0,0,'".$r['ispic']."',0,'".$isqf."',0,'".$r['isurl']."','".$truetime."','".$truetime."','".$r['havehtml']."','".$r['groupid']."','".$r['userfen']."','".$r['titlefont']."','".$r['titleurl']."','".$ret_r['tb']."','".$public_r['filedeftb']."','".$public_r['pldeftb']."','".$r['keyboard']."'".$ret_r['values'].");","ins");		
		//副表
		$empire->updatesql("insert into ".$update_infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$ret_r['datafields'].") values('".$l_id."','".$to_classid."','".$r['keyid']."','".$r['dokey']."','".$newstempid."','".$r['closepl']."',0,'".$r['infotags']."'".$ret_r['datavalues'].");","ins");
		//签发
		if($isqf==1)
		{
...
function MoveClassNews($add,$userid,$username){
	...
	//efz
	PubFzinfoUpChangeAll($add['classid'],$add['toclassid']);
	//生成信息列表
...
//批量推荐/头条信息
function GoodInfo_all($classid,$id,$isgood,$doing=0,$userid=0,$username=''){
...
function SetAllCheckInfo($bclassid,$classid,$userid,$username){
	...
	//附加链接参数
	$addecmscheck=empty($_POST['ecmscheck'])?'&ecmscheck=1':'';
	$sql=$empire->query("select id,classid,userid,ismember,isqf,stb,efz from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_check where classid='$classid'");
	$n=0;
...
function DoWfInfo($add,$userid,$username){
	...
	$wfinfor=$empire->fetch1("select id,checknum,wfid,tid,groupid,userclass,username,checktno from {$dbtbpre}enewswfinfo where id='$id' and classid='$classid'".do_dblimit_one());
	if(!$wfinfor['id'])
	{
		printerror('ErrorUrl','');
	}
	...
	if($doing==1)//通过
	{
		if($wfitemr['lztype']==0)//普签
		{
			...
			{
				$newwfitemr=$empire->fetch1("select tid,wfid,tno,groupid,userclass,username,lztype,tbdo,tddo,tstatus from {$dbtbpre}enewsworkflowitem where wfid='".$wfitemr['wfid']."' and tno>".$wfitemr['tno']." order by tno".do_dblimit_one());
				$empire->query("update {$dbtbpre}enewswfinfo set tid='".$newwfitemr['tid']."',groupid='".$newwfitemr['groupid']."',userclass='".$newwfitemr['userclass']."',username='".$newwfitemr['username']."',tstatus='".$newwfitemr['tstatus']."',checktno='0' where id='$id' and classid='$classid'".do_dblimit_upone());
				//日志
				InsertWfLog($classid,$id,$wfitemr['wfid'],$wfitemr['tid'],$username,$checktext,$wfinfor['checknum'],1);
			}
		}
		else//会签
		{
			...
			//下一个节点
			if(($newgroupid==''||$newgroupid==',')&&($newuserclass==''||$newuserclass==',')&&($newusername==''||$newusername==','||$newusername==',,'))
			{
				if($wfitemr['tno']=='100')//全部通过
				{
					$empire->query("update {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_index set checked=1 where id='$id'");
			...
			{
				$empire->query("update {$dbtbpre}enewswfinfo set groupid='$newgroupid',userclass='$newuserclass',username='$newusername' where id='$id' and classid='$classid'".do_dblimit_upone());
				//日志
				InsertWfLog($classid,$id,$wfitemr['wfid'],$wfitemr['tid'],$username,$checktext,$wfinfor['checknum'],1);
			}
		}
	...
	elseif($doing==2)//返工
	{
		...
		{
			$empire->query("update {$dbtbpre}enewswfinfo set tid=0,tstatus='',checktno='101' where id='$id' and classid='$classid'".do_dblimit_upone());
		}
		else//发给节点
		{
			$newwfitemr=$empire->fetch1("select tid,wfid,tno,groupid,userclass,username,lztype,tbdo,tddo,tstatus from {$dbtbpre}enewsworkflowitem where wfid='".$wfitemr['wfid']."' and tid='".$wfitemr['tbdo']."'".do_dblimit_one());
			$empire->query("update {$dbtbpre}enewswfinfo set tid='".$newwfitemr['tid']."',groupid='".$newwfitemr['groupid']."',userclass='".$newwfitemr['userclass']."',username='".$newwfitemr['username']."',tstatus='".$newwfitemr['tstatus']."',checktno='101' where id='$id' and classid='$classid'".do_dblimit_upone());
		}
		//日志
	...
	else//否决
	{
		...
		$empire->query("update {$dbtbpre}enewswfinfo set tid=0,tstatus='',checktno='102' where id='$id' and classid='$classid'".do_dblimit_upone());
		//日志
		InsertWfLog($classid,$id,$wfitemr['wfid'],$wfitemr['tid'],$username,$checktext,$wfinfor['checknum'],3);
		if($wfitemr['tddo'])//删除信息
		{
			$index_r=$empire->fetch1("select checked from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_index where id='$id'".do_dblimit_one());
			//返回表
			$infotb=ReturnInfoMainTbname($class_r[$classid]['tbname'],$index_r['checked']);
			$r=$empire->fetch1("select * from ".$infotb." where id='$id'".do_dblimit_one());
			$mid=$class_r[$classid]['modid'];
			...
			//efz
			DelInfoForFzData($tbname,$r['classid'],$id,$index_r['checked'],$r['stb'],$r['efz'],'');
			DelNewsFile($r['filename'],$r['newspath'],$classid,$r[$pf],$r['groupid']);//删除信息文件
...
function DelInfoData($start,$classid,$from,$retype,$startday,$endday,$startid,$endid,$tbname,$add,$userid,$username){
	...
	$b=0;
	$sql=$empire->query("select * from ".$infotb." where id>$start".$add1." order by id".do_dblimit($public_r['delnewsnum']));
	while($r=$empire->fetch($sql))
	{
		$b=1;
		$new_start=$r['id'];
		...
		if($pf)
		{
			if(strstr($emod_r[$mid]['tbdataf'],','.$pf.','))
			{
				$finfor=$empire->fetch1("select ".$pf." from ".$infodatatb." where id='".$r['id']."'".do_dblimit_one());
				$r[$pf]=$finfor[$pf];
			}
		}
		//存文本
		...
		//efz
		$efzchecked=$delckinfo==2?0:1;
		DelInfoForFzData($tbname,$r['classid'],$r['id'],$efzchecked,$r['stb'],$r['efz'],'');
		//删除信息文件
	...
	if(empty($b))
	{
		...
		{
			echo $fun_r['DelDataSuccess']."<script>self.location.href='ecmsinfo.php?enews=DelInfoData&start=0&from=".urlencode($from)."&delhtml=".$add['delhtml']."&dodelcheck=1".$search.hReturnEcmsHashStrHref(0).heformhash_get('DelInfoData',1)."';</script>";
			exit();
		}
	    //操作日志
	    insert_dolog("");
	...
	echo $fun_r['OneDelDataSuccess']."(ID:<font color=red><b>".$new_start."</b></font>)<script>self.location.href='ecmsinfo.php?enews=DelInfoData&start=$new_start&from=".urlencode($from)."&delhtml=".$add['delhtml'].$search.hReturnEcmsHashStrHref(0).heformhash_get('DelInfoData',1)."';</script>";
	exit();
}
...
function InfoToDoc_class($add,$userid,$username){
	...
	$sql=$empire->query("select * from {$dbtbpre}ecms_".$cr['tbname']." where id>$start and classid='$classid' and truetime<$doctime order by id".do_dblimit($line));
	while($r=$empire->fetch($sql))
	{
		$b=1;
		$new_start=$r['id'];
	...
	echo $fun_r['OneInfoToDocSuccess']."(ID:<font color=red><b>".$new_start."</b></font>)<script>self.location.href='ecmsinfo.php?enews=InfoToDoc&ecmsdoc=1&classid=$classid&start=$new_start&docfrom=".urlencode($add['docfrom']).hReturnEcmsHashStrHref(0).heformhash_get('InfoToDoc',1)."';</script>";
	exit();
}
...
function InfoToDoc($add,$userid,$username){
	...
	$line=$public_r['docnewsnum'];
	$b=0;
	$sql=$empire->query("select * from {$dbtbpre}ecms_".$selecttbname." where id>$start and ".$where." order by id".do_dblimit($line));
	while($r=$empire->fetch($sql))
	{
		$b=1;
		$new_start=$r['id'];
	...
	echo $fun_r['OneInfoToDocSuccess']."(ID:<font color=red><b>".$new_start."</b></font>)<script>self.location.href='ecmsinfo.php?enews=InfoToDoc&ecmsdoc=2&doing=".$add['doing']."&tbname=$tbname&start=$new_start&docfrom=".urlencode($add['docfrom']).$search.hReturnEcmsHashStrHref(0).heformhash_get('InfoToDoc',1)."';</script>";
	exit();
}
...
function DoDocInfo($tb,$r,$ecms=0){
	...
	$mid=$class_r[$r['classid']]['modid'];
	//索引表
	$index_r=$empire->fetch1("select * from ".$table." where id='".$r['id']."'".do_dblimit_one());
	...
	//主表
	$empire->query(do_dbReplaceInto()." into ".$ytable1."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard,efz".$ret_r['fields'].") values('".$r['id']."','".$r['classid']."','".$r['ttid']."','".$r['onclick']."','".$r['plnum']."','".$r['totaldown']."','".StripAddsData($r['newspath'])."','".StripAddsData($r['filename'])."','".$r['userid']."','".StripAddsData($r['username'])."','".$r['firsttitle']."','".$r['isgood']."','".$r['ispic']."','".$r['istop']."','".$r['isqf']."','".$r['ismember']."','".$r['isurl']."','".$r['truetime']."','".$r['lastdotime']."','".$r['havehtml']."','".$r['groupid']."','".$r['userfen']."','".StripAddsData($r['titlefont'])."','".StripAddsData($r['titleurl'])."','".$r['stb']."','".$r['fstb']."','".$r['restb']."','".StripAddsData($r['keyboard'])."','".$r['efz']."'".$ret_r['values'].");");
	//副表
	$empire->query(do_dbReplaceInto()." into ".$ytable2."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags,efzstb".$ret_r['datafields'].") values('".$r['id']."','".$r['classid']."','".$r['keyid']."','".$r['dokey']."','".$r['newstempid']."','".$r['closepl']."','".$r['haveaddfen']."','".StripAddsData($r['infotags'])."','".StripAddsData($r['efzstb'])."'".$ret_r['datavalues'].");");
	//删除
...
function DoDocCkInfo($tb,$where,$ecms=0){
	...
	$empire->query(do_dbReplaceInto(1)." into ".$ytable." select * from ".$table." where ".$where);
	$empire->query(do_dbReplaceInto(1)." into ".$ytable1." select * from ".$table1." where ".$where);
	$empire->query(do_dbReplaceInto(1)." into ".$ytable2." select * from ".$table2." where id in (".$fids.")");
	//删除
