...
function CheckQdoinfo($classid,$id,$userid,$tbname,$adminqinfo,$ecms=0){
	...
	$r=$empire->fetch1("select * from ".$infotb." where id='$id' and classid='$classid' and ismember=1 and userid='$userid'".do_dblimit_one());
	if(!$r['id'])
	{
		printerror("HaveNotLevelQInfo","history.go(-1)",1);
	}
...
function DoReqDownPath($downpath){
	...
	$r1[1]=addslashes(dgdbe_rpstr($r1[1],1));
	return $r1[1];
}
...
function DoqSpecialValue($mid,$f,$value,$add,$infor,$ecms=0){
	...
	{
		$add['msavepic']=0;
		$value=QReturnMorepicpath($add['msmallpic'],$add['mbigpic'],$add['mpicname'],$add['mdelpicid'],$add['mpicid'],$add,$add['mpicurl_qz'],$ecms,0,($ecms==1?$infor['fstb']:$public_r['filedeftb']));
...
//验证字段类型(前)
function qEModCkFieldType($tbname,$f,$fval){
	global $emod_pubr;
	//ID列表
...
function ReturnQAddinfoF($mid,$add,$infor,$classid,$filepass,$userid,$username,$ecms=0){
	...
	$ret_r=array();
	$pr=$empire->fetch1("select qaddtran,qaddtransize,qaddtranimgtype,qaddtranfile,qaddtranfilesize,qaddtranfiletype,closewords,closewordsf from {$dbtbpre}enewspublic".do_dblimit_one());
	$isadd=$ecms==0?1:0;
	...
	for($i=1;$i<$count;$i++)
	{
		...
		elseif($f=='writer'||$f=='befrom')	//wb
		{
			$fval=RepPostVar($add[$f]);
		}
		elseif($f=='infozm')	//字母
		{
		...
		$fval=qEModCkFieldType($emod_r[$mid]['tbname'],$f,$fval);//类型
		...
		if($ecms==1)
		{
			SameDataAddF($infor['id'],$classid,$mid,$f,$fval);
		}
		...
		{
			if($f=='infoip'||$f=='infoipport'||$f=='eipf')	//ip
			{
				continue;
			}
			if(strstr($emod_r[$mid]['tbdataf'],','.$f.','))//副表
	...
	if($tranf)
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			$tffile=$tf."file";
			//扩展名
			$tffiletype=GetFiletype($_FILES[$tffile]['name']);//取得文件类型
			...
			{
				if(!strstr($pr['qaddtranimgtype'],"|".$tffiletype."|"))
				{
					printerror("NotQTranFiletype","",1);
				}
				if(!strstr($ecms_config['sets']['tranpicturetype'],','.$tffiletype.','))
				{
					printerror("NotQTranFiletype","",1);
				}
			}
			...
			{
				if(!strstr($pr['qaddtranfiletype'],"|".$tffiletype."|"))
				{
					printerror("NotQTranFiletype","",1);
				}
				if(strstr($emod_r[$mid]['flashf'],','.$tf.','))//flash
				{
					if(!strstr($ecms_config['sets']['tranflashtype'],",".$tffiletype.","))
					{printerror("NotQTranFiletype","",1);}
				}
				if($f=="onlinepath")//视频
				{
					if(strstr($wmv_type,",".$tffiletype.","))
					{}
				}
			}
			...
			if($tfr['tran'])
			{
				...
				$classid=(int)$classid;
				eInsertFileTable($tfr['filename'],$filesize,$tfr['filepath'],'[Member]'.$username,$classid,'['.$tf.']'.addslashes(dgdbe_rpstr($add['title'])),$type,$infoid,$filepass,$public_r['fpath'],0,0,($ecms==1?$infor['fstb']:$public_r['filedeftb']));
				//删除旧文件
				if($ecms==1&&$infor[$tf])
				{
...
function DelYQTranFile($classid,$id,$file,$tf,$fstb='1'){
	...
	$fr=$empire->fetch1("select filename,path,fileid,fpath,classid from {$dbtbpre}enewsfile_{$fstb} where classid='$classid' and id='$id' and filename='$filename'".do_dblimit_one());
	if($fr['fileid'])
	{
...
function DodoInfo($add,$ecms=0){
	global $empire,$public_r,$emod_r,$level_r,$class_r,$dbtbpre,$fun_r,$etable_t;
	//验证来源
	if($ecms==0||$ecms==1)
	{
		CheckCanPostUrl();
	...
	if(!$tbname||!$qenter||$qenter==','||InfoIsInTable($tbname))
	{
	...
	$infor=array();
	//组合标题属性
	$titlecolor=addslashes(dgdbe_rpstr(RepPhpAspJspcodeText($add['titlecolor'])));
	$titlefont=dgdbe_rpstr(TitleFont($add['titlefont'],$titlecolor));
	$titlecolor="";
	$titlefont="";
	$ttid=(int)$add['ttid'];
	$add['keyboard']=RepPostVar($add['keyboard']);
	$keyboard=addslashes(dgdbe_rpstr(trim(DoReplaceQjDh($add['keyboard']))));
	$keyid='';
	//返回关键字组合
	...
	if($fztid&&$fzid)
	{
		...
		if(!$ftbname||InfoIsInTable($ftbname))
		{
	...
	if($ecms==0)
	{
		...
		if(empty($muserid))
		{
			$musername=dgdbe_rpstr($fun_r['guest']);
		}
		//会员投稿数更新
		if($setuserday)
		{
		...
		//索引表
		$indexsql=$empire->updatesql("insert into {$dbtbpre}ecms_".$tbname."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('$classid','$checked','$newstime','$truetime','$truetime','$havehtml');","ins");
		$id=$empire->lastid($dbtbpre.'ecms_'.$tbname.'_index','id');
		//返回表信息
		...
		if($haveaddfen)
		{
			if($cr['addinfofen']<0)
			{
				BakDown($classid,$id,0,$muserid,$musername,addslashes(dgdbe_rpstr($add['title'])),abs($cr['addinfofen']),3);
			}
		}
		//签发
		if($isqf==1)
		...
		if($fzdoadd==1)
		{
			$edbfunc_fzr['tbname']=$tbname;
			$edbfunc_fzr['classid']=(int)$classid;
			$edbfunc_fzr['id']=(int)$id;
			$edbfunc_fzr['newstime']=(int)$newstime;
			$edbfunc_fzr['checked']=(int)$checked;
			$edbfunc_fzr['fzupfc']=0;
			if($edbfunc_fzr['checked'])
			{
				$edbfunc_fzr['fzupfc']=1;
			}
			PubAddOneInfoToFzData($edbfunc_fzr['fzclassid'],$edbfunc_fzr['fzid'],$edbfunc_fzr['fzbcid'],$edbfunc_fzr['fzcid'],$edbfunc_fzr['tbname'],$edbfunc_fzr['classid'],$edbfunc_fzr['id'],$edbfunc_fzr['newstime'],$edbfunc_fzr['checked'],0,$edbfunc_fzr['fzupfc'],1);
		}
		//清除验证码
	...
	elseif($ecms==1)
	{
		...
		if($infor['isqf'])
		{
			$qck_qfr=$empire->fetch1("select wfid,checktno from {$dbtbpre}enewswfinfo where id='".$infor['id']."' and classid='".$infor['classid']."'".do_dblimit_one());
			if($qck_qfr['checktno']<100)
			{
				$qck_qfwfr=$empire->fetch1("select wfid,canedit from {$dbtbpre}enewsworkflow where wfid='".$qck_qfr['wfid']."'".do_dblimit_one());
		...
		if($keyboard)
		{
			$addfield=",keyboard='".addslashes($keyboard)."'";
			$faddfield=",keyid='".addslashes($keyid)."'";
		}
		//时间
		if(strstr($qenter,',newstime,'))
		{
	...
	elseif($ecms==2)
	{
		...
		//efz
		DelInfoForFzData($tbname,$classid,$id,$r['checked'],$r['stb'],$r['efz'],$r['efzstb']);
		//删除信息文件
...
function DoQCheckAddLevel($classid,$userid,$username,$rnd,$ecms=0,$isadd=0){
	...
	//是否登陆
	if($ecms==1||$ecms==2||$r['qaddgroupid'])
	{
	...
	if($r['qaddgroupid'])
	{
		$mvgresult=eMember_ReturnCheckMVGroup($user,$r['qaddgroupid']);
	...
	if($isadd==1)
	{
		...
		//检测投稿数
		if($r['qaddgroupid']&&$level_r[$user['groupid']]['dayaddinfo'])
		{
	...
	if(($ecms==0||$ecms==1)&&$userid)
	{
		...
		if($level_r[$user['groupid']]['infochecked']==-1)
		{
...
function DoQCheckAddNum($userid,$groupid){
	global $empire,$dbtbpre,$level_r,$public_r;
	$userid=(int)$userid;
	$ur=$empire->fetch1("select userid,todayinfodate,todayaddinfo from {$dbtbpre}enewsmemberpub where userid='$userid'".do_dblimit_one());
	$thetoday=date("Y-m-d");
	if($ur['userid'])
	{
	...
	{
		$query=do_dbReplaceInto()." into {$dbtbpre}enewsmemberpub(userid,todayinfodate,todayaddinfo) values('$userid','$thetoday',1);";
	}
	return $query;
}
...
function DoQTranFile($add,$file,$file_name,$file_type,$file_size,$userid,$username,$rnd,$ecms=0){
	...
	if(!$file_name||!$filepass||!$classid||!$class_r[$classid]['tbname']||InfoIsInTable($class_r[$classid]['tbname']))
	{
	...
	$pr=$empire->fetch1("select qaddtran,qaddtransize,qaddtranimgtype,qaddtranfile,qaddtranfilesize,qaddtranfiletype from {$dbtbpre}enewspublic".do_dblimit_one());
	if($type==1)//图片
	{
		if(!$pr['qaddtran'])
		{
...
function eCkQEdToText($mpid,$mid,$tbname,$f){
	...
	if(strstr($public_r['qedmpids'],','.$mpid.','))
	{
		return 1;
	}
	if(strstr($public_r['qedmids'],','.$mid.','))
	{
		return 1;
	}
	return 0;
