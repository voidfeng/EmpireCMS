...
function EditGbooktemp($temptext,$userid,$username){
	...
	$gid=(int)$_POST['gid'];
	$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set gbooktemp='".eaddslashes2($temptext)."'".do_dblimit_upone());
	//备份模板
...
function EditCptemp($temptext,$userid,$username){
	...
	$temptext=RepPhpAspJspcode($temptext);
	$gid=(int)$_POST['gid'];
	$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set cptemp='".eaddslashes2($temptext)."'".do_dblimit_upone());
	//备份模板
...
function EditLoginIframe($temptext,$userid,$username){
	...
	$temptext=RepPhpAspJspcode($temptext);
	$gid=(int)$_POST['gid'];
	$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set loginiframe='".eaddslashes2($temptext)."'".do_dblimit_upone());
	//备份模板
...
function EditLoginJstemp($temptext,$userid,$username){
	...
	$temptext=RepPhpAspJspcode($temptext);
	$gid=(int)$_POST['gid'];
	$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set loginjstemp='".eaddslashes2($temptext)."'".do_dblimit_upone());
	//备份模板
...
function EditSchallTemp($temptext,$sub,$formatdate,$userid,$username){
	...
	$gid=(int)$_POST['gid'];
	$sub=(int)$sub;
	$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set schalltemp='".eaddslashes2($temptext)."',schallsubnum='$sub',schalldate='".eaddslashes($formatdate)."'".do_dblimit_upone());
	//备份模板
...
function AddBq($add,$bqsay,$userid,$username){
	...
	//标签重复
	$num=$empire->gettotal("select count(*) as total from {$dbtbpre}enewsbq where bq='".$add['bq']."'".do_dblimit_cone());
	if($num)
	...
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsbq(bqname,bqsay,funname,bq,issys,bqgs,isclose,classid,myorder) values('".$add['bqname']."','".eaddslashes2($bqsay)."','".$add['funname']."','".$add['bq']."',0,'".eaddslashes2($add['bqgs'])."','".$add['isclose']."','".$classid."','".$myorder."');","ins");
	$bqid=$empire->lastid($dbtbpre.'enewsbq','bqid');
	if($sql)
	{
		//操作日志
...
function EditBq($add,$bqsay,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"bq");
	//标签重复
	$num=$empire->gettotal("select count(*) as total from {$dbtbpre}enewsbq where bq='".$add['bq']."' and bqid<>'".$add['bqid']."'".do_dblimit_cone());
	if($num)
	{printerror("ReBq","history.go(-1)");}
	//函数是否存在
	if(!function_exists($add['funname']))
...
function EditIndextemp($temptext,$userid,$username){
	...
	if($gid==$public_r['deftempid']||(!$public_r['deftempid']&&($gid==1||$gid==0)))
	{
		...
		//更新动态页面缓存
		$ecms_fclast=time();
		$empire->query("update {$dbtbpre}enewspublic_fc set fclastindex='$ecms_fclast'".do_dblimit_upone());
	}
	if($sql)
	{
...
function PreviewIndexpage($tempid,$userid,$username){
	...
	$pr=$empire->fetch1("select sitekey,siteintro from {$dbtbpre}enewspublic".do_dblimit_one());
	//页面
	...
	$onclick='';
	$file=ECMS_PATH.'c/ecachetmp/etmprefile/indexpage'.$tempid.Moreport_eReturnReTempGidF().'.php';
...
function LoadTempInClass($path,$start,$userid,$username){
	...
	$b=0;
	$sql=$empire->query("select classid,classtempid,islist from {$dbtbpre}enewsclass where islast=0 and islist<>1 and classid>$start order by classid".do_dblimit($public_r['loadtempnum']));
	while($r=$empire->fetch($sql))
	{
		...
		$file="../../c/ecachepub/eloadtemp/".$r['classid'].".htm";
		if(file_exists($file))
		{
	...
	echo $fun_r['LoadOneTempSuccess']."(ID:<font color=red><b>".$newstart."</b></font>)<script>self.location.href='ecmstemp.php?enews=LoadTempInClass&start=$newstart".hReturnEcmsHashStrHref(0).heformhash_get('LoadTempInClass',1)."';</script>";
	exit();
}
...
function LoadOutBq($add,$userid,$username){
	...
	$filepath=ECMS_PATH.'c/ecachetmp/etmploadfile/eltemp/'.$filename;
...
function LoadInBq($add,$file,$file_name,$file_type,$file_size,$userid,$username){
	...
	$field="<!--#empirecms.bq-phome.net#--!>";
	$path=ECMS_PATH.'c/ecachetmp/etmploadfile/eltemp/uploadbq'.time().'.bq';
	//上传文件
	...
	$num=$empire->gettotal("select count(*) as total from {$dbtbpre}enewsbq where bq='".$r[3]."' or funname='".$r[2]."'".do_dblimit_cone());
	if($num)
	{
		printerror("ReLoadInBq","history.go(-1)");
	}
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsbq(bqname,bqsay,funname,bq,issys,bqgs,isclose,classid,myorder) values('".addslashes($r[0])."','".addslashes(addslashes($r[1]))."','".addslashes($r[2])."','".addslashes($r[3])."',0,'".addslashes(addslashes($r[4]))."',0,'$classid',0);","ins");
	$bqid=$empire->lastid($dbtbpre.'enewsbq','bqid');
	//操作日志
...
function DoRepTemp($add,$userid,$username){
	...
	if($add['indextemp']||$add['cptemp']||$add['sformtemp']||$add['otherlinktemp']||$add['gbooktemp']||$add['loginiframe']||$add['pljstemp']||$add['schalltemp']||$add['loginjstemp']||$add['downpagetemp'])
	{
		...
		$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set id=1".$set."".do_dblimit_upone());
	}
	//修改栏目封面模板
	if($add['classtemp'])
	{
...
function DefTtempGroup($add,$userid,$username){
	...
	$upsql=$empire->query("update {$dbtbpre}enewspublic set deftempid=$gid".do_dblimit_upone());
	if($usql&&$sql&&$upsql)
	{
		GetConfig();
		//操作日志
...
function DelTempTb($gid){
	...
	$sql=do_dbTableDelTb($droptb,-1);//pgsql
	return $sql;
}
...
function ClearTempTb($gid,$en){
	...
	{
		$tb=$dbtbpre.$r[$i].$en;
		do_dbTableClearTbData($tb);//pgsql
	}
}
...
function DelTempGroup($add,$userid,$username){
	...
	if($r['isdefault'])
	{
		$upsql=$empire->query("update {$dbtbpre}enewspublic set deftempid=0".do_dblimit_upone());
		//GetConfig();
	}
	DelTempTb($gid);
	//删除备份记录
...
function LoadTempGroup($add,$userid,$username){
	...
	$file="e".time().".temp";
	$filepath=ECMS_PATH.'c/ecachetmp/etmploadfile/eltemp/'.$file;
...
function LoadInTempChangeChar($tempchar,$text){
	...
	elseif($tempchar=='UTF8')//简体UTF-8
	{
		...
		if($ecms_config['sets']['pagechar']=='gb2312')
		{
			$text=DoIconvVal("UTF8","GB2312",$text);
	...
	elseif($tempchar=='TCUTF8')//繁体UTF-8
	{
		...
		if($ecms_config['sets']['pagechar']=='gb2312')
		{
			$text=DoIconvVal("UTF8","BIG5",$text);
			$text=DoIconvVal("BIG5","GB2312",$text);
			$text=preg_replace('/charset=utf-8/i','charset=gb2312',$text);
		}
		elseif($ecms_config['sets']['pagechar']=='big5')
...
function LoadInTempGroup($add,$file,$file_name,$file_type,$file_size,$userid,$username){
	...
	//上传文件
	$path=ECMS_PATH.'c/ecachetmp/etmploadfile/eltemp/uploadtg'.time().make_password(10).'.temp';
	if(!eCkIsTranFile($file))
	{
		printerror("LoadInTempGroupMusttemp","");
	}
	...
	if(empty($gid))//新建模板组
	{
		$sql=$empire->updatesql("insert into {$dbtbpre}enewstempgroup(gname,isdefault) values('".addslashes($gname)."',0);","ins");
		$gid=$empire->lastid($dbtbpre.'enewstempgroup','gid');
		$gname=$pr[0];
		$en="_".$gid;
...
function LoadTGPubtemp($gid,$en,$pageexp,$record,$field){
	...
	$r=$empire->fetch1("select * from ".$tb."".do_dblimit_one());
...
function DelEBakTemp($temptype,$gid,$tempid){
	global $empire,$dbtbpre;
	$pr=$empire->fetch1("select baktempnum from {$dbtbpre}enewspublic".do_dblimit_one());
	if(!$pr['baktempnum'])
	{
		return $pr;
	}
	...
	if($num+1>$pr['baktempnum'])
	{
		...
		$ids='';
		$dh='';
		$sql=$empire->query("select bid from {$dbtbpre}enewstempbak where temptype='$temptype' and gid='$gid' and tempid='$tempid' order by bid".do_dblimit($limitnum));
		while($r=$empire->fetch($sql))
		{
			$ids.=$dh.$r['bid'];
			$dh=',';
...
function ReEBakTemp($add,$userid,$username){
	...
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set indextemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubcptemp')//控制面板模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set cptemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubsearchtemp')//高级搜索表单模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set searchtemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubsearchjstemp')//搜索JS模板[横向]
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set searchjstemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubsearchjstemp1')//搜索JS模板[纵向]
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set searchjstemp1='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubotherlinktemp')//相关链接模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set otherlinktemp='".addslashes($r['temptext'])."',otherlinktempsub='".$r['subtitle']."',otherlinktempdate='".StripAddsData($r['showdate'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubdownsofttemp')//下载地址模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set downsofttemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubonlinemovietemp')//在线播放地址模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set onlinemovietemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='publistpagetemp')//列表分页模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set listpagetemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubpljstemp')//评论JS调用模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set pljstemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubdownpagetemp')//最终下载页模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set downpagetemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubgbooktemp')//留言板模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set gbooktemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='publoginiframe')//登陆状态模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set loginiframe='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='publoginjstemp')//JS调用登陆状态模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set loginjstemp='".addslashes($r['temptext'])."'".do_dblimit_upone());
	}
	elseif($r['temptype']=='pubschalltemp')//全站搜索模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set schalltemp='".addslashes($r['temptext'])."',schallsubnum='".$r['subnews']."',schalldate='".StripAddsData($r['showdate'])."'".do_dblimit_upone());
	}
	if($sql)
	{
		//操作日志
...
//模板ID同步(增加模板)
function sametg_AddTemp($tbname,$gid,$tempid,$tempname,$myvar=''){
	global $empire,$dbtbpre,$public_r;
	$gid=(int)$gid;
	$tempid=(int)$tempid;
	$tempname=hRepPostStr($tempname,1);
	$tempname=$tempname.'-S'.$gid;
	if(!$public_r['sametgids']||!strstr($public_r['sametgids'],','.$gid.','))
	{
		return '';
	}
	if(!strstr($public_r['sametgdo'],',add,'))
	{
		return '';
	}
	$gr=explode(',',$public_r['sametgids']);
	$count=count($gr)-1;
	if($count<2)
	{
		return '';
	...
	$f='tempid,tempname';
	$v="'".$tempid."','".$tempname."'";
	if($tbname=='enewstempvar')//公共模板变量
	{
		$myvar=hRepPostStr($myvar,1);
		$idf='varid';
		$f='varid,myvar,varname';
		$v="'".$tempid."','".$myvar."','".$tempname."'";
	}
	for($i=1;$i<$count;$i++)
	{
		$thisgid=(int)$gr[$i];
		...
		$cktr=$empire->fetch1("select gid from {$dbtbpre}enewstempgroup where gid='$thisgid'");
		if(!$cktr['gid'])
		{
			continue;
		}
		$temptb=GetDoTemptb($tbname,$thisgid);
		$tnum=$empire->gettotal("select count(*) as total from ".$temptb." where ".$idf."='".$tempid."'".do_dblimit_cone());
		if($tnum)
		{
			continue;
		}
...
//模板ID同步(删除模板)
function sametg_DelTemp($tbname,$gid,$tempid){
	global $empire,$dbtbpre,$public_r;
	$gid=(int)$gid;
	$tempid=(int)$tempid;
	if(!$public_r['sametgids']||!strstr($public_r['sametgids'],','.$gid.','))
	...
	if(!strstr($public_r['sametgdo'],',del,'))
	{
		return '';
	}
	$gr=explode(',',$public_r['sametgids']);
...
//模板ID同步(修改ID)
function sametg_EditTempid($tbname,$gid,$tempid,$newtempid,$redo){
	global $empire,$dbtbpre,$public_r;
	$gid=(int)$gid;
	$tempid=(int)$tempid;
	$newtempid=(int)$newtempid;
	$redo=(int)$redo;
	if(!$public_r['sametgids']||!strstr($public_r['sametgids'],','.$gid.','))
	{
		return '';
	}
	if(!strstr($public_r['sametgdo'],',editid,'))
	{
		return '';
	}
	$gr=explode(',',$public_r['sametgids']);
	...
	for($i=1;$i<$count;$i++)
	{
		...
		$temptb=GetDoTemptb($tbname,$thisgid);
		//原
		$otr=$empire->fetch1("select ".$idf." from ".$temptb." where ".$idf."='$tempid'");
		if(!$otr[$idf])
		{
			continue;
		}
		//新
		$ntr=$empire->fetch1("select ".$idf." from ".$temptb." where ".$idf."='$newtempid'");
		//ID已存在
		$lastid=0;
		if($ntr[$idf])
		{
			$lastr=$empire->fetch1("select ".$idf." from ".$temptb." order by ".$idf." desc".do_dblimit_one());
			$lastid=$lastr[$idf]+1;
			$empire->query("update ".$temptb." set ".$idf."='$lastid' where ".$idf."='$newtempid'");
		}
		$empire->query("update ".$temptb." set ".$idf."='$newtempid' where ".$idf."='$tempid'");
		//对换
		if($ntr[$idf]&&$redo==1&&$lastid)
		{
