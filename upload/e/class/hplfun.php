...
function UpdateAllInfoPlnum($add,$userid,$username){
	...
	echo $fun_r['OneUpdateInfoPlnumSuccess']."(ID:<font color=red><b>".$new_start."</b></font>)<script>self.location.href='ecmspl.php?enews=UpdgxAllInfoPlnum&tbname=$tbname&classid=$classid&start=$new_start&from=".urlencode($from)."&retype=$retype&startday=$startday&endday=$endday&startid=$startid&endid=$endid&doids=$whereid".hReturnEcmsHashStrHref(0).heformhash_get('UpdgxAllInfoPlnum',1)."';</script>";
	exit();
}
...
function CheckRePlF($add,$ecms=0){
	...
	//主表
	$s=do_dbFieldRowList($dbtbpre.'enewspl_1');
	$b=0;
...
function AddPlF($add,$userid,$username){
	...
	//新增字段
	$tbr=$empire->fetch1("select pldatatbs from {$dbtbpre}enewspl_set".do_dblimit_one());
	if($tbr['pldatatbs'])
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			do_dbTableAddF($dbtbpre.'enewspl_'.$dtbr[$i],$add['f'],$add['ftype'],$add['flen'],'');//pgsql
		}
	}
	//处理变量
	$add['ismust']=(int)$add['ismust'];
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsplf(f,fname,fzs,ftype,flen,ismust) values('".$add['f']."','".$add['fname']."','".addslashes($add['fzs'])."','".$add['ftype']."','".$add['flen']."','".$add['ismust']."');","ins");
	$lastid=$empire->lastid($dbtbpre.'enewsplf','fid');
	UpdatePlF();//更新字段
	GetConfig();//更新缓存
	if($sql)
	{
...
function EditPlF($add,$userid,$username){
	...
	if($cr['f']<>$add['f']||$cr['ftype']<>$add['ftype']||$cr['flen']<>$add['flen'])
	{
		...
		if($tbr['pldatatbs'])
		{
			...
			for($i=1;$i<$count-1;$i++)
			{
				do_dbTableEditF($dbtbpre.'enewspl_'.$dtbr[$i],$add['f'],$cr['f'],$add['ftype'],$add['flen'],'');//pgsql
			}
		}
	}
	//处理变量
...
function DelPlF($add,$userid,$username){
	...
	if($tbr['pldatatbs'])
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			do_dbTableDelF($dbtbpre.'enewspl_'.$dtbr[$i],$r['f']);//pgsql
		}
	}
...
function UpdatePlF(){
	...
	$empire->query("update {$dbtbpre}enewspl_set set plf='$plf',plmustf='$plmustf'".do_dblimit_upone());
}
