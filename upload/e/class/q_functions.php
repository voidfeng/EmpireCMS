...
function AddFeedback($add){
	...
	$pr=$empire->fetch1("select feedbacktfile,feedbackfilesize,feedbackfiletype from {$dbtbpre}enewspublic".do_dblimit_one());
	//必填项
	$addf='';
	$addval='';
	...
	for($i=0;$i<$count-1;$i++)
	{
		...
		$addf.=",".$f."";
		$addval.=",'".addslashes(dgdbe_rpstr($fval))."'";
	}
	$type=0;
	$classid=0;
	$filename='';
	...
	if($tranf)
	{
		...
		for($i=0;$i<$count;$i++)
		{
			$tf=$tranr[$i];
			//扩展名
			$tffiletype=GetFiletype($_FILES[$tf]['name']);//取得文件类型
			if(CheckSaveTranFiletype($tffiletype))
			{
				printerror("NotQTranFiletype","",1);
			}
			if(!strstr($pr['feedbackfiletype'],"|".$tffiletype."|"))
			{
				printerror("NotQTranFiletype","",1);
			}
			...
			if($tfr['tran'])
			{
				...
				eInsertFileTable($tfr['filename'],$filesize,$tfr['filepath'],'[Member]'.$username,$classid,'[FB]'.addslashes(dgdbe_rpstr($add['title'])),$type,$filepass,$filepass,$public_r['fpath'],0,4,0);
	...
	$filepath=dgdbe_rpstr($filepath,1);
	$filename=dgdbe_rpstr($filename,1);
	$ip=egetip();
	$eipport=egetipport();
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsfeedback(bid,saytime,ip,filepath,filename,userid,username,haveread,eipport".$addf.") values('$bid','$saytime','$ip','$filepath','$filename','$userid','$username',0,'$eipport'".$addval.");","ins");
	$fid=$empire->lastid($dbtbpre.'enewsfeedback','id');
	//更新附件
...
function AddError($add){
	...
	$ip=egetip();
	$errortext=dgdbe_rpstr($add['errortext']);
