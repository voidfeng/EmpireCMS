...
function GetZtFform($type,$f,$fvalue,$fformsize=''){
	...
	$file="../../e/data/html/classfhtml.txt";
...
function ChangeZtForm(){
	...
	$file='../../c/ecachemod/emodpub/ztaddform.php';
	$mtemp='';
...
function AddZtF($add,$userid,$username){
	...
	//字段是否重复
	$s=do_dbFieldRowList($dbtbpre.'enewsztadd');
	$b=0;
	...
	$s=do_dbFieldRowList($dbtbpre.'enewszt');
	$b=0;
	while($r=$empire->fetch($s))
	{
		if($r['Field']==$add['f'])
	...
	//新增字段
	$asql=do_dbTableAddF($dbtbpre.'enewsztadd',$add['f'],$add['ftype'],$add['flen'],'');//pgsql
	//替换代码
	...
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsztf(f,fname,fform,fhtml,fzs,myorder,ftype,flen,fvalue,fformsize) values('".$add['f']."','".$add['fname']."','".$add['fform']."','".eaddslashes2($fhtml)."','".eaddslashes($add['fzs'])."','".$add['myorder']."','".$add['ftype']."','".$add['flen']."','".eaddslashes2($add['fvalue'])."','".$add['fformsize']."');","ins");
	$lastid=$empire->lastid($dbtbpre.'enewsztf','fid');
	//更新表单
	ChangeZtForm();
	if($asql&&$sql)
	{
...
function EditZtF($add,$userid,$username){
	...
	$field="".$add['f']." ".$type." NOT NULL".$def;
	$usql=do_dbTableEditF($dbtbpre.'enewsztadd',$add['f'],$add['oldf'],$add['ftype'],$add['flen'],'');//pgsql
	//替换代码
...
function DelZtF($add,$userid,$username){
	...
	$usql=do_dbTableDelF($dbtbpre.'enewsztadd',$r['f']);
...
function DoPostZtVar($add){
	...
	$add['usernames']=AddAddsData($add['usernames']);
	//目录
	$add['ztpath']=$add['pripath'].$add['ztpath'];
	$add['ztpath']=eaddslashes($add['ztpath']);
	$add['ecmsvpf']=AddAddsData($add['ecmsvpf']);
	return $add;
}
...
function AddZt($add,$userid,$username){
	...
	$sql=$empire->updatesql("insert into {$dbtbpre}enewszt(ztname,ztnum,listtempid,onclick,ztpath,zttype,zturl,classid,islist,maxnum,reorder,intro,ztimg,zcid,showzt,ztpagekey,classtempid,myorder,usezt,yhid,endtime,closepl,checkpl,restb,usernames,addtime,pltempid,ecmsvpf) values('".$add['ztname']."','".$add['ztnum']."','".$add['listtempid']."',0,'".$add['ztpath']."','".$add['zttype']."','".$add['zturl']."','".$add['classid']."','".$add['islist']."','".$add['maxnum']."','".$add['reorder']."','".$add['intro']."','".$add['ztimg']."','".$add['zcid']."','".$add['showzt']."','".$add['ztpagekey']."','".$add['classtempid']."','".$add['myorder']."','".$add['usezt']."','".$add['yhid']."','".$add['endtime']."','".$add['closepl']."','".$add['checkpl']."','".$public_r['pldeftb']."','".$add['usernames']."','".$addtime."','".$add['pltempid']."','".$add['ecmsvpf']."');","ins");
	$ztid=$empire->lastid($dbtbpre.'enewszt','ztid');
	//副表
	$ret_zr=ReturnZtAddF($add,0);
	$empire->updatesql(do_dbReplaceInto()." into {$dbtbpre}enewsztadd(ztid,classtext".$ret_zr[0].") values('$ztid','".eaddslashes2($add['classtext'])."'".$ret_zr[1].");","rep");
	//更新附件
...
function TogZt($add,$userid,$username){
	...
	$tbr=$empire->fetch1("select tid from {$dbtbpre}enewstable where tbname='$tbname'".do_dblimit_one());
	if(!$tbr['tid'])
	{
		printerror('EmptyTogZt','history.go(-1)');
	}
...
function SaveTogZtInfo($add,$userid,$username){
	...
	$add['togztname']=eaddslashes($add['togztname']);
	...
	{
		$sql=$empire->updatesql("insert into {$dbtbpre}enewstogzts(keyboard,searchf,query,specialsearch,classid,retype,startday,endday,startid,endid,pline,doecmszt,togztname) values('".eaddslashes($add['keyboard'])."','".$searchf."','".eaddslashes($add['query'])."','".$specialsearch."','".$add['classid']."','".$add['retype']."','".eaddslashes($add['startday'])."','".eaddslashes($add['endday'])."','".$add['startid']."','".$add['endid']."','".$add['pline']."','".$add['doecmszt']."','".eaddslashes($add['togztname'])."');","ins");
		$togid=$empire->lastid($dbtbpre.'enewstogzts','togid');
	}
	if($sql)
	{
...
function ChangeClassForm(){
	global $empire,$dbtbpre;
	$file='../../c/ecachemod/emodpub/classaddform.php';
	$mtemp='';
...
function AddClassF($add,$userid,$username){
	...
	//字段是否重复
	$s=do_dbFieldRowList($dbtbpre.'enewsclassadd');
	$b=0;
	while($r=$empire->fetch($s))
	{
		if($r['Field']==$add['f'])
	...
	$s=do_dbFieldRowList($dbtbpre.'enewsclass');
	$b=0;
	while($r=$empire->fetch($s))
	{
		if($r['Field']==$add['f'])
	...
	$field="".$add['f']." ".$type." NOT NULL".$def;
	//新增字段
	$asql=do_dbTableAddF($dbtbpre.'enewsclassadd',$add['f'],$add['ftype'],$add['flen'],'');//pgsql
	//替换代码
	...
	//处理变量
	$add['myorder']=(int)$add['myorder'];
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsclassf(f,fname,fform,fhtml,fzs,myorder,ftype,flen,fvalue,fformsize) values('".$add['f']."','".$add['fname']."','".$add['fform']."','".eaddslashes2($fhtml)."','".eaddslashes($add['fzs'])."','".$add['myorder']."','".$add['ftype']."','".$add['flen']."','".eaddslashes2($add['fvalue'])."','".$add['fformsize']."');","ins");
	$lastid=$empire->lastid($dbtbpre.'enewsclassf','fid');
	//更新表单
	ChangeClassForm();
	if($asql&&$sql)
	{
...
function EditClassF($add,$userid,$username){
	...
	$field="".$add['f']." ".$type." NOT NULL".$def;
	$usql=do_dbTableEditF($dbtbpre.'enewsclassadd',$add['f'],$add['oldf'],$add['ftype'],$add['flen'],'');//pgsql
	//替换代码
	if($add['f']<>$add['oldf']||$add['fform']<>$add['oldfform']||$add['fvalue']<>$add['oldfvalue']||$add['fformsize']<>$add['oldfformsize']){
		$fhtml=GetClassFform($add['fform'],$add['f'],$add['fvalue'],$add['fformsize']);
		if($add['fform']=='select'||$add['fform']=='radio'||$add['fform']=='checkbox')
...
function DelClassF($add,$userid,$username){
	...
	$usql=do_dbTableDelF($dbtbpre.'enewsclassadd',$r['f']);//pgsql
...
function TogNotReClass($changecache=0){
	...
	$empire->query("update {$dbtbpre}enewspublic set nreclass='$nreclass',nreinfo='$nreinfo',nrejs='$nrejs',nottobq='$nottobq'".do_dblimit_upone());
	if($changecache==1)
	{
		GetConfig();
	}
...
function DoPostClassVar($add){
	...
	{$add['bname']=eaddslashes($add['bname']);}
	$add['myorder']=(int)$add['myorder'];
	if($add['infopath']==0)
	{
		$add['ipath']='';
...
function AddClass($add,$userid,$username){
	...
	else
	{
		...
		//建立栏目目录
		CreateClassPath($classpath);
		$sql=$empire->updatesql("insert into {$dbtbpre}enewsclass(bclassid,classname,sonclass,is_zt,lencord,link_num,newstempid,onclick,listtempid,featherclass,islast,classpath,classtype,newspath,filename,filetype,openpl,openadd,newline,hotline,goodline,classurl,groupid,myorder,filename_qz,hotplline,modid,checked,firstline,bname,islist,searchtempid,tid,tbname,maxnum,checkpl,down_num,online_num,listorder,reorder,intro,classimg,jstempid,addinfofen,listdt,showclass,showdt,checkqadd,qaddlist,qaddgroupid,qaddshowkey,adminqinfo,doctime,classpagekey,dtlisttempid,classtempid,nreclass,nreinfo,nrejs,nottobq,ipath,addreinfo,haddlist,sametitle,definfovoteid,wburl,qeditchecked,wapstyleid,repreinfo,pltempid,cgroupid,yhid,wfid,cgtoinfo,bdinfoid,repagenum,keycid,addtime,oneinfo,addsql,wapislist,fclast,ecmsvpf) values('".$add['bclassid']."','".$add['classname']."','".$sonclass."',0,'".$add['lencord']."','".$add['link_num']."','".$add['newstempid']."',0,'".$add['listtempid']."','".$featherclass."','".$add['islast']."','".$classpath."','".$add['classtype']."','".$add['newspath']."','".$add['filename']."','".$add['filetype']."','".$add['openpl']."','".$add['openadd']."','".$add['newline']."','".$add['hotline']."','".$add['goodline']."','".$add['classurl']."','".$add['groupid']."','".$add['myorder']."','".$add['filename_qz']."','".$add['hotplline']."','".$add['modid']."','".$add['checked']."','".$add['firstline']."','".$add['bname']."','".$add['islist']."','".$add['searchtempid']."','".$tabler['tid']."','".$tabler['tbname']."','".$add['maxnum']."','".$add['checkpl']."','".$add['down_num']."','".$add['online_num']."','".$add['listorder']."','".$add['reorder']."','".$add['intro']."','".$add['classimg']."','".$add['jstempid']."','".$add['addinfofen']."','".$add['listdt']."','".$add['showclass']."','".$add['showdt']."','".$add['checkqadd']."','".$add['qaddlist']."','".$add['qaddgroupid']."','".$add['qaddshowkey']."','".$add['adminqinfo']."','".$add['doctime']."','".$add['classpagekey']."','".$add['dtlisttempid']."','".$add['classtempid']."','".$add['nreclass']."','".$add['nreinfo']."','".$add['nrejs']."','".$add['nottobq']."','".$add['ipath']."','".$add['addreinfo']."','".$add['haddlist']."','".$add['sametitle']."','".$add['definfovoteid']."','','".$add['qeditchecked']."','".$add['wapstyleid']."','".$add['repreinfo']."','".$add['pltempid']."','".$add['cgroupid']."','".$add['yhid']."','".$add['wfid']."','".$add['cgtoinfo']."','".$add['smallbdinfoid']."','".$add['repagenum']."','".$add['keycid']."','".$addtime."','".$add['oneinfo']."','".$add['addsql']."','".$add['wapislist']."','".$ecms_fclast."','".$add['ecmsvpf']."');","ins");
		$lastid=$empire->lastid($dbtbpre.'enewsclass','classid');
		//副表
		$ret_cr=ReturnClassAddF($add,0);
		$empire->updatesql(do_dbReplaceInto()." into {$dbtbpre}enewsclassadd(classid,classtext,eclasspagetext".$ret_cr[0].") values('$lastid','".eaddslashes2($add['classtext'])."','".$add['eclasspagetext']."'".$ret_cr[1].");","rep");
...
function EditClass($add,$userid,$username){
	...
	if($sql)
	{
		...
		$cache_mess='EditClassSuccess';
		$cache_url="CreacjCache.php?enews=$cache_enews&mid=$cache_mid&oldmid=$cache_oldmid&ecmstourl=$cache_ecmstourl&mess=$cache_mess".hReturnEcmsHashStrHref2(0);
...
function ChangeClassIslast($reclassid,$userid,$username){
	...
	//操作权限
	CheckLevel($userid,$username,$classid,"class");
	$reclassid=eCheckEmptyArray($reclassid);
...
function ChangeSonclass($start,$userid,$username){
	...
	$start=(int)$start;
	$b=0;
	$sql=$empire->query("select classid from {$dbtbpre}enewsclass where islast=0 and classid>".$start." order by classid".do_dblimit($public_r['relistnum']));
	while($r=$empire->fetch($sql))
	{
		$b=1;
		$newstart=$r['classid'];
	...
	echo $fun_r['OneChangeSonclassSuccess']."(ID:<font color=red><b>".$newstart."</b></font>)<script>self.location.href='ecmsclass.php?enews=ChangeSonclass&start=$newstart".hReturnEcmsHashStrHref(0).heformhash_get('ChangeSonclass',1).$mphref."';</script>";
	exit();
}
