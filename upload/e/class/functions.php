...
//采集开启确认
function eCkCanUseCj($ecms=0){
	global $empire,$dbtbpre;
	$open=0;
	$r=$empire->fetch1("select canusecj from {$dbtbpre}enewspublicadd".do_dblimit_one());
	if($open==1&&!$r['canusecj'])
	{
		$ecms=(int)$ecms;
		$gotourl='cjreadme.php?egop='.$ecms.hReturnEcmsHashStrHref2(0);
...
//显示会员组+访问组选项
function eReturnMVGroupFormSelect($gid=0){
	global $empire,$dbtbpre;
	$gid=(int)$gid;
	//访问组
	$ygroup='';
...
//不允许设置扩展名
function eCheckNotSetTranFiletype($filetypes,$exp='|'){
	if(stristr($filetypes,'.php'))
	{
		printerror("ErrorSetFiletype","history.go(-1)");
	}
}
...
function DelListEnews(){
	$file=ECMS_PATH."c/ecachepub/eclassfc/ListEnews.php";
	DelFiletext($file);
	$file1=ECMS_PATH."c/ecachepub/eclassfc/ListClass0.php";
	DelFiletext($file1);
	$file2=ECMS_PATH."c/ecachepub/eclassfc/ListClass1.php";
	DelFiletext($file2);
}

//删除模板临时缓存文件
function DelOneTempTmpfile($classid){
	$file=ECMS_PATH.'c/ecachetmp/etmprefile/dt_temp'.$classid.'.php';
	if(file_exists($file))
	{
		DelFiletext($file);
	}
...
function RepPhpAspJspcode($string){
	...
	if(!$public_r['candocode']){
		...
		$string=str_replace("<?","&lt;?",$string);
		$string=str_replace("<%","&lt;%",$string);
		if(!is_array($string)&&@stristr($string,' language'))
		{
...
//密码复杂度检测(后台)
function ePasswordCkChar_hck($password){
	global $public_r;
	if($public_r['ckedpassc'])
	{
		$ckedpassmess=ePasswordCkChar($password);
		if($ckedpassmess!='EmpireCMS')
		{
			printerror($ckedpassmess,"history.go(-1)");
		}
	}
}

//密码有效期检测(后台)
function ePasswordCkTime_hck($userid,$username,$edpasstime,$urladd=''){
	global $public_r;
	$mustdoeditpass=0;
	if($public_r['ckedpasst'])
	{
		$gotourl=$urladd."user/EditPassword.php".hReturnEcmsHashStrHref2(1);
		$ckedpasst_time=time();
		$ckedpasst_outtime=abs($public_r['ckedpasst'])*24*3600;
		if($ckedpasst_time-$edpasstime>$ckedpasst_outtime)
		{
			if(defined('EMPIRECMSHEPPAGE'))
			{
				$mustdoeditpass=1;
			}
			else
			{
				printerror("PasswordTsOld",$gotourl);
			}
		}
	}
	return $mustdoeditpass;
}
...
function ReplaceWord($newstext){
	...
	while($r=$empire->fetch($sql))
	{
		if(strstr($r['oldword'],$exp))
		{
			$oldwdr=explode($exp,$r['oldword']);
			if(strstr($r['newword'],$exp))
			{
				$newwdr=explode($exp,$r['newword']);
				$newstext=str_replace($oldwdr,$newwdr,$newstext);
			}
			else
			{
				$newstext=str_replace($oldwdr,$r['newword'],$newstext);
			}
		}
		else
		{
...
function eFirstTitleCheckLevel($levelid,$ttype=0){
	...
	$r=$empire->fetch1("select tid,groupid from {$dbtbpre}enewsgoodtype where levelid='$levelid' and ttype='$ttype'".do_dblimit_one());
	if(!$r['tid'])
	{
		return 0;
	}
...
function AddInfoToZt($ztid,$zcid,$classid,$id,$newstime,$isgood=0,$ecms=0){
	...
	if($ecms==1)//检查是否增加过
	{
		$infor=$empire->fetch1("select zid,ztid,cid from {$dbtbpre}enewsztinfo where ztid='$ztid' and classid='$classid' and id='$id'".do_dblimit_one());
		if($infor['ztid'])
		{
			if($infor['cid']!=$zcid)
			{
				$empire->query("update {$dbtbpre}enewsztinfo set cid='$zcid',newstime='$newstime' where zid='".$infor['zid']."'".do_dblimit_upone());
			}
		}
		else
		{
...
function AddMoreInfoToZt($ztid,$zcid,$tbname,$where,$ecms=0){
	...
	while($r=$empire->fetch($sql))
	{
		$zinfor=$empire->fetch1("select zid,ztid,cid from {$dbtbpre}enewsztinfo where ztid='".$ztid."' and classid='".$r['classid']."' and id='".$r['id']."'".do_dblimit_one());
		if($zinfor['ztid'])
		{
			if($zinfor['cid']!=$zcid)
			{
				$empire->query("update {$dbtbpre}enewsztinfo set cid='$zcid' where zid='".$zinfor['zid']."'".do_dblimit_upone());
			}
		}
		else
		{
...
function InsertZtInfo($ztids,$zcids,$oldztids,$oldzcids,$classid,$id,$newstime){
	...
	if($zcids)
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			{
				$zcr=$empire->fetch1("select ztid from {$dbtbpre}enewszttype where cid='$cid'".do_dblimit_one());
				if(!$zcr['ztid'])
				{
					continue;
				}
...
function InfoInsertToWorkflow($id,$classid,$wfid,$userid,$username){
	...
	$wfitemr=$empire->fetch1("select tid,tno,groupid,userclass,username,tstatus from {$dbtbpre}enewsworkflowitem where wfid='$wfid' order by tno".do_dblimit_one());
	//状态更新
...
function InfoUpdateToWorkflow($id,$classid,$wfid,$userid,$username){
	...
	$userid=(int)$userid;
	$username=RepPostVar($username);
	$wfinfor=$empire->fetch1("select checknum,wfid,tid,checktno from {$dbtbpre}enewswfinfo where id='$id' and classid='$classid'".do_dblimit_one());
	...
	if($wfinfor['tid'])
	{
		...
		$wfitemr=$empire->fetch1("select tid,tno,groupid,userclass,username,tstatus from {$dbtbpre}enewsworkflowitem where wfid='".$wfinfor['wfid']."' and tno>".$ywfitemr['tno']." order by tno".do_dblimit_one());
	}
	else
	{
		$wfitemr=$empire->fetch1("select tid,tno,groupid,userclass,username,tstatus from {$dbtbpre}enewsworkflowitem where wfid='".$wfinfor['wfid']."' order by tno".do_dblimit_one());
	}
	//状态更新
	$empire->query("update {$dbtbpre}enewswfinfo set tid='".$wfitemr['tid']."',groupid='".$wfitemr['groupid']."',userclass='".$wfitemr['userclass']."',username='".$wfitemr['username']."',checknum=checknum+1,tstatus='".$wfitemr['tstatus']."',checktno='0' where id='$id' and classid='$classid'".do_dblimit_upone());
	//日志
	InsertWfLog($classid,$id,$wfinfor['wfid'],0,$username,'',$wfinfor['checknum'],0);
}

//改变信息工作流
function InfoChangeToWorkflow($id,$classid,$oldwfid,$newwfid,$userid,$username){
	global $empire,$dbtbpre,$class_r;
	$id=(int)$id;
	$classid=(int)$classid;
	$oldwfid=(int)$oldwfid;
	...
	$wfinfor=$empire->fetch1("select checknum,wfid,tid,checktno from {$dbtbpre}enewswfinfo where id='$id' and classid='$classid'".do_dblimit_one());
	/*
	if($wfinfor['wfid']==$newwfid)
	{
		return '';
	}
	*/
	$wfitemr=$empire->fetch1("select tid,tno,groupid,userclass,username,tstatus from {$dbtbpre}enewsworkflowitem where wfid='$newwfid' order by tno".do_dblimit_one());
	/*
	if(!$wfitemr['tid'])
	{
		return '';
	...
	*/
	...
	{
		//状态更新
		$empire->query("insert into {$dbtbpre}enewswfinfo(id,classid,wfid,tid,groupid,userclass,username,checknum,tstatus,checktno) values('".$id."','".$classid."','".$newwfid."','".$wfitemr['tid']."','".$wfitemr['groupid']."','".$wfitemr['userclass']."','".$wfitemr['username']."',1,'".$wfitemr['tstatus']."',0);");
		//日志
		InsertWfLog($classid,$id,$newwfid,0,$username,'',1,0);
	}
	else//改
	{
		//状态更新
		$empire->query("update {$dbtbpre}enewswfinfo set wfid='".$newwfid."',tid='".$wfitemr['tid']."',groupid='".$wfitemr['groupid']."',userclass='".$wfitemr['userclass']."',username='".$wfitemr['username']."',checknum=1,tstatus='".$wfitemr['tstatus']."',checktno='0' where id='$id' and classid='$classid'".do_dblimit_upone());
		//旧日志
		...
		//日志
		InsertWfLog($classid,$id,$newwfid,0,$username,'',1,4);
	}
	return '1';
}
...
function InsertWfLog($classid,$id,$wfid,$tid,$username,$checktext,$checknum,$checktype){
	...
	$checktime=time();
	$checktext=dgdbe_rpstr($checktext);
...
//tags信息表互转
function MoveCheckTagsData($checked,$tid,$tagid,$classid,$id,$newstime,$mid){
	global $empire,$dbtbpre;
	$checked=(int)$checked;
	$tid=(int)$tid;
	$tagid=(int)$tagid;
	$classid=(int)$classid;
	$id=(int)$id;
	$newstime=RepPostVar($newstime);
	$mid=(int)$mid;
	if($checked==1)
	{
		$addup='num=num-1,cnum=cnum+1';
		$ydatatb=$dbtbpre.'enewstagsdata';
		$ndatatb=$dbtbpre.'enewstagsdata_check';
	}
	else
	{
		$addup='num=num+1,cnum=cnum-1';
		$ydatatb=$dbtbpre.'enewstagsdata_check';
		$ndatatb=$dbtbpre.'enewstagsdata';
	}
	$empire->query("insert into ".$ndatatb."(tagid,classid,id,newstime,mid) values('$tagid','$classid','$id','$newstime','$mid');");
	$empire->query("delete from ".$ydatatb." where tid='$tid'");
	$empire->query("update {$dbtbpre}enewstags set ".$addup." where tagid='$tagid'");
}

//加入TAG表
function eInsertTags($tags,$classid,$id,$newstime,$checked=1){
	global $empire,$dbtbpre,$class_r;
	if(!trim($tags))
	{
		return '';
	...
	if($checked==1)
	{
		...
		$incnumval=0;
		$upnumsql='num=num+1';
		$updatatb=$dbtbpre.'enewstagsdata';
	}
	else
	{
		$innumval=0;
		$incnumval=1;
		$upnumsql='cnum=cnum+1';
		$updatatb=$dbtbpre.'enewstagsdata_check';
	}
	...
	for($i=0;$i<$count;$i++)
	{
		$tagname=RepPostVar($tr[$i]);
		if(empty($tagname))
		{
			continue;
		}
		$r=$empire->fetch1("select tagid,num,cnum from {$dbtbpre}enewstags where tagname='$tagname'".do_dblimit_one());
		if($r['tagid'])
		{
			$datar=$empire->fetch1("select tid,tagid,classid,newstime from {$dbtbpre}enewstagsdata where tagid='".$r['tagid']."' and id='$id' and mid='$mid'".do_dblimit_one());
			if($datar['tagid'])
			{
				if($checked==0)
				{
					MoveCheckTagsData(1,$datar['tid'],$datar['tagid'],$classid,$id,$newstime,$mid);
				}
				else
				{
					if($datar['classid']!=$classid||$datar['newstime']!=$newstime)
					{
						$empire->query("update {$dbtbpre}enewstagsdata set classid='$classid',newstime='$newstime' where tid='".$datar['tid']."'");
					}
				}
			}
			else
			{
				$datacr=$empire->fetch1("select tid,tagid,classid,newstime from {$dbtbpre}enewstagsdata_check where tagid='".$r['tagid']."' and id='$id' and mid='$mid'".do_dblimit_one());
				if($datacr['tagid'])
				{
					if($checked==1)
					{
						MoveCheckTagsData(0,$datacr['tid'],$datacr['tagid'],$classid,$id,$newstime,$mid);
					}
					else
					{
						if($datacr['classid']!=$classid||$datacr['newstime']!=$newstime)
						{
							$empire->query("update {$dbtbpre}enewstagsdata_check set classid='$classid',newstime='$newstime' where tid='".$datacr['tid']."'");
						}
					}
				}
				else
				{
					$empire->query("update {$dbtbpre}enewstags set ".$upnumsql." where tagid='".$r['tagid']."'");
					$empire->query("insert into ".$updatatb."(tagid,classid,id,newstime,mid) values('".$r['tagid']."','$classid','$id','$newstime','$mid');");
				}
			}
		}
		else
		{
			$empire->updatesql("insert into {$dbtbpre}enewstags(tagname,num,isgood,cid,cnum) values('$tagname','$innumval',0,0,'$incnumval');","ins");
			$tagid=$empire->lastid($dbtbpre.'enewstags','tagid');
			$empire->updatesql("insert into ".$updatatb."(tagid,classid,id,newstime,mid) values('$tagid','$classid','$id','$newstime','$mid');","ins");
		}
	}
}
...
function eReturnInfoTags($classid,$id,$mid){
	...
	$csql=$empire->query("select tagid from {$dbtbpre}enewstagsdata_check where id='$id' and mid='$mid' order by tagid");
	while($cr=$empire->fetch($csql))
	{
		$ctr=$empire->fetch1("select tagname from {$dbtbpre}enewstags where tagid='".$cr['tagid']."'");
		$tags.=$dh.$ctr['tagname'];
		$dh=',';
	}
	return $tags;
...
function MoveCheckInfoData($tbname,$checked,$stb,$where){
	...
	$empire->query(do_dbReplaceInto(1)." into ".$ntbname." select * from ".$ytbname." where ".$where);
	$empire->query(do_dbReplaceInto(1)." into ".$ndatatbname." select * from ".$ydatatbname." where ".$where);
	//删除原表
...
function UpdateAllDataTbField($tbname,$update,$where,$upcheck=1,$updoc=1){
	global $empire,$dbtbpre;
	//已审核
	$tbr=$empire->fetch1("select datatbs from {$dbtbpre}enewstable where tbname='$tbname'".do_dblimit_one());
	if($tbr['datatbs'])
	{
...
function ReturnInfoFilename($classid,$id,$filenameqz){
	...
	{
		$filename=$class_r[$classid]['filename_qz'].md5(uniqid(microtime()).'-'.$classid.','.$id);
	}
	elseif($class_r[$classid]['filename']==6)	//md5命名(16位)
	{
		$filename=$class_r[$classid]['filename_qz'].substr(md5(uniqid(microtime()).'-'.$classid.','.$id),8,16);
	}
...
function GetInfoTranFstb($classid,$id,$fstb){
	...
	if($id)
	{
		...
		{
			return intval($public_r['filedeftb']);
		}
		$index_r=$empire->fetch1("select id,classid,checked from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_index where id='$id'".do_dblimit_one());
		if(!$index_r['id'])
		{
			return intval($public_r['filedeftb']);
		}
		...
		$infor=$empire->fetch1("select fstb from ".$infotb." where id='$id'".do_dblimit_one());
		$fstb=$infor['fstb'];
	}
	elseif($fstb)
	{
...
function UpdateTheIspic($classid,$id,$checked){
	...
	$r=$empire->fetch1("select titlepic,ispic from ".$infotb." where id='$id'".do_dblimit_one());
...
function GetFpicToTpic($classid,$id,$num=1,$getfirsttitlespic=0,$swidth=0,$sheight=0,$fstb=1){
	...
	$classid=(int)$classid;
	$id=(int)$id;
	$getfirsttitlespic=(int)$getfirsttitlespic;
	$swidth=(int)$swidth;
	$sheight=(int)$sheight;
	...
	$picr=$empire->fetch1("select fileid,filename,path,id,classid,no,fpath from {$dbtbpre}enewsfile_{$fstb} where pubid='$pubid' and type=1 order by fileid".do_dblimit($num,1));
	$firsttitlepic="";
	if($picr['fileid'])
	{
...
//增加信息更新用户最后保存开启状态
function AddInfoUserSaveLastIsOpen($modid){
	global $public_r,$emod_r;
	$usavelast=(int)$public_r['usavelast'];
	if(!$usavelast)
	{
		return 0;
	}
	if(!eInfoHaveModField($modid,'newstext',0,0))
	{
		return 0;
	}
	return 1;
...
function GetKeyid($keyboard,$classid,$id,$link_num){
	global $empire,$public_r,$class_r,$fun_r,$dbtbpre,$eyh_r,$etable_r,$etable_t;
	if($keyboard)
	{
		...
		$keyid="";
		$first=0;
		$key_sql=$empire->query("select id from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where ".$yhadd.$add.$where." and id<>$id order by newstime desc".do_dblimit($link_num));
...
function DelNewsFile($filename,$newspath,$classid,$newstext,$groupid=0){
	...
	if($filename!=addslashes($filename))
	{
		return '';
	}
	if(strstr($filename,'/'))
...
function RepImg($text,$copyflash){
	...
	{
		$zz2="/\<(a|A) (.*?)(href|Href)=('|\"|\\\\\"|)(.+?)><(img|IMG) (.*?)(src|SRC)=('|\"|\\\\\"|)(.+?)(\.jpg|\.JPG|\.gif|\.GIF|\.png|\.PNG|\.bmp|\.BMP|\.webp|\.WEBP|\.jpeg|\.JPEG)(.*?)><\/(a|A)>/is";
		$text=preg_replace($zz2,"<\\6 \\7\\8=\\9\\10\\11\\12>",$text);
	}
	$zz1="/\<(img|IMG) (.*?)(src|SRC)=('|\"|\\\\\"|)(.+?)(\.jpg|\.JPG|\.gif|\.GIF|\.png|\.PNG|\.bmp|\.BMP|\.webp|\.WEBP|\.jpeg|\.JPEG)(.*?)>/is";
...
//生成缩略图
function GetMySmallImg($classid,$no,$insertfile,$filepath,$yname,$maxwidth,$maxheight,$name,$id,$cjid,$userid,$username,$modtype=0,$fstb=1,$cid=0,$cid2=0){
	...
	if($filer['file'])
	{
		...
		$cid=(int)$cid;
		$cid2=(int)$cid2;
		eInsertFileTable($insertfile,$filesize,$filepath,$username,$classid,$no,1,$id,$cjid,$public_r['fpath'],$pubid,$modtype,$fstb,$cid,$cid2);
		//FileServer
...
function ReturnVote($votename,$votenum,$delvid,$vid,$enews=0){
	global $empire,$dbtbpre;
	$votename=eCheckEmptyArray($votename);
	$delvid=eCheckEmptyArray($delvid);
	$f_exp="::::::";
	$r_exp="\r\n";
	$returnstr="";
	//增加投票
...
function GetListTemp($tempid){
	global $empire;
	$tempid=(int)$tempid;
	$r=$empire->fetch1("select temptext,subnews,listvar,".do_dbkeyfield_spe('rownum').",showdate,modid,subtitle,docode from ".GetTemptb("enewslisttemp")." where tempid='$tempid'");
...
function GetIndextemp(){
	...
	{
		$r=$empire->fetch1("select temptext from {$dbtbpre}enewsindexpage where tempid='".intval($public_r['indexpageid'])."'");
		return $r['temptext'];
	}
	$r=$empire->fetch1("select indextemp from ".GetTemptb("enewspubtemp")."".do_dblimit_one());
	return $r['indextemp'];
}
...
function AddCheckClassLevel($classid,$groupid,$classpath){
	...
	define('EPAGECHECKPATH','".$include1."');
...
function DoSpReFile($r,$spid=0){
	...
	if($spid)
	{
		$r=$empire->fetch1("select spid,varname,refile,spfile,spfileline,spfilesub from {$dbtbpre}enewssp where spid='$spid'".do_dblimit_one());
	}
	if(!$r['refile'])
	{
		return '';
...
function NewsBq($classid,$indextext,$enews=0,$doing=0){
	global $empire,$dbtbpre,$public_r,$emod_r,$class_r,$class_zr,$fun_r,$navclassid,$navinfor,$class_tr,$level_r,$etable_r,$etable_t;
	$classid=(int)$classid;
	...
	if($enews==0)//生成大栏目
	{
		...
		$file=eReturnTrueEcmsPath().'c/ecachetmp/etmprefile/class'.$classid.Moreport_eReturnReTempGidF().'.php';
	...
	elseif($enews==3)//专题
	{
		...
		$file=eReturnTrueEcmsPath().'c/ecachetmp/etmprefile/zt'.$classid.Moreport_eReturnReTempGidF().'.php';
		$indextext=str_replace("[!--newsnav--]",$url,$indextext);//位置导航
	...
	elseif($enews==4)//专题子类
	{
		...
		$file=eReturnTrueEcmsPath().'c/ecachetmp/etmprefile/ztc'.$classid.Moreport_eReturnReTempGidF().'.php';
		$indextext=str_replace("[!--newsnav--]",$url,$indextext);//位置导航
		$indextext=Class_ReplaceSvars($indextext,$url,$classid,$pagetitle,$pagekey,$pagedes,$classimg,$add,1);
	}
	elseif($enews==1)//生成首页文件
	{
		$pr=$empire->fetch1("select sitekey,siteintro,indexpagedt from {$dbtbpre}enewspublic".do_dblimit_one());
		...
		$file=eReturnTrueEcmsPath().'c/ecachetmp/etmprefile/index'.Moreport_eReturnReTempGidF().'.php';
...
function InfoNewsBq($classid,$indextext){
	...
	$file=eReturnTrueEcmsPath().'c/ecachetmp/etmprefile/temp'.$classid.Moreport_eReturnReTempGidF().'.php';
...
function GetInfoNewsBq($classid,$newstemp_r,$ecms_gr,$docheckrep){
	...
	if($_GET['reallinfotime'])
	{
		$file=eReturnTrueEcmsPath().'c/ecachetmp/etmprefile/tempnews'.$newstemp_r['tempid'].Moreport_eReturnReTempGidF().'_all.php';
	}
	else
	{
		$file=eReturnTrueEcmsPath().'c/ecachetmp/etmprefile/tempnews'.$newstemp_r['tempid'].Moreport_eReturnReTempGidF().'.php';
	}
	//变量处理
...
//标签替换5(自定义动态页)
function DtNewsBq_dtup($classid,$indextext,$ecms=0){
	global $public_r;
	$classid=(int)$classid;
	$file=ECMS_PATH.'c/ecacheapi/edtuserpage/dt'.$classid.'.php';
	$indextext=stripSlashes($indextext);
	$indextext=ReplaceTempvar($indextext);//替换全局模板变量
	//替换标签
	$indextext=DoRepEcmsLoopBq($indextext);
...
?>";
	...
	//数组参数
	if($public_r['openbqr']==1||$public_r['openbqr']==2)
	{
		$temp=DoRepEcmsLoopBqr($temp);
	}
	if($public_r['openbqr']==1||$public_r['openbqr']==3)
	{
		$temp=DoRepEcmsIndexLoopBqr($temp);
	}
	return $temp;
}
...
//替换灵动标签(数组参数)
function DoRepEcmsLoopBqr($temp){
	$yzz="/\[e:loopr={(.+?)}\](.+?)\[\/e:loopr\]/is";
	$xzz="<?php
\$bqno=0;
\$ecms_bq_sql=sys_EcmsBqr_eloop(\\1);
if(\$ecms_bq_sql){
while(\$bqr=\$empire->fetch(\$ecms_bq_sql)){
\$bqsr=sys_ReturnEcmsLoopStext(\$bqr);
\$bqno++;
...
//替换索引灵动标签(数组参数)
function DoRepEcmsIndexLoopBqr($temp){
	$yzz="/\[e:indexloopr={(.+?)}\](.+?)\[\/e:indexloopr\]/is";
	$xzz="<?php
\$bqno=0;
\$ecms_bq_sql=sys_EcmsBqr_eindexloop(\\1);
if(\$ecms_bq_sql){
while(\$indexbqr=\$empire->fetch(\$ecms_bq_sql)){
if(empty(\$class_r[\$indexbqr['classid']]['tbname'])){continue;}
\$bqr=\$empire->fetch1(\"select * from {\$dbtbpre}ecms_\".\$class_r[\$indexbqr['classid']]['tbname'].\" where id='\".\$indexbqr['id'].\"'\");
...
function ListHtml($classid,$fields,$enews=0,$userlistr=""){
	global $empire,$dbtbpre,$emod_r,$public_r,$class_r,$class_zr,$fun_r,$class_tr,$level_r,$etable_r,$etable_t;
	$classid=(int)$classid;
	//不生成栏目
	...
	if($enews==0)//子栏目列表
	{
		...
		if($totalrepage)
		{
			$limit=do_dblimit($totalrepage);
		}
		if($class_r[$classid]['maxnum'])//总记录数
		{
			if($class_r[$classid]['maxnum']<$totalrepage)
			{
				$limit=do_dblimit($class_r[$classid]['maxnum']);
			}
	...
	elseif($enews==4)//按sql语句生成列表
	{
		...
		{
			$limit=do_dblimit($userlistr['maxnum']);
		...
		$dopath=eReturnTrueEcmsPath().$userlistr['filepath'];//maddpath
		//$dolink=$public_r['newsurl'].str_replace('../../','',$userlistr['filepath']);
		$dolink=$public_r['newsurl'].$userlistr['filepath'];
...
function ListHtmlIndex($classid,$fields,$enews=0,$userlistr=""){
	...
	if($enews==0)//专题列表
	{
		...
		{
			$limit=do_dblimit($class_zr[$classid]['maxnum']);
	...
	while($k=$empire->fetch($sql))
	{
		...
		$infor=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$k['classid']]['tbname']." where id='".$k['id']."'".do_dblimit_one());
		if(empty($infor['id']))
		{
			$no++;
			continue;
...
function GetHtmlRepVar($tempr,$classid){
	global $public_r,$class_r,$class_zr,$fun_r,$empire,$dbtbpre,$emod_r,$class_tr,$level_r,$etable_r,$etable_t;
	$mid=$class_r[$classid]['modid'];
	$tbname=$class_r[$classid]['tbname'];
	...
	{
	$infonext='<?php
	$next_r=$empire->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}ecms_".$class_r[$ecms_gr[\'classid\']][\'tbname\']." where id>".$ecms_gr[\'id\']." and classid=\'".$ecms_gr[\'classid\']."\' order by id".do_dblimit_one());
	if(empty($next_r[\'id\']))
	...
	{
	$infopre='<?php
	$next_r=$empire->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}ecms_".$class_r[$ecms_gr[\'classid\']][\'tbname\']." where id<".$ecms_gr[\'id\']." and classid=\'".$ecms_gr[\'classid\']."\' order by id desc".do_dblimit_one());
	if(empty($next_r[\'id\']))
	{$infonext="<a href=\'".$grclassurl."\'>'.$fun_r['HaveNoNextLink'].'</a>";}
	else
	{
...
function GetHtml($classid,$id,$add,$ecms=0,$doall=0){
	...
	if($ecms==0)//主表
	{
		$add=$empire->fetch1("select ".ReturnSqlTextF($mid,1)." from {$dbtbpre}ecms_".$tbname." where id='$id'".do_dblimit_one());
	}
	$add['id']=$id;
	$add['classid']=$classid;
	if($add['isurl'])
	...
	//副表
	$addr=$empire->fetch1("select ".ReturnSqlFtextF($mid)." from {$dbtbpre}ecms_".$tbname."_data_".$add['stb']." where id='".$add['id']."'".do_dblimit_one());
	...
	$newstemp_r=$empire->fetch1("select temptext,showdate from ".GetTemptb("enewsnewstemp")." where tempid='".$add['newstempid']."'".do_dblimit_one());
	...
	if($add['groupid']||$class_r[$add['classid']]['cgtoinfo'])
	{
		...
		define('empirecms','wm_chief');
		define('EPAGECHECKPATH','".$include1."');
		define('EPAGECHECKTBNAME','".$class_r[$add['classid']]['tbname']."');
	...
	{
		$empire->query("update {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']."_index set havehtml=1 where id='".$add['id']."'".do_dblimit_upone());
		$empire->query("update {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']." set havehtml=1 where id='".$add['id']."'".do_dblimit_upone());
	}
}
...
function GetKeyboard($keyboard,$keyid,$classid,$id,$link_num){
	...
	{
		$add="id in (".$keyid.")";
		$tr=$empire->fetch1("select otherlinktemp,otherlinktempsub,otherlinktempdate from ".GetTemptb("enewspubtemp")."".do_dblimit_one());//取得相关链接模板
		$temp_r=explode("[!--empirenews.listtemp--]",$tr['otherlinktemp']);
		$key_sql=$empire->query("select id,newstime,title,isurl,titleurl,classid,titlepic from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where ".$add." order by newstime desc".do_dblimit($link_num));
		while($link_r=$empire->fetch($key_sql))
		{
...
//生成js
function GetNewsJs($classid,$line,$sub,$showdate,$enews=0,$tempr=null){
	global $empire,$public_r,$class_r,$class_tr,$emod_r,$etable_r,$dbtbpre,$eyh_r,$etable_t;
	if(empty($line))
	{
		$line=10;
	}
	...
	$query='select '.ReturnSqlListF($mid).' from '.$dbtbpre.'ecms_'.$tbname.$query.' order by '.ReturnSetTopSql('js').$order.' desc'.do_dblimit($line);
	$sql=$empire->query($query);
	//取得js模板
...
function ReUserjs($jsr,$addpath){
	...
	$jsfilepath=eReturnTrueEcmsPath().$jsr['jsfilename'];//maddpath
...
function ReUserlist($listr,$addpath){
	...
	DoFileMkDir(eReturnTrueEcmsPath().$listr['filepath']);//建目录maddpath
...
function GetSearch($mid=0){
	...
	$mid=(int)$mid;
	//取得模板
	$tr=$empire->fetch1("select searchtemp,searchjstemp,searchjstemp1 from ".GetTemptb("enewspubtemp")."".do_dblimit_one());
	//返回栏目搜索
	$fcfile=eReturnTrueEcmsPath()."c/ecachepub/eclassfc/ListEnews.php";
	$fcjsfile=eReturnTrueEcmsPath()."c/ecachepub/eclassfc/cmsclass.js";
	...
	//$options="<script src=".$public_r['newsurl']."c/ecachepub/eclassfc/searchclass.js></script>";
...
function GetPlTempPage($pltempid=0){
	...
	$pagekey=$pagetitle;
	$pagedes=$pagetitle;
	$pr=$empire->fetch1("select plf from {$dbtbpre}enewspl_set".do_dblimit_one());
	//回车字段
	$tobrf=',';
	...
	while($ptr=$empire->fetch($ptsql))
	{
		...
		$pl_filename=eReturnTrueEcmsPath().'c/ecachetemp/epltemp/pl'.$ptr['tempid'].'.php';
		$pltemp=$ptr['temptext'];
		//头部变量
...
function GetPlJsPage(){
	...
	$pl_filename=eReturnTrueEcmsPath()."c/ecachetemp/epltemp/esp_pljs.php";
	...
	$pr['pltobrf']=$tobrf;
	//取得评论JS模板
	$pl_r=$empire->fetch1("select pljstemp from ".GetTemptb("enewspubtemp")."".do_dblimit_one());
...
function ReGbooktemp(){
	...
	$file=eReturnTrueEcmsPath()."c/ecachetemp/egbtemp/esp_gbook.php";
	$gbtemp=ReadFiletext($tfile);
	//取得留言页面模板
	$pr=$empire->fetch1("select gbooktemp from ".GetTemptb("enewspubtemp")."".do_dblimit_one());
	$url="<?=\$url?>";
	$pagetitle="<?=\$bname?>";
...
function ReCptemp(){
	...
	$pr=$empire->fetch1("select cptemp from ".GetTemptb("enewspubtemp")."".do_dblimit_one());
	$url="<?=\$url?>";
	...
	$file1=eReturnTrueEcmsPath()."c/ecachetemp/epubtemp/cp_1.php";
	WriteFiletext($file1,AddCheckViewTempCode().$r[0]);
	$file2=eReturnTrueEcmsPath()."c/ecachetemp/epubtemp/cp_2.php";
...
function ReLoginIframe(){
	...
	$pr=$empire->fetch1("select loginiframe,loginjstemp from ".GetTemptb("enewspubtemp")."".do_dblimit_one());
	//框架登陆状态调用
	...
	$file=eReturnTrueEcmsPath()."c/ecachetemp/emtemp/esp_loginiframe.php";
	...
	$file=eReturnTrueEcmsPath()."c/ecachetemp/emtemp/esp_loginjs.php";
...
function RepVoteTempListvar($temptext,$votebox,$votename){
	...
	$temptext=str_replace("[!--vote.name--]",eDoRepPostComStr($votename,1),$temptext);
	return $temptext;
}
...
function GetPrintPage($printtempid=0){
	...
	while($ptr=$empire->fetch($ptsql))
	{
		...
		$truefile=eReturnTrueEcmsPath().'c/ecachetemp/eprinttemp/print'.$ptr['tempid'].'.php';
...
function GetDownloadPage(){
	global $empire,$public_r,$dbtbpre,$fun_r;
	$pr=$empire->fetch1("select downpagetemp from ".GetTemptb("enewspubtemp")."".do_dblimit_one());
	...
	$temptext=str_replace("[!--totaldown--]","<?=\$r['totaldown']?>",$temptext);
	$temptext=str_replace("[!--onclick--]","<?=\$r['onclick']?>",$temptext);
	$file=eReturnTrueEcmsPath()."c/ecachetemp/epubtemp/downpagetemp.php";
...
function ReSchAlltemp(){
	...
	$file=eReturnTrueEcmsPath()."c/ecachetemp/epubtemp/esp_sch.php";
	$temp=ReadFiletext($tfile);
	//取得页面模板
	$pr=$empire->fetch1("select schalltemp,schallsubnum,schalldate from ".GetTemptb("enewspubtemp")."".do_dblimit_one());
	$url="<?=\$url?>";
...
function CheckLevel($userid,$username,$classid,$enews){
	global $empire,$dbtbpre;
	$userid=(int)$userid;
	$r=$empire->fetch1("select groupid,adminclass from {$dbtbpre}enewsuser where userid='$userid'".do_dblimit_one());
	if(!$r['groupid'])
	{
	...
	{
		//操作所有栏目权限
		$gr=$empire->fetch1("select groupid,doall,doselfinfo,doaddinfo,doeditinfo,dodelinfo,docheckinfo,dogoodinfo,dodocinfo,domoveinfo,domustcheck,docheckedit,docanhtml,doinfofile,doisqf,doeditqf from {$dbtbpre}enewsgroup where groupid='".$r['groupid']."'");
		if(!$gr['groupid'])
		{
			printerror("NotLevel","history.go(-1)");
		}
...
function is_login($uid=0,$uname='',$urnd=''){
	...
	//db
	$adminr=$empire->fetch1("select userid,groupid,classid,userprikey,uprnd,isot,loginnum,lasttime,lgac,goac,lgactime from {$dbtbpre}enewsuser where userid='$userid' and username='".$username."' and rnd='".$rnd."' and checked=0".do_dblimit_one());
	if(!$adminr['userid'])
	{
	...
	if($logintime)
	{
		...
		{
			eAdminLogin_LgOutTime($adminr['userid']);
...
function is_login_ebak($userid,$username,$rnd){
	global $empire,$public_r,$ecms_adminloginr;
	$userid=(int)$userid;
	$username=RepPostVar($username);
...
function is_login_other($userid,$username,$rnd){
	...
	$adminstyle=1;
	//db
	$adminr=$empire->fetch1("select userid,groupid,classid,userprikey,isot,loginnum,lasttime,lgac,goac,lgactime from {$dbtbpre}enewsuser where userid='$userid' and username='".$username."' and rnd='".$rnd."' and checked=0".do_dblimit_one());
	if(!$adminr['userid'])
	{
		printerror("NotLogin","index.php");
	}
...
//后台登录激活检查
function eAdminLogin_LgacCheck($ur){
	if($ur['goac']==1)
	{
		eAdminLogin_LgacGoUrl($ur);
	}
	if(!$ur['lgac'])
	{
		eAdminLogin_LgacGoUrl($ur);
...
//后台登录激活转向
function eAdminLogin_LgacGoUrl($ur){
	$lgacuserid=$ur['userid'];
	$lgaclasttime=date("Y-m-d H:i:s",$ur['lasttime']);
	$lgacloginnum=$ur['loginnum'];
	$inclgacfile=ECMS_PATH.'e/eapub/aloginac.php';
	@include($inclgacfile);
	exit();
}

//登录超时处理
function eAdminLogin_LgOutTime($userid){
...
//COOKIE加密
function DoECookieRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime,$rndsr){
	global $ecms_config;
	$ip=$ecms_config['esafe']['ckhloginip']==0?'127.0.0.1':egetip();
	...
	DoECreatFileRnd($userid,$username,$rnd,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval,$rndsr);
	DoECreatAndAuthRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval,$rndsr);
}
...
//rndsource
function eRndSource_make($ur){
	$yr=explode('<e>',$ur['rnds']);
	$yr[0]=(int)$yr[0];
	$addrnd_ip=egetip();//ip
	$addrnd_ipport=egetipport();//ipport
	$addrnd_time=time();//time
	$addrnd_date=date("Y-m-d-H:i:s");//date
	$addrnd_mtime=microtime();//mtime
	$addrnd_no=EcmsRandInt(0,9);//no
	$addrnd_ua=$_SERVER['HTTP_USER_AGENT'];//ua
	$addrnd_uniqid=md5(uniqid(microtime()));//uniqid
	//rnd
	$addrnd_rndno=EcmsRandInt(0,6);
	$addrnd_rnd=make_password(28+$addrnd_rndno);
	//rt
	$r=array();
	$r['rndsno1']=$addrnd_no;
	$r['rndsno2']=$yr[0]-3;
	$r['rnds1']=md5($yr[4].$addrnd_ip.','.$addrnd_rnd.'-'.$addrnd_ua.'!'.$addrnd_mtime);
	$r['rnds2']=md5($yr[3].$addrnd_uniqid.'!'.$addrnd_date.'@'.$addrnd_ipport.')'.$addrnd_time);
	$r['rnds3']=md5($yr[2].$addrnd_mtime.'@'.$addrnd_rnd.'('.$addrnd_no.'-'.$addrnd_ip);
	$r['rnds4']=md5($yr[1].$addrnd_ipport.'|'.$addrnd_rnd.'@'.$addrnd_time.','.$addrnd_uniqid);
	$r['newrnds']=$r['rndsno1'].'<e>'.$r['rnds1'].'<e>'.$r['rnds2'].'<e>'.$r['rnds3'].'<e>'.$r['rnds4'];
	return $r;
}

function eRndSource_get($userid,$rnds){
	global $empire,$dbtbpre;
	if(empty($rnds))
	{
		$userid=(int)$userid;
		$ur=$empire->fetch1("select userid,rnds from {$dbtbpre}enewsuser where userid='$userid'");
		$rnds=$ur['rnds'];
	}
	$r=explode('<e>',$rnds);
	return $r;
...
//返回用户缓存信息
function hReturnAdminLoginFileInfo($userid,$username,$rnd,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval,$rndsr){
	global $ecms_config;
	$adminlogins='';
	$ernd=make_password(29+$rndsr['rndsno2']);
	$erndtwo=make_password(22+$rndsr['rndsno2']);
	$erndadd=make_password(34+$rndsr['rndsno2']);
...
//表单来源Hash验证
//返回表单来源认证码
function heformhash_get($ecms='',$retype=0){
	global $ecms_config;
	if($ecms_config['esafe']['ckformhash'])
	{
		return '';
	}
	$r=heformhash_set($ecms);
	if($retype)
	{
		$fhstr='&'.$r['vname'].'='.$r['vval'];
	}
	else
	{
		$fhstr='<input type=hidden name='.$r['vname'].' value="'.$r['vval'].'">';
	}
	return $fhstr;
}

//返回表单来源认证码(数组)
function heformhash_getr($ecms='',$retype=0){
	global $empire,$dbtbpre,$ecms_config,$logininid;
	$fhr['vname']='eefh';
	$fhr['vval']='empirecms';
	$fhr['vform']='<input type=hidden name=eefh value=empirecms>';
	if($ecms_config['esafe']['ckformhash'])
	{
		return $fhr;
	}
	...
	$fhr['vname']=$r['vname'];
	$fhr['vval']=$r['vval'];
	$fhr['vform']='<input type=hidden name='.$r['vname'].' value="'.$r['vval'].'">';
	return $fhr;
}

//设置表单来源认证码
function heformhash_set($ecms=''){
	global $empire,$dbtbpre,$ecms_config,$logininid;
	if(!$ecms)
	{
		return '';
	...
	$doecms=$logininid.'!'.$ecms;
	$time=time();
	$ckr=$empire->fetch1("select ckname,ckval,dotime from {$dbtbpre}enewsformhash where ecms='".$doecms."'".do_dblimit_one());
	if($ckr['ckname'])
	{
		$r['vname']=$ckr['ckname'];
		$r['vval']=$ckr['ckval'];
	...
	{
		$len=strlen($ecms);
		$prestr=RepPostVar(substr($ecms,2,3));
		$addstr=make_password(6);
		$r['vname']='e'.$prestr.$len.$addstr;
		$r['vval']=make_password(12);
		$empire->query(do_dbReplaceInto()." into {$dbtbpre}enewsformhash(ecms,userid,ckname,ckval,dotime) values('$doecms','$logininid','".addslashes($r['vname'])."','".addslashes($r['vval'])."','$time');");
	}
	return $r;
}

//验证表单来源认证码
function heformhash_check(){
	global $empire,$dbtbpre,$ecms_config,$logininid;
	if($ecms_config['esafe']['ckformhash'])
	{
		return '';
	}
	if(defined('InEmpireBakRe'))
	{
		return '';
	}
	...
	if($_POST[$vname]&&$_GET[$vname])
	{
		if('dg'.$_POST[$vname]!='dg'.$_GET[$vname])
		{
			printerror("FailHash","history.go(-1)");
		}
	}
	$logininid=(int)$logininid;
	$ecms=RepPostVar($epostget[$vname]);
	$doecms=$logininid.'!'.$ecms;
	$time=time();
	$ckr=$empire->fetch1("select userid,ckname,ckval,dotime from {$dbtbpre}enewsformhash where ecms='".$doecms."'".do_dblimit_one());
	if(!$ckr['userid']||!$ckr['ckname']||!$ckr['ckval'])
	{
		printerror("FailHash","history.go(-1)");
	}
	$post_vval=$epostget[$ckr['ckname']];
	if(!$post_vval)
	{
		printerror("FailHash","history.go(-1)");
	}
	if($ckr['userid']!=$logininid)
	{
		printerror("FailHash","history.go(-1)");
	}
	if('dg'.$ckr['ckval']!='dg'.$post_vval)
	{
		printerror("FailHash","history.go(-1)");
	}
}
...
//清理表单来源认证码
function heformhash_del($userid,$dall=0){
	global $empire,$dbtbpre,$ecms_config;
	if($ecms_config['esafe']['ckformhash'])
	{
		return '';
	...
	$sql=$empire->query("delete from {$dbtbpre}enewsformhash".$where);
}
...
function hCheckEcmsRHash(){
	...
	if($_GET[$rhashvar]&&'dg'.$_GET[$rhashvar]=='dg'.$rhash)
	{
	}
	elseif($_POST[$rhashvar]&&'dg'.$_POST[$rhashvar]=='dg'.$rhash)
	{
	}
	else
	{
...
function hCheckEcmsEHash(){
	...
	if($_GET[$ehashvar]&&'dg'.$_GET[$ehashvar]=='dg'.$ehash)
	{
	}
	elseif($_POST[$ehashvar]&&'dg'.$_POST[$ehashvar]=='dg'.$ehash)
	{
	}
	else
	{
...
//文件认证处理
function DoECreatFileRnd($userid,$username,$rnd,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval,$rndsr){
	global $ecms_config;
	$userid=(int)$userid;
	$file=ECMS_PATH.'c/ecacheadmin/eadminlogin/user'.$userid.'_'.md5(md5($username.'-empirecms!check.file'.$truelogintime.'-'.$rnd.$ecms_config['esafe']['ecookiernd']).'-'.$ip.'-'.$userid.'-'.$rnd.$adminstyle.'-'.$groupid.'-'.$dbdata.$sessval).'.php';
	$filetext=hReturnAdminLoginFileInfo($userid,$username,$rnd,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval,$rndsr);
...
function Moreport_eChangeMoreportAdmin($pid,$ecms,$douserid,$dousername){
	global $empire,$dbtbpre,$public_r,$ecms_config,$emoreport_r;
	...
	$fromfile=ECMS_PATH.'c/ecacheadmin/eadminlogin/'.$userloginfile;
	$tofile=$pr['ppath'].'c/ecacheadmin/eadminlogin/'.$userloginfile;
	...
	if($ecms==0)
	{
		DelFiletext2($fromfile);
	}
	//$selfurl=eReturnSelfPage(0);
	//$gourl=substr($pr['purl'],0,-1).str_replace('/enews.php','/admin.php',$selfurl).hReturnEcmsHashStrHref2(1);
	//当前端
	$selfpid=eReturnSMPid();
	$selfpid=$selfpid>1?$selfpid:1;
	$selfpurl=$emoreport_r[$selfpid]['purl'];
	if(!strstr($selfpurl,'://'))
	{
		$selfpurl=eReturnDomain().$selfpurl;
	}
	//新页面
	$selfurl=eReturnSelfLocationPage();
	$gourl=str_replace('/enews.php','/admin.php',$selfurl);
	$gourl=str_replace($selfpurl,$pr['purl'],$gourl).hReturnEcmsHashStrHref2(1);
	printerror("ChangeMoreportAdminSuccess",$gourl);
}

function DoEDelFileRnd($userid){
	$userid=(int)$userid;
	$path=ECMS_PATH.'c/ecacheadmin/eadminlogin/';
	$hand=@opendir($path);
	while($file=@readdir($hand))
	{
		...
		{
			DelFiletext2($path.$file);
		}
	}
}

//中转认证码
function DoECreatAddAuthRnd($userid,$username,$rnd,$groupid,$rndsr,$first=0){
	global $empire,$dbtbpre,$ecms_config;
	$defaddauthvn='ecmsaddrnd';
	$userid=(int)$userid;
	$time=time();
	if($first==1)
	{
		$newaddauthvn=$defaddauthvn;
		$addauth=md5(md5(make_password(30).'-em!pi;re-'.$time).'-fir%st-'.md5($userid).',cm@s,'.$time.'-rnd-'.md5($username));
		$setaddauth=substr($addauth,0,18);
	}
	else
	{
		$ecmsarget=RepPostVar($_GET['ecmsar']);
		$ecmsar=RepPostVar(getcvar($defaddauthvn,1));
		$ckecmsar=$ecmsar.$ecmsarget;
		$anduser_r=$empire->fetch1("select addauthvn,addauth from {$dbtbpre}enewsuserloginck where userid='$userid'");
		if(!$ecmsar||!$ecmsarget||!$anduser_r['addauth']||!$anduser_r['addauthvn']||strlen($anduser_r['addauth'])>=50||$anduser_r['addauthvn']!=$defaddauthvn||strlen($ecmsarget)!=14)
		{
			printerror('NotLogin','index.php');
		}
		if('dg'.$anduser_r['addauth']!='dg'.$ckecmsar)
		{
			printerror('NotLogin','index.php');
		}
		$newaddauthvn='ear'.abc_make_password(7);
		$addauth=EcmsRandIntStr(50,60);
		$setaddauth=$addauth;
	}
	$empire->query("update {$dbtbpre}enewsuserloginck set addauthvn='$newaddauthvn',addauth='$addauth' where userid='$userid'".do_dblimit_upone());
	esetcookie($newaddauthvn,$setaddauth,0,1);
	if(!$first)
	{
		esetcookie($defaddauthvn,'',0,1);
	}
	return $addauth;
}

//附加码认证
function DoECreatAndAuthRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval,$rndsr){
	global $empire,$dbtbpre,$ecms_config;
	$userid=(int)$userid;
	$arnds=substr($rndsr['rnds3'],0,8);
	$andauth=md5(md5($rnd.'-'.$username.'-empirecms!check.andauth'.$truelogintime.'-'.$ecms_config['esafe']['ecookiernd'].$arnds.$userkey).$sessval.'-'.$ip.'-'.$userid.$rnd.'-'.$adminstyle.'-'.$groupid.$username.'-'.$dbdata);
	DoEDelAndAuthRnd($userid);
	$empire->query(do_dbReplaceInto()." into {$dbtbpre}enewsuserloginck(userid,andauth) values('$userid','$andauth');");
}

function DoECheckAndAuthRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime){
	global $empire,$dbtbpre,$ecms_config,$ecms_adminloginr;
	$userid=(int)$userid;
	$anduser_r=$empire->fetch1("select andauth,addauthvn,addauth from {$dbtbpre}enewsuserloginck where userid='$userid'");
	...
	//$otherinfo=DoECkOtherInfo();
	$sessval=ReESessionRnd();
	$arnds=substr($ecms_adminloginr['eafrnds'],0,8);
	$ckandauth=md5(md5($rnd.'-'.$username.'-empirecms!check.andauth'.$truelogintime.'-'.$ecms_config['esafe']['ecookiernd'].$arnds.$userkey).$sessval.'-'.$ip.'-'.$userid.$rnd.'-'.$adminstyle.'-'.$groupid.$username.'-'.$dbdata);
	if('dg'.$anduser_r['andauth']<>'dg'.$ckandauth)
	{
		printerror('NotLogin','index.php');
	}
	//addrnd
	if(!defined('EmpireCMSEARPage'))
	{
		if(!$anduser_r['addauth']||!$anduser_r['addauthvn']||strlen($anduser_r['addauth'])<50||$anduser_r['addauthvn']=='ecmsaddrnd')
		{
			printerror('NotLogin','index.php');
		}
		$ckecmsar=RepPostVar(getcvar($anduser_r['addauthvn'],1));
		if('dg'.$anduser_r['addauth']!='dg'.$ckecmsar)
		{
			printerror('NotLogin','index.php');
		}
...
function insert_dolog($doing,$pubid=0){
	...
	$doing=addslashes(addslashes(stripSlashes($doing)));
	//ip
	$logip=egetip();
	$ipport=egetipport();
	...
	$loginin=RepPostVar($loginin);
	$logininid=(int)$logininid;
	$sql=$empire->query("insert into {$dbtbpre}enewsdolog(username,logip,logtime,enews,doing,pubid,ipport,userid) values('$loginin','$logip','$logtime','$enews','$doing','$pubid','$ipport','$logininid');");
}
...
function ReturnFtpInfo($ftpid){
	global $empire,$dbtbpre;
	$r=$empire->fetch1("select * from {$dbtbpre}enewspublic".do_dblimit_one());
	return $r;
}
...
function AddPostUrlData($postdata,$userid,$username){
	...
	$postdata=eCheckEmptyArray($postdata);
	...
	echo $fun_r['AddPostDataSuccess']."<script>self.location.href='enews.php?enews=PostUrlData&start=0&line=$line&rnd=$rnd".hReturnEcmsHashStrHref(0).heformhash_get('PostUrlData',1)."';</script>";
	exit();
}
...
function PostUrlData($start,$rnd,$userid,$username){
	...
	$start=(int)$start;
	$b=0;
	$sql=$empire->query("select postid,postdata,ispath from {$dbtbpre}enewspostdata where rnd='$rnd' and postid>$start order by postid".do_dblimit($line));
	while($r=$empire->fetch($sql))
	{
		$b=1;
		$newstart=$r['postid'];
	...
	echo $fun_r['OnePostDataSuccess']."<script>self.location.href='enews.php?enews=PostUrlData&start=$newstart&line=$line&rnd=$rnd".hReturnEcmsHashStrHref(0).heformhash_get('PostUrlData',1)."';</script>";
	exit();
}
...
function DoRehValue($mid,$f,$val){
	...
	*/
	...
	$val=ehtmlspecialchars($val);//替换html
	return $val;
}

//复制表
function CopyEcmsTb($otb,$tb,$tbidf=''){
	global $empire;
	do_dbCopyTb($otb,$tb,$tbidf);//pgsql
}
...
function SetCreateTable($sql,$dbcharset) {
	global $ecms_config;
	//pgsql
	return do_dbTableCreateSql($sql,$ecms_config['db']['dbver'],$dbcharset);
}
...
function TogSaveTxtF($ecms=0){
	...
	$empire->query("update {$dbtbpre}enewspublic set savetxtf='$savef'".do_dblimit_upone());
	if($ecms==0)
	{
		GetConfig();
	}
...
function ChGetFname($mid,$f){
	...
	$mid=(int)$mid;
	$f=RepPostVar($f);
	$r=$empire->fetch1("select fname from {$dbtbpre}enewsf where f='$f' and tid='".$emod_r[$mid]['tid']."'".do_dblimit_one());
...
function ChIsOnlyAddF($mid,$id,$f,$value,$isq=0){
	...
	if(strstr($emod_r[$mid]['onlyf'],','.$f.','))
	{
		...
		$value=dgdbe_rpstr($value);
		//已审核
		$num=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_".$emod_r[$mid]['tbname']." where ".$f."='".addslashes($value)."'".$and."".do_dblimit_cone());
		//未审核
		if(empty($num))
		{
			$num=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_".$emod_r[$mid]['tbname']."_check where ".$f."='".addslashes($value)."'".$and."".do_dblimit_cone());
		}
		if($num)
		{
...
function SameDataAddF($id,$classid,$mid,$f,$value){
	...
	{
		$index_r=$empire->fetch1("select checked from {$dbtbpre}ecms_".$emod_r[$mid]['tbname']."_index where id='$id'".do_dblimit_one());
		//返回表
		$infotb=empty($index_r['checked'])?$dbtbpre.'ecms_'.$emod_r[$mid]['tbname'].'_check':$dbtbpre.'ecms_'.$emod_r[$mid]['tbname'];
		$value=addslashes(dgdbe_rpstr($value));
		$r=$empire->fetch1("select ".$f." from ".$infotb." where id='$id'".do_dblimit_one());
		$r[$f]=addslashes(dgdbe_rpstr($r[$f]));
		if($r[$f]<>$value)
		{
...
function DoFieldMoreValue($f,$add,$ecms=0){
	$rvarname=$f.'_1';
	$add[$rvarname]=eCheckEmptyArray($add[$rvarname]);
	...
	$mvdelid=eCheckEmptyArray($add[$mvdelidvarname]);
	//处理
...
//信息表字段类型处理
function eModDoTbFieldTypeVal($mid,$tbf,$tbfval){
	global $empire,$dbtbpre,$emod_r;
	$mid=(int)$mid;
	if(stristr($emod_r[$mid]['typeintf'],','.$tbf.','))
	{
		$tbfval=(float)$tbfval;
	}
	return $tbfval;
...
//验证字段类型(后)
function hEModCkFieldType($tbname,$f,$fval){
	global $emod_pubr;
	//ID列表
	if(stristr($emod_pubr['fieldchids'],'|'.$tbname.'.'.$f.'|'))
	{
		if(!eCkIdsListStr($fval,',',0))
		{
		
			printerror('字段 '.$f.' 内容格式不对','',0,0,1);
			return '';
		}
		$fval=eReturnInids($fval);
	}
	return $fval;
}
...
function ReturnAddF($add,$modid,$userid,$username,$do=0,$rdata=0,$ch=0){
	...
	if(empty($do))//增加
	{
		...
		for($i=1;$i<$count;$i++)
		{
			...
			$value=hEModCkFieldType($emod_r[$modid]['tbname'],$f,$value);//类型
			//处理函数
...
function ReturnAddCj($add,$cj,$do=0){
	...
	for($i=0;$i<count($record_r)-1;$i++)
	{
		...
		if(empty($do))
		{
			...
			$f4="save_".$field_r[1];
			$ret_r[0].=",".$f1.",".$f2.",".$f3.",".$f4;
			$ret_r[1].=",'".eaddslashes2($add[$f1])."','".eaddslashes2($add[$f2])."','".eaddslashes2($add[$f3])."','".eaddslashes2($add[$f4])."'";
	    }
		//修改
		else
		{
			...
			$f3="qz_".$field_r[1];
			$f4="save_".$field_r[1];
			$ret_r[0].=",".$f1."='".eaddslashes2($add[$f1])."',".$f2."='".eaddslashes2($add[$f2])."',".$f3."='".eaddslashes2($add[$f3])."',".$f4."='".eaddslashes2($add[$f4])."'";
	    }
    }
	return $ret_r;
}
...
function GetPubCache(){
	...
	//模板组
	$tgidlist=',';
	$tglistsql=$empire->query("select gid from {$dbtbpre}enewstempgroup");
	while($tglistr=$empire->fetch($tglistsql))
	{
		$tgidlist.=$tglistr['gid'].',';
	}
	//公共变量
	$r=$empire->fetch1("select * from {$dbtbpre}enewspublic".do_dblimit_one());
	$tr=$empire->fetch1("select downsofttemp,onlinemovietemp,listpagetemp from ".GetTemptb("enewspubtemp")."".do_dblimit_one());
	$fsr=$empire->fetch1("select purl from {$dbtbpre}enewspostserver where ptype=1".do_dblimit_one());
	$plr=$empire->fetch1("select * from {$dbtbpre}enewspl_set".do_dblimit_one());
	$fzr=$empire->fetch1("select openfz,openfzpage,fzdeftb from {$dbtbpre}enewsfz_set".do_dblimit_one());
	$paddr=$empire->fetch1("select * from {$dbtbpre}enewspublicadd".do_dblimit_one());
...
'smtpssl'=>".$r['smtpssl'].",
'sitedm'=>'".addslashes($r['sitedm'])."',
'qmaxpage'=>".$r['qmaxpage'].",
'regphonly'=>".$r['regphonly'].",
'regmustef'=>".$r['regmustef'].",
'upicsize'=>".$r['upicsize'].",
'upictype'=>'".addslashes($r['upictype'])."',
'upicdef'=>'".addslashes($r['upicdef'])."',
'tfilechmod'=>".$r['tfilechmod'].",
'fpnpub'=>'".addslashes($r['fpnpub'])."',
'eaopenpage'=>".$paddr['eaopenpage'].",
'eackrnd'=>'".addslashes($paddr['eackrnd'])."',
'eacktime'=>".$paddr['eacktime'].",
'drretype'=>".$paddr['drretype'].",
'drrepage'=>'".addslashes($paddr['drrepage'])."',
'qedtext'=>".$paddr['qedtext'].",
'qedmpids'=>'".addslashes($paddr['qedmpids'])."',
'qedmids'=>'".addslashes($paddr['qedmids'])."',
'chlisttemp'=>".$paddr['chlisttemp'].",
'nchlisttemp'=>'".addslashes($paddr['nchlisttemp'])."',
'chnewstemp'=>".$paddr['chnewstemp'].",
'nchnewstemp'=>'".addslashes($paddr['nchnewstemp'])."',
'chclasstemp'=>".$paddr['chclasstemp'].",
'nchclasstemp'=>'".addslashes($paddr['nchclasstemp'])."',
...
'phmreg'=>".$paddr['phmreg'].",
'phmckst'=>".$paddr['phmckst'].",
'phmckrnd'=>'".addslashes($paddr['phmckrnd'])."',
'phmlgtoreg'=>".$paddr['phmlgtoreg'].",
'phmdo'=>'".addslashes($paddr['phmdo'])."',
'openipf'=>".$paddr['openipf'].",
'openbqr'=>".$paddr['openbqr'].",
'openfz'=>".$fzr['openfz'].",
'openfzpage'=>".$fzr['openfzpage'].",
'fzdeftb'=>".$fzr['fzdeftb'].",
'ctimefz'=>".$paddr['ctimefz'].",
'rewritefz'=>'".addslashes($r['rewritefz'])."',
'searchuptime'=>".$r['searchuptime'].",
'searchupnum'=>".$r['searchupnum'].",
'searchfmax'=>".$r['searchfmax'].",
'schalltype'=>".$r['schalltype'].",
'schallfmax'=>".$r['schallfmax'].",
'schallupnum'=>".$r['schallupnum'].",
'schalluptime'=>".$r['schalluptime'].",
'sametgids'=>'".addslashes($paddr['sametgids'])."',
'sametgdo'=>'".addslashes($paddr['sametgdo'])."',
'canusedm'=>'".addslashes($paddr['canusedm'])."',
'usavelast'=>".$paddr['usavelast'].",
'ckedpasst'=>".$paddr['ckedpasst'].",
'ckedpassc'=>".$paddr['ckedpassc'].",
'fntype'=>".$r['fntype'].",
'loginckt'=>".$r['loginckt'].",
'lenids'=>".$r['lenids'].",
'plcanlogin'=>".$plr['plcanlogin'].",
'plnewaddtime'=>".$plr['plnewaddtime'].",
...
function GetModCache(){
	...
	$tables='';
	$tableids='';
	$tablesql=$empire->query("select tid,tbname,deftb,yhid,mid,intb from {$dbtbpre}enewstable");
	while($tabler=$empire->fetch($tablesql))
	{
		$alltbids.=$tabler['tid'].',';
		$alltbs.=$tabler['tbname'].',';
		$tables.="\$etable_r['".$tabler['tbname']."']=Array('deftb'=>'".addslashes($tabler['deftb'])."',
'yhid'=>".$tabler['yhid'].",
'intb'=>".$tabler['intb'].",
'mid'=>".$tabler['mid'].");
";
		$tableids.="\$etable_t['".$tabler['tid']."']=Array('tbname'=>'".addslashes($tabler['tbname'])."');
";
	}
	//系统模型
	$alltbidseo=',';//年月日点击统计表id集合
	$alltbidsed=',';//年月日下载统计表id集合
	$alllinkfields='|';//关联同步
	$allfieldchids='|';//ID列表字段
	...
	while($mr=$empire->fetch($modsql))
	{
		...
		{
			if($fr['ftype']=='TINYINT'||$fr['ftype']=='SMALLINT'||$fr['ftype']=='INT'||$fr['ftype']=='BIGINT'||$fr['ftype']=='FLOAT'||$fr['ftype']=='DOUBLE')
			{
				$typeintf.=$fr['f'].',';
			}
			//onclickdown
			...
			if($fr['fform']=='fieldchid')
			{
				$allfieldchids.=$fr['tbname'].'.'.$fr['f'].'|';
			}
...
\$emod_pubr=Array('linkfields'=>'".addslashes($alllinkfields)."','alltbs'=>'".addslashes($alltbs)."','alltbids'=>'".addslashes($alltbids)."','allmids'=>'".addslashes($allmids)."','fieldchids'=>'".addslashes($allfieldchids)."','tbidseo'=>'".addslashes($alltbidseo)."','tbidsed'=>'".addslashes($alltbidsed)."');
...
'regps'=>'".addslashes($r['regps'])."',
'loginps'=>'".addslashes($r['loginps'])."',
'msgnum'=>".$r['msgnum'].");
";
	}
	//管理组
