<?php
//增加留言分类
function AddGbookClass($add,$do=0,$userid=0,$username=''){
	...
	$sql=$empire->updatesql("insert into ".$table."(bname".$mychecked.") values('".$add['bname']."'".$mycheckedvalue.");","ins");
	if($sql)
	{
		$bid=$empire->lastid($table,'bid');
		//操作日志
...
//修改留言分类
function EditGbookClass($add,$do=0,$userid=0,$username=''){
	global $empire,$dbtbpre;
	$add['bid']=(int)$add['bid'];
...
//删除留言分类
function DelGbookClass($bid,$do=0,$userid=0,$username=''){
	global $empire,$dbtbpre;
	$bid=(int)$bid;
	if(!$bid)
	{
...
function DelFeedbackFile($filename,$filepath){
	...
	if($filename)
	{
		...
		for($j=0;$j<$fcount;$j++)
		{
			if(!$getfpath)
			{
				$ftr=$empire->fetch1("select fpath from {$dbtbpre}enewsfile_other where modtype=4 and path='$filepath' and filename='".$filer[$j]."'".do_dblimit_one());
				$fpath=$ftr['fpath'];
				$getfpath=1;
			}
...
function AddFeedbackF($add,$userid,$username){
	...
	//字段是否重复
	$s=do_dbFieldRowList($dbtbpre.'enewsfeedback');
	$b=0;
	...
	//新增字段
	$asql=do_dbTableAddF($dbtbpre.'enewsfeedback',$add['f'],$add['ftype'],$add['flen'],'');//pgsql
	//处理变量
	$add['myorder']=(int)$add['myorder'];
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsfeedbackf(f,fname,fform,fzs,myorder,ftype,flen,fformsize,fvalue) values('".$add['f']."','".$add['fname']."','".$add['fform']."','".eaddslashes($add['fzs'])."','".$add['myorder']."','".$add['ftype']."','".$add['flen']."','".$add['fformsize']."','".eaddslashes2($add['fvalue'])."');","ins");
	$lastid=$empire->lastid($dbtbpre.'enewsfeedbackf','fid');
	if($asql&&$sql)
	{
		//操作日志
...
function EditFeedbackF($add,$userid,$username){
	...
	$field="".$add['f']." ".$type." NOT NULL".$def;
	$usql=do_dbTableEditF($dbtbpre.'enewsfeedback',$add['f'],$add['oldf'],$add['ftype'],$add['flen'],'');//pgsql
	//处理变量
	$add['myorder']=(int)$add['myorder'];
...
function DelFeedbackF($add,$userid,$username){
	...
	$usql=do_dbTableDelF($dbtbpre.'enewsfeedback',$r['f']);//pgsql
...
function ReturnFeedbackBtemp($cname,$center,$mustenter){
	...
	{
		$v=$center[$i];
		$fr=$empire->fetch1("select fform,fformsize,fvalue from {$dbtbpre}enewsfeedbackf where f='".RepPostVar($v)."'".do_dblimit_one());
		if($fr['fform']=="file")
		{
...
function ReFeedbackClassFile($bid){
	...
	$file=eReturnTrueEcmsPath()."c/ecachetemp/efbtemp/feedback".$bid.".php";
	$btemp="<?php
...
//批量生成反馈表单文件
function ReMoreFeedbackClassFile($start=0,$userid=0,$username=''){
	global $empire,$dbtbpre;
	//验证权限
...
function AddFeedbackClass($add,$userid,$username){
	...
	$sql=$empire->updatesql("insert into {$dbtbpre}enewsfeedbackclass(bname,btemp,bzs,enter,mustenter,filef,groupid,checkboxf,usernames) values('".$add['bname']."','".eaddslashes2($add['btemp'])."','".eaddslashes($add['bzs'])."','".$enter."','".$mustenter."','".$filef."','".$groupid."','".$checkboxf."','".$add['usernames']."');","ins");
	$bid=$empire->lastid($dbtbpre.'enewsfeedbackclass','bid');
	//生成表单页面
...
function DelFeedbackClass($add,$userid,$username){
	...
	//删除表单文件
	$file="../../../c/ecachetemp/efbtemp/feedback".$bid.".php";
	DelFiletext($file);
	if($sql)
	{
		//操作日志
...
//批量发送站内信息
function DoSendMsg($add,$ecms=0,$userid=0,$username=''){
	global $empire,$dbtbpre;
	$start=(int)$add['start'];
	$line=(int)$add['line'];
	...
	if($ecms==1)//发送邮件
	{
		...
		$pr=$empire->fetch1("select sendmailtype,smtphost,fromemail,loginemail,emailusername,emailpassword,smtpport,emailname from {$dbtbpre}enewspublic".do_dblimit_one());
		//发送初使化
	...
	$sql=$empire->query("select ".eReturnSelectMemberF('userid,username,havemsg,groupid,email')." from ".eReturnMemberTable()." where ".egetmf('userid').">$start".$a." order by ".egetmf('userid')."".do_dblimit($line));
	while($r=$empire->fetch($sql))
	{
		$b=1;
		$newstart=$r['userid'];
...
function SendSiteMsg($title,$msgtext,$msgtime,$userid,$username,$havemsg){
	...
	if(!$havemsg)
	{
		...
		$usql=$empire->query("update ".eReturnMemberTable()." set ".egetmf('havemsg')."='$newhavemsg' where ".egetmf('userid')."='".$userid."'".do_dblimit_upone());
	}
}
?>