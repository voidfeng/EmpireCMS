...
function SearchAllChangeChar($value){
	global $ecms_config,$char,$targetchar,$iconv,$public_r;
	if($public_r['schalltype'])
	{
		return $value;
	}
	...
	{
		$value=DoIconvVal($char,$targetchar,$value);
	}
	return $value;
}
...
function LoadSearchAll($lid,$start,$userid,$username){
	...
	if(empty($lr['tbname'])||InfoIsInTable($lr['tbname']))
	{
		printerror('ErrorUrl','');
	}
	//不导入栏目
	$pr=$empire->fetch1("select schallnotcid from {$dbtbpre}enewspublic".do_dblimit_one());
	$line=$lr['loadnum'];
	if(empty($line))
	{
		$line=300;
	...
	$selectf='';
	$savetxtf='';
	$fsql=$empire->query("select tid,f,savetxt,tbdataf from {$dbtbpre}enewsf where (f='".$lr['titlefield']."' or f='".$lr['infotextfield']."') and tbname='".$lr['tbname']."'".do_dblimit(2));
	...
	$b=0;
	$sql=$empire->query("select id,stb,classid,isurl,newstime".$selectf." from {$dbtbpre}ecms_".$lr['tbname']." where id>$start order by id".do_dblimit($line));
	while($r=$empire->fetch($sql))
	{
		...
		//重复
		$havenum=$empire->gettotal("select count(*) as total from {$dbtbpre}enewssearchall where id='".$r['id']."' and classid='".$r['classid']."'".do_dblimit_cone());
		if($havenum)
		{
			continue;
		}
	...
	echo"<link rel=\"stylesheet\" href=\"../../data/images/css.css\" type=\"text/css\"><meta http-equiv=\"refresh\" content=\"0;url=LoadSearchAll.php?enews=LoadSearchAll&lid=$lid&start=$newstart".hReturnEcmsHashStrHref(0).heformhash_get('LoadSearchAll',1)."\">".$fun_r['OneLoadSearchAllSuccess']."(ID:<font color=red><b>".$newstart."</b></font>)";
	exit();
}
...
function ReturnSearchAllSql($add){
	...
	if(strstr($keyboard,' '))
	{
		...
		for($i=0;$i<$kcount;$i++)
		{
			...
			$where.=$andkey.ReturnSearchAllFieldLike($sf,$kb);
			$andkey=' and ';
		}
		if(empty($where))
		{
	...
	else
	{
		...
		$where.=ReturnSearchAllFieldLike($sf,$keyboard);
	}
	$returnr['where']=$where;
	return $returnr;
}
...
//返回查询字段条件
function ReturnSearchAllFieldLike($field,$kb){
	global $public_r,$ecms_config;
	if(!$public_r['schalltype']&&$ecms_config['db']['usedb']!='pgsql')
	{
		$where="MATCH(".$field.") AGAINST('".$kb."' IN BOOLEAN MODE)";
	}
	else
	{
		$fr=explode(',',$field);
		if($fr[1])
		{
			$where="(".$fr[0]." like '%".$kb."%' or ".$fr[1]." like '%".$kb."%')";
		}
		else
		{
			$where=$field." like '%".$kb."%'";
