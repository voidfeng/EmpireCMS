...
function DelFile($fileid,$userid,$username){
	...
	$fstb=(int)$_GET['fstb'];
	$r=$empire->fetch1("select filename,path,classid,fpath from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid'".do_dblimit_one());
...
function DelFile_all($fileid,$userid,$username){
	...
	{
		$userid=(int)$userid;
		$ur=$empire->fetch1("select groupid,adminclass,filelevel from {$dbtbpre}enewsuser where userid='$userid'".do_dblimit_one());
		if($ur['filelevel'])
		{
	...
	$fileid=eCheckEmptyArray($fileid);
	$count=count($fileid);
	if(!$count)
	{printerror("NotFileid","history.go(-1)");}
	...
	for($i=0;$i<count($fileid);$i++)
	{
		...
		$r=$empire->fetch1("select filename,path,classid,fpath from ".eReturnFileTable($modtype,$fstb)." where fileid='".$fileid[$i]."'".do_dblimit_one());
...
function DelPathFile($filename,$userid,$username){
	...
	//操作权限
	CheckLevel($userid,$username,$classid,"file");
	$filename=eCheckEmptyArray($filename);
...
//批量修改文件别名
function EditFile_all($add,$userid,$username){
	global $empire,$dbtbpre,$class_r;
	//操作权限
	if($_POST['enews']=='TEditFile_all')
	{
		$userid=(int)$userid;
		$ur=$empire->fetch1("select groupid,adminclass,filelevel from {$dbtbpre}enewsuser where userid='$userid'".do_dblimit_one());
		if($ur['filelevel'])
	...
	$dofileid=eCheckEmptyArray($add['dofileid']);
	$dofileno=$add['dofileno'];
	$count=count($dofileid);
	if(!$count)
	{
	...
	$modtype=(int)$_POST['modtype'];
	$fstb=(int)$_POST['fstb'];
	for($i=0;$i<count($dofileid);$i++)
	{
		$dofileid[$i]=(int)$dofileid[$i];
		if(!$dofileid[$i])
		{
			continue;
		}
		$dofileno[$i]=addslashes(dgdbe_rpstr($dofileno[$i]));
		if(!$dofileno[$i])
		{
			continue;
		}
	...
	if($sql)
	{
		//操作日志
		insert_dolog("");
		printerror("EditFileAllSuccess",EcmsGetReturnUrl());
    }
	else
	{
		printerror("DbError","history.go(-1)");
...
function DoMarkSmallPic($add,$userid,$username){
	...
	$fileid=eCheckEmptyArray($add['fileid']);
	$count=count($fileid);
	if($count==0)
	{
		printerror("EmptyDopicFileid","history.go(-1)");
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$r=$empire->fetch1("select classid,filename,path,no,fpath,cid,cid2 from ".eReturnFileTable($modtype,$fstb)." where fileid='".$fileid[$i]."'");
		...
		if($add['getsmall'])
		{
			...
			GetMySmallImg($add['classid'],$r['no'],$insertfile,$r['path'],$yname,$add['width'],$add['height'],$name,$add['filepass'],$add['filepass'],$userid,$username,$modtype,$fstb,$r['cid'],$r['cid2']);
		}
		//水印
		if($add['getmark'])
		{
...
//修改附件文件
function EditFileOne($add,$userid,$username){
	global $empire,$dbtbpre,$class_r;
	$modtype=(int)$add['modtype'];
	$fstb=(int)$add['fstb'];
	$fileid=(int)$add['fileid'];
	$no=addslashes(dgdbe_rpstr($add['fileno']));
	$cid=(int)$add['cid'];
	$cid2=(int)$add['cid2'];
	if(!$fileid)
	{
	...
	{
		//操作日志
		insert_dolog("modtype=$modtype&fstb=$fstb&fileid=$fileid");
		printerror("EditFileOneSuccess",EcmsGetReturnUrl());
    }
	else
	{
		printerror("DbError","history.go(-1)");
...
function TranMoreFile($file,$file_name,$file_type,$file_size,$no,$type,$userid,$username){
	...
	$file_name=eCheckEmptyArray($file_name);
	...
	for($i=0;$i<$count;$i++)
	{
		...
		eInsertFileTable($r['filename'],$r['filesize'],$r['filepath'],$username,$classid,$no[$i],$type,0,0,$public_r['fpath'],0,5,0,$cid,$cid2);
	}
	insert_dolog("");//操作日志
