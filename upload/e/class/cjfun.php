...
function CjNewsIn($classid,$id,$checked,$uptime,$userid,$username){
	...
	while($r=$empire->fetch($sql))
	{
		...
		//索引表
		$havehtml=0;
		$indexsql=$empire->updatesql("insert into {$dbtbpre}ecms_".$class_r[$cr['newsclassid']]['tbname']."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('".$cr['newsclassid']."','".$checked."','".$r['newstime']."','".$r['truetime']."','".$r['truetime']."','".$havehtml."');","ins");
		$id=$empire->lastid($dbtbpre.'ecms_'.$class_r[$cr['newsclassid']]['tbname'].'_index','id');
		$infotbr=ReturnInfoTbname($class_r[$cr['newsclassid']]['tbname'],$checked,$stb);
		//主表
		$isurl=$r['titleurl']?1:0;
		$isql=$empire->updatesql("insert into ".$infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard,newstime".$ifield.") values('".$id."','".$cr['newsclassid']."',0,0,0,0,'".$newspath."','".$filename."','".$r['userid']."','".$r['username']."',0,0,'".$ispic."',0,'".$isqf."',0,'".$isurl."','".$r['truetime']."','".$r['truetime']."','".$havehtml."',0,0,'".$r['titlefont']."','".$r['titleurl']."','".$stb."','".$public_r['filedeftb']."','".$public_r['pldeftb']."','".$r['keyboard']."','".$r['newstime']."'".$ivalue.");","ins");
		//副表
		$fisql=$empire->updatesql("insert into ".$infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$dataifield.") values('".$id."','".$cr['newsclassid']."','".$keyid."',1,'".$newstempid."',0,0,''".$dataivalue.");","ins");
		//更新栏目信息数
...
function CjNewsIn_all($classid,$checked,$uptime,$start,$userid,$username){
	...
	$cjr=explode($record,$mr['cj']);
	$ccount=count($cjr);
	$sql=$empire->query("select * from {$dbtbpre}ecms_infotmp_".$cr['tbname']." where classid='$classid' and checked=0 and id>$start order by id".do_dblimit($cr['insertnum']));
	$todaytime=time();
	$filetime=$todaytime;
	$b=0;
	while($r=$empire->fetch($sql))
	...
	echo"<meta http-equiv=\"refresh\" content=\"".$cr['loadkeeptime'].";url=ecmscj.php?enews=CjNewsIn_all&checked=$checked&uptime=$uptime&classid=$classid&start=$newstart&fm=$fm&from=".ehtmlspecialchars($_GET['from']).hReturnEcmsHashStrHref(0).heformhash_get('CjNewsIn_all',1)."\"><b>".$cr['classname']."</b>&nbsp;&nbsp;".$fun_r['OneCjLoadDbSuccess']."(ID:<font color=red><b>".$newstart."</b></font>)";
	exit();
}
...
function EchoUrl($text,$exp,$exp1,$dr,$url,$classid,$num,$checkrnd){
	...
	for($i=1;$i<count($r)&&$i<=$num;$i++)
	{
		...
		{
			$dbnum=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_infotmp_".$dr['tbname']." where oldurl='$dourl'".do_dblimit_cone());
			if($dbnum)
			{continue;}
		}
		//图片链接
...
function PageEchoUrl($classid,$cr,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		{
			$dbnum=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_infotmp_".$cr['tbname']." where oldurl='$dourl'".do_dblimit_cone());
			if($dbnum)
			{continue;}
		}
		//将地址写入数据库
	...
	echo $fun_r['GetUrlOver']."<script>self.location.href='ecmscj.php?enews=GetNewsInfo&classid=$classid&start=0&checkrnd=$checkrnd".hReturnEcmsHashStrHref(0).heformhash_get('GetNewsInfo',1)."';</script>";
	exit();
}
...
function CJUrl($classid,$start,$checkrnd,$userid,$username){
	...
	//echo $fun_r['GetOneListUrl']."<script>self.location.href='ecmscj.php?enews=CjUrl&classid=$classid&start=$newstart&checkrnd=$checkrnd".hReturnEcmsHashStrHref(0).heformhash_get('CjUrl',1)."';</script>";
	echo"<meta http-equiv=\"refresh\" content=\"".$r['keeptime'].";url=ecmscj.php?enews=CjUrl&classid=$classid&start=$newstart&checkrnd=$checkrnd".hReturnEcmsHashStrHref(0).heformhash_get('CjUrl',1)."\">".$fun_r['GetOneListUrl'];
	exit();
}
...
function ViewCjList($classid,$userid,$username){
	...
	for($i=0;$i<$j;$i++)
	{
		...
		$data.="<tr><td><a href='".$url_r[$i]."' target=_blank>".$url_r[$i]."</a></td><td align=center>[<a href='ecmscj.php?enews=ViewCjUrl&classid=$classid".$ecmshashhref.heformhash_get('ViewCjUrl',1)."&listpage=".$dourl."' target=_blank>".$fun_r['view']."</a>]</td></tr>";
    }
...
function PageViewCjList($classid,$cr,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$data.="<tr><td><a href='".$url_r[$i]."' target=_blank>".$url_r[$i]."</a></td><td align=center>[<a href='ecmscj.php?enews=ViewGetNewsInfo&classid=$classid".$ecmshashhref.heformhash_get('ViewGetNewsInfo',1)."&newspage=".$dourl."' target=_blank>".$fun_r['view']."</a>]</td></tr>";
    }
	$data="<p align=center><b>".$fun_r['CjListUrl']."</b></p><table width='96%' border=1 align=center cellpadding=3 cellspacing=0>
  <tr><td width=70% align=center><b>URL</b></td><td align=center><b>VIEW</b></td></tr>".$data."</table>";
	echo $data;
...
function ViewEchoUrl($text,$exp,$exp1,$dr,$url,$classid,$num,$checkrnd){
	...
	for($i=1;$i<count($r)&&$i<=$num;$i++)
	{
		...
		echo "<tr><td><a href='".$dourl."' target=_blank>".$dourl."</a>".$a."</td><td align=center>[<a href='ecmscj.php?enews=ViewGetNewsInfo&classid=$classid".$ecmshashhref.heformhash_get('ViewGetNewsInfo',1)."&newspage=".urlencode($dourl)."' target=_blank>".$fun_r['view']."</a>]</td></tr>";
	}
}
...
function GetNewsInfo($classid,$checkrnd,$start,$userid,$username){
	...
	{$start=0;}
	$b=0;
	$sql=$empire->query("select linkid,newsurl,titlepic from {$dbtbpre}enewslinktmp where checkrnd='$checkrnd' and linkid>".$start." order by linkid".do_dblimit($r['renum']));
	//采集时间
	...
	while($nr=$empire->fetch($sql))
	{
		...
		{
			$tmpnum=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_infotmp_".$r['tbname']." where oldurl='".$nr['newsurl']."'".do_dblimit_cone());
			if($tmpnum)
			{continue;}
		}
		//取得远程页面，三次取不到值的将不取值
		...
		for($c=0;$c<$count-1;$c++)
		{
			...
			if($dofield=="title")
			{
				...
				{
					//入库比较
					$onum=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_".$class_r[$r['newsclassid']]['tbname']." where title='".addslashes($zzvalue)."' and classid='".$r['newsclassid']."'".do_dblimit_cone());
					if($onum)
					{
						$next=1;
						break;
					...
					/*采集库比较
					$onum=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_infotmp_".$r['tbname']." where title='".addslashes($zzvalue)."'".do_dblimit_cone());
					if($onum)
					{
						$next=1;
						break;
				...
				if($r['titlelen'])//标题相似
				{
					...
					$onum=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_".$class_r[$r['newsclassid']]['tbname']." where title like '%$c_title%' and classid='".$r['newsclassid']."'".do_dblimit_cone());
					if($onum)
					{
						$next=1;
						break;
					...
					/*采集比较
					$onum=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_infotmp_".$r['tbname']." where title like '%$c_title%'".do_dblimit_cone());
					if($onum)
					{
						$next=1;
						break;
	...
	if(empty($b))
	{
		...
		if($r['justloadin'])
		{
			echo $fun_r['cjLoadInInfos']."<script>self.location.href='ecmscj.php?enews=CjNewsIn_all&classid=$classid&checked=".$r['justloadcheck']."&fm=1".hReturnEcmsHashStrHref(0).heformhash_get('CjNewsIn_all',1)."';</script>";
			exit();
		}
		//单个节点
	...
	//echo"(ID:<font color=red><b>".$newstart."</b></font>)<script>self.location.href='ecmscj.php?enews=GetNewsInfo&checkrnd=$checkrnd&classid=$classid&start=$newstart".hReturnEcmsHashStrHref(0).heformhash_get('GetNewsInfo',1)."';</script>";
	echo"<meta http-equiv=\"refresh\" content=\"".$r['keeptime'].";url=ecmscj.php?enews=GetNewsInfo&checkrnd=$checkrnd&classid=$classid&start=$newstart".hReturnEcmsHashStrHref(0).heformhash_get('GetNewsInfo',1)."\">".$r['classname']." (ID:<font color=red><b>".$newstart."</b></font>)";
	exit();
}
...
function ViewGetNewsInfo($classid,$newspage,$userid,$username){
	...
	if(empty($r['recjtheurl']))
	{
		$tmpnum=$empire->gettotal("select count(*) as total from {$dbtbpre}ecms_infotmp_".$r['tbname']." where oldurl='$newspage'".do_dblimit_cone());
		if($tmpnum)
		{
			echo $fun_r['ReCj'];
			exit();
...
function LoadOutCj($classid,$userid,$username){
	...
	$filepath=ECMS_PATH."c/ecachetmp/etmploadfile/elcj/".$file;
...
function LoadInCj($add,$file,$file_name,$file_type,$file_size,$userid,$username){
	...
	$path=ECMS_PATH.'c/ecachetmp/etmploadfile/elcj/uploadcj'.time().'.cj';
	//istran
...
function LoadInCjInsertMainstr($classid,$tid,$tbname,$mainstr,$fieldexp,$sfieldexp){
	...
	$empire->updatesql("insert into {$dbtbpre}enewsinfoclass(bclassid,newsclassid,tid,tbname,".$mainfield.") values('0','$classid','$tid','$tbname'".$mainvalues.");","ins");
	$lastid=$empire->lastid($dbtbpre.'enewsinfoclass','classid');
	return $lastid;
}
