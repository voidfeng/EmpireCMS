...
function ChangeInfoOtherLink($start,$classid,$from,$retype,$startday,$endday,$startid,$endid,$tbname,$userid,$username){
	...
	$b=0;
	$sql=$empire->query("select id,classid,checked from {$dbtbpre}ecms_".$tbname."_index where id>$start".$add1." order by id".do_dblimit($public_r['infolinknum']));
	while($r=$empire->fetch($sql))
	{
		...
		$infopr=$empire->fetch1("select diyotherlink from {$dbtbpre}enewsinfovote where pubid='$pubid'".do_dblimit_one());
		...
		$infor=$empire->fetch1("select stb,keyboard from ".$infotb." where id='".$r['id']."'".do_dblimit_one());
		//返回表信息
		...
		$usql=$empire->query("update ".$infodatatb." set keyid='$newkeyid' where id='".$r['id']."'".do_dblimit_upone());
	}
	if(empty($b))
	{
	    insert_dolog("");//操作日志
...
function AddUserpage($add,$userid,$username){
	...
	$gid=(int)$add['gid'];
	$sql=$empire->updatesql("insert into {$dbtbpre}enewspage(title,path,pagetext,classid,pagetitle,pagekeywords,pagedescription,tempid,infocid) values('$title','$path','".eaddslashes2($pagetext)."','$classid','".eaddslashes($pagetitle)."','".eaddslashes($pagekeywords)."','".eaddslashes($pagedescription)."','$tempid','$infocid');","ins");
	$id=$empire->lastid($dbtbpre.'enewspage','id');
...
function EditUserpage($add,$userid,$username){
	...
	$ckr=$empire->fetch1("select title,id,path from {$dbtbpre}enewspage where id='$id'");
	if(!$ckr['id'])
	{
		printerror("EmptyUserpagePath","history.go(-1)");
	}
	...
	if($ckr['path']<>$path)
	{
		DelFiletext(ECMS_PATH.$ckr['path']);
	}
	$title=hRepPostStr($title,1);
	$path=hRepPostStr($path,1);
	$pagetext=RepPhpAspJspcode($pagetext);
	...
	$tempid=(int)$add['tempid'];
	$gid=(int)$add['gid'];
	$sql=$empire->query("update {$dbtbpre}enewspage set title='$title',path='$path',pagetext='".eaddslashes2($pagetext)."',classid='$classid',pagetitle='".eaddslashes($pagetitle)."',pagekeywords='".eaddslashes($pagekeywords)."',pagedescription='".eaddslashes($pagedescription)."',tempid='$tempid',infocid='$infocid' where id='$id'");
	ReUserpage($id,$pagetext,$path,$title,$pagetitle,$pagekeywords,$pagedescription,$tempid);
	if($sql)
	{
		//操作日志
...
function DoRepNewstext($start,$oldword,$newword,$field,$classid,$tid,$tbname,$over,$dozz,$dotxt,$userid,$username){
	...
	$fr=$empire->fetch1("select tid,savetxt,tbdataf from {$dbtbpre}enewsf where tbname='$tbname' and f='$field'".do_dblimit_one());
	//系统字段
	...
	elseif($fr['savetxt'])
	{
		...
		$start=(int)$start;
		$b=0;
		$sql=$empire->query("select id,checked from {$dbtbpre}ecms_".$tbname."_index where id>$start".$add1." order by id".do_dblimit($public_r['dorepnum']));
		while($r=$empire->fetch($sql))
		{
			$b=1;
			$newstart=$r['id'];
			...
			$infotb=ReturnInfoMainTbname($tbname,$r['checked']);
			//主表
			$infor=$empire->fetch1("select isurl".$selectf." from ".$infotb." where id='".$r['id']."'".do_dblimit_one());
			if($infor['isurl'])
			{
				continue;
			}
			...
			if($fr['tbdataf'])
			{
				//返回表信息
				$infodatatb=ReturnInfoDataTbname($tbname,$r['checked'],$infor['stb']);
				$finfor=$empire->fetch1("select ".$field." from ".$infodatatb." where id='".$r['id']."'".do_dblimit_one());
...
function EchoRepFieldForm($tid,$tbname,$over,$dozz,$dotxt,$newstart,$fieldform,$classid,$oldword,$newword){
	...
	<form name="RepFieldForm" method="post" action="ecmscom.php">
		...
		<?php echo heformhash_get('DoRepNewstext'); ?>
...
function ReturnDoclassVar($ecms){
	...
	elseif($ecms=="fzinfo")
	{
		$r['tbname']=$dbtbpre."enewsfz_infoclass";
		$r['mess']="Fzinfo";
		$r['thelevel']="fzinfo";
		$r['returnpage']="fzinfo/FzinfoClass.php";
	}
	else
	{
		printerror("ErrorUrl","history.go(-1)");
...
function AddThisClass($add,$userid,$username){
	...
	$sql=$empire->updatesql("insert into ".$thisr['tbname']."(classname) values('".$add['classname']."');","ins");
	if($sql)
	{
		$lastid=$empire->lastid($thisr['tbname'],'classid');
		//操作日志
...
function RepDownLevel($add,$userid,$username){
	...
	$fr=$empire->fetch1("select tid,savetxt,tbdataf from {$dbtbpre}enewsf where tbname='$tbname' and f='$checkf'".do_dblimit_one());
	if($fr['tbdataf'])//副表
	{
		$sfield=$field;
		$field='';
	...
	$update="";
	$b=0;
	$sql=$empire->query("select id,stb".$field." from {$dbtbpre}ecms_".$tbname." where id>$start".$wheresql." order by id".do_dblimit($public_r['dorepdlevelnum']));
	while($r=$empire->fetch($sql))
	{
		$b=1;
		$newstart=$r['id'];
	...
	if(empty($b))
	{
		...
		while($r=$empire->fetch($sql))
		{
			...
			if($fr['tbdataf'])
			{
				$finfor=$empire->fetch1("select id".$sfield." from {$dbtbpre}ecms_".$tbname."_check_data where id='".$r['id']."'".do_dblimit_one());
				$r['downpath']=$finfor['downpath'];
				$r['onlinepath']=$finfor['onlinepath'];
			}
			$update='';
...
function RepDownLevelStrip($downpath,$add){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		if($add['dogroup'])
		{
			if("dg".$add['oldgroupid']=="dg"."no")//不设置
			{
...
function EchoRepDownLevelForm($add,$newstart){
	...
	<form name="RepDownLevelForm" method="post" action="ecmscom.php">
		<?=hReturnEcmsHashStrForm(0)?>
		<?php echo heformhash_get('RepDownLevel'); ?>
...
function ClearTmpFileData($add,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"changedata");
	//临时文件目录
	$tmppath=eReturnTrueEcmsPath().'c/ecachetmp/etmprefile';
	...
	//临时模板导出目录
	$temppath=eReturnTrueEcmsPath().'c/ecachetmp/etmploadfile/eltemp';
	...
	//临时模型导出目录
	$modpath=eReturnTrueEcmsPath().'c/ecachetmp/etmploadfile/elmod';
	...
	//临时采集导出目录
	$cjpath=eReturnTrueEcmsPath().'c/ecachetmp/etmploadfile/elcj';
	$hand=@opendir($cjpath);
	while($file=@readdir($hand))
	{
	...
	//采集临时表
	do_dbTableClearTbData($dbtbpre.'enewslinktmp');//pgsql
	//远程发布临时表
	do_dbTableClearTbData($dbtbpre.'enewspostdata');//pgsql
