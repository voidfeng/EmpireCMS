...
include_once(ECMS_PATH."e/class/userfun.php");

//列表模板分页函数
function sys_ShowListPage($num,$pagenum,$dolink,$dotype,$page,$lencord,$ok,$search="",$add=null){
	global $fun_r;
	//文件名
	if(empty($add['dofile']))
	{
...
//列表模板之列表式分页
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add=null){
...
function sys_ShowTextPageSelect($thispagenum,$dolink,$add,$filetype,$n_r){
	...
	for($j=1;$j<=$thispagenum;$j++)
	{
		...
		$titleselect.='<option value="'.$dolink.$plink.'?'.$j.'">'.strip_tags($title).'</option>';
	}
...
function sys_ReturnBqQuery($classid,$line,$enews=0,$do=0,$ewhere='',$eorder=''){
	global $empire,$public_r,$class_r,$class_zr,$navclassid,$dbtbpre,$fun_r,$class_tr,$emod_r,$etable_r,$eyh_r,$etable_t;
	if($enews==24)//按sql查询
	{
	...
	if($enews==0||$enews==1||$enews==2||$enews==9||$enews==12||$enews==15)//栏目
	{
		...
		{
			if('dg'.$classid=='dgselfinfo')//显示当前栏目信息
			{
				$classid=(int)$navclassid;
			}
	...
	elseif($enews==25||$enews==26||$enews==27||$enews==28||$enews==29||$enews==30)//标题分类
	{
		...
		{
			if('dg'.$classid=='dgselfinfo')//显示当前标题分类信息
			{
				$classid=(int)$navclassid;
			}
			$where="ttid='$classid'";
...
//带模板的标签
function sys_GetEcmsInfo($classid,$line,$strlen,$have_class=0,$enews=0,$tempid=0,$doing=0,$ewhere='',$eorder=''){
...
//万能标签(数组参数)
function sys_EcmsBqr_ecmsinfo($r){
	sys_GetEcmsInfo($r['edoid'],$r['eline'],$r['elen'],$r['eshowclass'],$r['ecms'],$r['etempid'],$r['eispic'],$r['ewhere'],$r['eorder']);
}

//灵动标签(数组参数)
function sys_EcmsBqr_eloop($r){
	return sys_ReturnBqQuery($r['edoid'],$r['eline'],$r['ecms'],$r['eispic'],$r['ewhere'],$r['eorder']);
}

//索引灵动标签(数组参数)
function sys_EcmsBqr_eindexloop($r){
	return sys_ReturnEcmsIndexLoopBq($r['edoid'],$r['eline'],$r['ecms'],$r['eclassid'],$r['emid'],$r['ewhere']);
}

//返回相关链接操作类型
function sys_OtherLinkQuery($classid,$line,$enews,$doing){
	global $empire,$public_r,$class_r,$class_zr,$navinfor,$dbtbpre,$eyh_r,$etable_r,$class_tr,$etable_t;
	if($enews==1)//按表
	{
		$tbname=$classid;
	}
...
//图文信息调用
function sys_GetClassNewsPic($classid,$line,$num,$width,$height,$showtitle=true,$strlen=1,$enews=0,$ewhere='',$eorder=''){
	global $empire;
...
function sys_GetAd($adid){
	...
	//到期
	if($r['endtime']<>eDefEmptyDate()&&time()>to_time($r['endtime']))
	{
...
function sys_GetInfoVote($classid,$id){
	...
	$classid=(int)$classid;
	$id=(int)$id;
	$r=$empire->fetch1("select * from {$dbtbpre}enewsinfovote where id='$id' and classid='$classid'".do_dblimit_one());
	if(empty($r['votetext']))
	{
		return '';
	}
...
//flash幻灯图片信息调用
function sys_FlashPixpic($classid,$line,$width,$height,$showtitle,$strlen,$enews=0,$sec=5,$ewhere='',$eorder=''){
...
//带模板的标签显示-循环
function sys_GetEcmsInfoMore($classid,$line,$strlen,$have_class=0,$ecms=0,$tr=null,$doing=0,$field=null,$cr=null,$dofirstinfo=0,$fsubtitle=0,$fsubnews=0,$fdoing=0,$ewhere='',$eorder=''){
	global $empire,$public_r;
	//操作类型
	if($ecms==0)//栏目最新
	{
...
function sys_ForSonclassDataFirstInfo($temptext,$cr,$ecms=0,$subtitle=0,$subnews=0,$fdoing=0){
	...
	elseif($ecms==2)//推荐信息
	{
		$r=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$cr['classid']]['tbname']." where isgood>0 and (".$where.")".$add." order by newstime desc".do_dblimit_one());
	}
	elseif($ecms==3)//头条信息
	{
		$r=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$cr['classid']]['tbname']." where firsttitle>0 and (".$where.")".$add." order by newstime desc".do_dblimit_one());
	}
	elseif($ecms==4)//最新信息
	{
		$r=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$cr['classid']]['tbname']." where (".$where.")".$add." order by newstime desc".do_dblimit_one());
	}
	else
	{
		return $temptext;
...
//循环子栏目数据
function sys_ForSonclassData($classid,$line,$strlen,$have_class=0,$enews=0,$tempid=0,$doing=0,$cline=0,$dofirstinfo=0,$fsubtitle=0,$fsubnews=0,$fdoing=0,$ewhere='',$eorder='',$ecwhere='',$ecorder=''){
	...
	$corder=empty($ecorder)?'myorder,classid':$ecorder;
	//取得模板
	$tr=sys_ReturnBqTemp($tempid);
	if(empty($tr['tempid']))
	{return "";}
	...
	$csql=$empire->query("select classid".$addclassfield." from {$dbtbpre}enewsclass where ".$where." and wburl='' order by ".$corder.$limit);
...
function ReplaceShowPlVars($no,$listtemp,$r,$formatdate,$subnews=0){
	...
	//标题
	$infor=$empire->fetch1("select isurl,titleurl,classid,id,title,titlepic from {$dbtbpre}ecms_".$class_r[$r['classid']]['tbname']." where id='".$r['id']."'".do_dblimit_one());
...
function sys_ShowMemberInfo($userid=0,$fields=''){
	...
	$r=$empire->fetch1("select ".$fields." from ".eReturnMemberTable()." u LEFT JOIN {$dbtbpre}enewsmemberadd ui ON u.".egetmf('userid')."=ui.userid where u.".egetmf('userid')."='$uid'".do_dblimit_one());
...
//显示TAGS
function sys_eShowTags($cid,$num=0,$line=0,$order='',$isgood='',$isgoodshow='',$showjg='',$shownum=0,$cs='',$vartype='',$classid=0){
	...
	if('dg'.$cid=='dgselfinfo')
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			if($vartype=='tagid')
			{
				$tagr=$empire->fetch1("select tagid from {$dbtbpre}enewstags where tagname='".RepPostVar($r[$i])."'".do_dblimit_one());
			}
	...
	else
	{
		...
		if($classid)
		{
			$where.=$and.(strstr($classid,',')?"classid in ($classid)":"classid='$classid'");
			$and=' and ';
		}
		if($isgood)
		{
...
function sys_ReturnEcmsIndexLoopBq($id=0,$line=10,$enews=3,$classid='',$mid='',$ewhere=''){
	...
	elseif($enews==12)//子信息最新
	{
		$type='fzdata';
		$eorder='newstime desc';
		$selectf=',bpubid,mid,bcid,cid,newstime,tid,isgood,firsttitle';
	}
	elseif($enews==13)//子信息最早
	{
		$type='fzdata';
		$eorder='newstime asc';
		$selectf=',bpubid,mid,bcid,cid,newstime,tid,isgood,firsttitle';
	}
	elseif($enews==14)//父信息最新
	{
		$type='fzinfo';
		$eorder='pubid desc';
		$selectf=',pubid,mid,fzstb,cid,usefz,sclassid,tid';
	}
	elseif($enews==15)//父信息最早
	{
		$type='fzinfo';
		$eorder='pubid asc';
		$selectf=',pubid,mid,fzstb,cid,usefz,sclassid,tid';
...
function sys_ReturnTogQuery($type,$id,$line,$classid='',$mid='',$ewhere='',$eorder='',$selectf=''){
	...
	elseif($type=='fzdata')//子信息
	{
		...
		$getfzstb=eRePubFzInfoStb(0,0,$getbpubid,0);
		if(!$getfzstb)
		{
			echo'Bpubid Error: '.$getbpubid;
			return '';
		}
		$idf='bpubid';
		$orderf='newstime desc';
		$table=$dbtbpre.'enewsfz_data_'.$getfzstb;
	}
	elseif($type=='fzinfo')//父信息
	{
		$idf='cid';
		$orderf='pubid desc';
		$table=$dbtbpre.'enewsfz_info';
	}
...
function sys_eShowSpInfo($spvar,$line=10,$strlen=0){
	...
	$spr=$empire->fetch1("select spid,spname,sppic,spsay,tempid,sptype from {$dbtbpre}enewssp where varname='$spvar'".do_dblimit_one());
...
function sys_eShowSp3($spid){
	...
	$r=$empire->fetch1("select sptext from {$dbtbpre}enewssp_3 where spid='$spid'".do_dblimit_one());
	echo $r['sptext'];
}

//调用生成缩图
function sys_ResizeImg($file,$width,$height,$docut=0,$target_filename='',$target_path='d/etmp/etitlepic/'){
...
//幻灯图片信息调用
function sys_BqHdInfoPic($enews,$classid,$line,$width,$height,$showtitle,$strlen,$titlecolor='',$titlesize='',$ewhere='',$eorder=''){
	global $empire,$public_r,$class_r,$class_zr;
	$sql=sys_ReturnBqQuery($classid,$line,$enews,1,$ewhere,$eorder);
	if(!$sql)
	{return "";}
	...
	while($r=$empire->fetch($sql))
	{
		...
		if($showtitle)
		{
			$title=$strlen?sub($r['title'],0,$strlen,false):$r['title'];
		}
		//编号
		$showno.=$i==1?'<li class="ehdthistitle"></li>':'<li></li>';
		//内容
		$showpic.='<li>
		<div style="position:relative;">
			<a href="'.$titleurl.'" target="_blank" title="'.$r['title'].'"><img src="'.$r['titlepic'].'" width="'.$width.'" height="'.$height.'" border="0"></a>
			<div class="ehdoUlplayfont"><a href="'.$titleurl.'" target="_blank" title="'.$r['title'].'">'.$title.'</a></div>
		</div>
       </li>';
	}
	$stwidth=$i*20;
...
?>
<script type="text/javascript" src="<?=$public_r['newsurl']?>e/data/images/hdfiles/hd.js"></script>
<style>
#ehdplayBox{ margin:0; padding:0; list-style:none;}
#ehddivSmallBox{overflow:hidden;*display:inline;*zoom:1;width:10px;height:10px;margin:0 5px;border-radius:10px;background:#ffffff;}
#ehdplayBox{ width:<?=$width?>px; height:<?=$height?>px; margin:0px auto; background:#333; position:relative; overflow:hidden;}
#ehdplayBox .ehdoUlplay { width:99999px; position:absolute; left:0; top:0; }
#ehdplayBox .ehdoUlplay li{ float:left;}
#ehdplayBox .ehdoUlplayfont { position:absolute; z-index:2; left:10px; bottom:36px; font: <?=$titlesize?>px/1.5 Verdana, Arial, Helvetica, sans-serif, "宋体"; color: <?=$titlecolor?>; }
#ehdplayBox .ehdoUlplayfont a { color: <?=$titlecolor?>; text-decoration: none;  font: <?=$titlesize?>px/1.5 Verdana, Arial, Helvetica, sans-serif, "宋体"; }
#ehdplayBox .ehdpre{ cursor:pointer; width:20px; height:20px; background:url(<?=$public_r['newsurl']?>e/data/images/hdfiles/l.png) no-repeat; position:absolute; top:190px; left:10px; z-index:10;}
#ehdplayBox .ehdnext{ cursor:pointer; width:20px; height:20px; background:url(<?=$public_r['newsurl']?>e/data/images/hdfiles/r.png) no-repeat; position:absolute; top:190px; right:10px; z-index:10;}
#ehdplayBox .ehdsmalltitle {width:<?=$width?>px; height:10px; position:absolute; bottom:15px; z-index:10}
#ehdplayBox .ehdsmalltitle ul{ width:<?=$stwidth?>px; margin:0 auto; }
#ehdplayBox .ehdsmalltitle ul li{ width:10px; height:10px; margin:0 5px; border-radius:10px; background:#ffffff; float:left; overflow:hidden;*display:inline;*zoom:1;}
#ehdplayBox .ehdsmalltitle .ehdthistitle{ background:#69aaec;}
</style>
<!-- hdstart -->
  <div id="ehdplayBox">
    <div class="ehdpre"></div>
    <div class="ehdnext"></div>
    <div class="ehdsmalltitle">
      <ul>
        <?=$showno?>
      </ul>
    </div>
