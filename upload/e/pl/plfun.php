...
function AddPl($username,$password,$nomember,$key,$saytext,$id,$classid,$repid,$add){
	global $empire,$dbtbpre,$public_r,$class_r,$level_r,$emod_r;
	//验证本时间允许操作
	...
	if($muserid)//已登陆
	{
		...
		if($cklgr['islogin'])
		{
			$ur=$empire->fetch1("select ".eReturnSelectMemberF('userid,username,checked,registertime,groupid,userfen,userdate,zgroupid,ingid,agid,isern,isot')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$muserid' and ".egetmf('username')."='$musername' and ".egetmf('rnd')."='$mrnd'".do_dblimit_one());
			if(empty($ur['userid']))
			{
	...
	{
		if(!$nomember&&$public_r['plcanlogin'])//非匿名
		{
			if(!$username||!$password)
			{
				printerror("FailPassword","history.go(-1)",1);
			}
			$ur=$empire->fetch1("select ".eReturnSelectMemberF('userid,username,salt,password,checked,registertime,groupid,userfen,userdate,zgroupid,ingid,agid,isern,isot')." from ".eReturnMemberTable()." where ".egetmf('username')."='$username'".do_dblimit_one());
			if(empty($ur['userid']))
			{
				printerror("FailPassword","history.go(-1)",1);
			}
	...
	if($public_r['plgroupid'])
	{
		...
		$mvgresult=eMember_ReturnCheckMVGroup($ur,$public_r['plgroupid']);
		if($mvgresult<>'empire.cms')
		{
			printerror("NotLevelToPl","history.go(-1)",1);
		}
	}
	if($level_r[$mgroupid]['plchecked']==-1)
	{
		printerror('MGNotLevel','',1);
	}
	//新会员评论验证
	if($public_r['plnewaddtime'])
	{
		if(!$muserid)
		{
			printerror("GuestNotToPl","history.go(-1)",1);
		...
		if(time()-$registertime<=$public_r['plnewaddtime']*60)
		{
			printerror('NewMemberAddPlError','',1);
		}
	}
	//实名验证
	...
	else//信息
	{
		...
		//是否关闭评论
		$r=$empire->fetch1("select classid,stb,restb from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where id='$id'".do_dblimit_one());
		...
		$finfor=$empire->fetch1("select closepl from {$dbtbpre}ecms_".$class_r[$classid]['tbname']."_data_".$r['stb']." where id='$id'".do_dblimit_one());
		if($finfor['closepl'])
		{
	...
	//设置参数
	$plsetr=$empire->fetch1("select pltime,plsize,plincludesize,plclosewords,plmustf,plf,plmaxfloor,plquotetemp from {$dbtbpre}enewspl_set".do_dblimit_one());
	...
	$time=time();
	$saytime=$time;
	$pltime=intval(getcvar('lastpltime'));
	if($pltime)
	{
	...
	$saytext=nl2br(RepFieldtextNbsp(dgdbe_rpstr($saytext)));
	if($repid)
	{
	...
	$plid=$empire->lastid($dbtbpre.'enewspl_'.$restb,'plid');
	if($doaction!='dozt')
	{
		//评论时间
		$mid=(int)$class_r[$classid]['modid'];
		$tbmainf=$emod_r[$mid]['tbmainf'];
		$addupf='';
		if(strstr($tbmainf,',elpltime,'))
		{
			$addupf.=",elpltime='".time()."'";
		}
		//信息表加1
		$usql=$empire->query("update {$dbtbpre}ecms_".$class_r[$classid]['tbname']." set plnum=plnum+1".$addupf." where id='$id'".do_dblimit_upone());
	}
	//更新新评论数
...
function RepPlTextQuote($repid,$saytext,$pr,$restb){
	...
	$r=$empire->fetch1("select plid,userid,username,saytime,checked,saytext from {$dbtbpre}enewspl_".$restb." where plid='$repid'");
	if(!$r['plid']||$r['checked'])
	{
		return $saytext;
	}
	if($pr['plmaxfloor']<0)
	{
		$r['saytext']=RepYPlQuote($r['saytext']);
		if($r['saytext']!=addslashes($r['saytext']))
		{
			return $saytext;
		}
	}
	if(empty($r['username']))
	{
		$r['username']=RepPostVar($fun_r['nomember']);
	}
	if($r['userid'])
	{
...
function ReturnPlAddF($add,$pr,$ecms=0){
	...
	for($i=1;$i<$count;$i++)
	{
		...
		$fval=dgdbe_rpstr($fval);
		//必填
		if(strstr($pr['plmustf'],','.$f.','))
		{
			if(!trim($fval))
			{
				$chfr=$empire->fetch1("select fname from {$dbtbpre}enewsplf where f='$f'".do_dblimit_one());
...
function DoForPl($add){
	...
	{
		if(!$classid||!$id||!$plid||!$class_r[$classid]['tbname']||InfoIsInTable($class_r[$classid]['tbname']))
		{
			$doajax==1?ajax_printerror('','','ErrorUrl',1):printerror('ErrorUrl','',1);
		}
