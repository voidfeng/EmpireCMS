...
{
	$eeid_r=explode(',',$eeid);
	$classid=(int)$eeid_r[1];
	$id=(int)$eeid_r[2];
	$page=(int)$eeid_r[0];
	$chtempid=(int)$eeid_r[3];
}
//eeid
...
//moretemp
if($public_r['chnewstemp']&&$chtempid)
{
	$class_r[$classid]['showdt']=2;
}
//是否支持动态内容页
...
if($public_r['chnewstemp']&&$chtempid)
{
	if(strstr($public_r['nchnewstemp'],','.$chtempid.','))
	{
		printerror("ErrorUrl","history.go(-1)",1);
	}
	$newstemp_r=$empire->fetch1("select tempid,temptext,showdate,modid from ".GetTemptb("enewsnewstemp")." where tempid='$chtempid'");
	if(!$newstemp_r['tempid'])
	{
		printerror("ErrorUrl","history.go(-1)",1);
	}
	if($public_r['chnewstemp']==2)//模型
	{
		if($mid!=$newstemp_r['modid'])
		{
			printerror("ErrorUrl","history.go(-1)",1);
		}
	}
	elseif($public_r['chnewstemp']==3)
	{
		if($tbname!=$emod_r[$newstemp_r['modid']]['tbname'])
		{
			printerror("ErrorUrl","history.go(-1)",1);
		}
	}
...
$ecms_tofunr['cacheids']=$classid.','.$id.$ecms_tofunr['cacheids_add'].','.$page;
...
if($ecms_tofunr['cacheopen']==1&&!($r['groupid']||$class_r[$classid]['cgtoinfo']))
{
	...
	if($ecms_tofunr['cacheuse'])
	{
		...
		//更新点击
		$empire->query("update {$dbtbpre}ecms_".$tbname." set onclick=onclick+1".$ymdtotaleo." where id='$id'".do_dblimit_upone());
		db_close();
		$empire=null;
		exit();
	}
...
if($r['groupid']||$class_r[$classid]['cgtoinfo'])
{
	...
	define('EPAGECHECKPATH','../../');
	define('EPAGECHECKTBNAME',$tbname);
	$check_tbname=$tbname;
	$check_infoid=$id;
...
//初始值
$search="&classid=$classid&id=$id".$search;
$line=1;
$start=0;
$page_line=6;//每页显示链接数
...
//替换模板变量
function DtGetHtml($add,$newstemp_r,$mid,$tbname,$line,$page_line,$start,$page,$search,$chtempid=0){
	global $public_r,$class_r,$class_zr,$class_tr,$fun_r,$empire,$dbtbpre,$emod_r,$level_r,$emod_pubr;
	//年月日统计
	$ymdtotaleo='';
	$ymdtid=(int)$class_r[$add['classid']]['tid'];
	if(strstr($emod_pubr['tbidseo'],','.$ymdtid.','))
	{
		$ymdtotaleo=eReturnYmdTotalf($add,0);
	}
	//更新点击
	$empire->query("update {$dbtbpre}ecms_".$tbname." set onclick=onclick+1".$ymdtotaleo." where id='".$add['id']."'".do_dblimit_upone());
	...
	if($pf&&strstr($add[$pf],$expage))//有分页
	{
		...
		//伪静态
		$pagefunr=eReturnRewriteInfoUrl($add['classid'],$add['id'],0,$chtempid);
		...
		if(strstr($newstemptext,'[!--title.select--]'))
		{
			for($j=0;$j<$thispagenum;$j++)
			{
				...
				$titleselect.='<option value="'.$spurl.'"'.$select.'>'.strip_tags($sptitle).'</option>';
			}
	...
	{
		$next_r=$empire->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']." where id>".$add['id']." and classid='".$add['classid']."' order by id".do_dblimit_one());
		if(empty($next_r['id']))
		{
	...
	{
		$next_r=$empire->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']." where id<".$add['id']." and classid='".$add['classid']."' order by id desc".do_dblimit_one());
		if(empty($next_r['id']))
		{
			$infonext="<a href='".$classurl."'>".$fun_r['HaveNoNextLink']."</a>";
		}
...
$string=DtGetHtml($r,$newstemp_r,$mid,$tbname,$line,$page_line,$start,$page,$search,$ecms_tofunr['eusechtemp']);
//缓存
