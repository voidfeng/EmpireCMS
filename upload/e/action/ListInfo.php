...
if($public_r['lenids'])
{
	$einfoids=$_GET['einfoids'];
	if($einfoids)
	{
		if(strlen($einfoids)<=$public_r['lenids'])
		{
			$einfoids=eReturnInids($einfoids);
			$add.=' and id in ('.$einfoids.')';
			$search.='&einfoids='.$einfoids;
		}
	}
}
//栏目
...
if($classid)
{
	...
	else
	{
		...
		$cr=$empire->fetch1("select classid,classpagekey,intro,classimg,cgroupid,ecmsvpf from {$dbtbpre}enewsclass where classid='$trueclassid'");
		//viewpass
		if($cr['ecmsvpf'])
		{
