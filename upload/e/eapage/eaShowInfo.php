...
//副表
$finfor=$empire->fetch1("select ".ReturnSqlFtextF($mid)." from ".$infodatatb." where id='".$r['id']."'".do_dblimit_one());
...
function DtGetHtml($add,$newstemp_r,$mid,$tbname,$line,$page_line,$start,$page,$search){
	global $public_r,$class_r,$class_zr,$class_tr,$fun_r,$empire,$dbtbpre,$emod_r,$level_r,$urlcs;
	//更新点击
	//$empire->query("update {$dbtbpre}ecms_".$tbname." set onclick=onclick+1 where id='".$add['id']."'".do_dblimit_upone());
	...
	if($pf&&strstr($add[$pf],$expage))//有分页
	{
		...
		$pagefunr['pageurl']=$public_r['newsurl']."e/eapage/eaShowInfo.php?".$urlcs;
		$pagefunr['rewrite']=0;
