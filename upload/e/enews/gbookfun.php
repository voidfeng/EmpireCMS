...
function AddGbook($add){
	...
	$name=dgdbe_rpstr(trim($add['name']));
	$email=dgdbe_rpstr($add['email']);
	$mycall=dgdbe_rpstr($add['mycall']);
	$lytext=dgdbe_rpstr($add['lytext']);
	...
	$sql=$empire->query("insert into {$dbtbpre}enewsgbook(name,email,mycall,lytime,lytext,retext,bid,ip,checked,userid,username,eipport,eipf) values('$name','$email','$mycall','$lytime','$lytext','','$bid','$ip','".$br['checked']."','$userid','$username','$eipport','$eipf');");
