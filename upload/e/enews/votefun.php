...
function AddVote($voteid,$vote){
	...
	$lasttime=intval(getcvar('lastvotetime'));
	if($lasttime)
	{
...
function AddInfoVote($classid,$id,$vote){
	...
	$lasttime=intval(getcvar('lastivotetime'));
	if($lasttime)
	{
		if(time()-$lasttime<$public_r['revotetime'])
		{
	...
	$r=$empire->fetch1("select id,voteip,votetext,voteclass,doip,dotime from {$dbtbpre}enewsinfovote where pubid='$pubid'".do_dblimit_one());
	if(empty($r['id'])||empty($r['votetext']))
	{
		printerror("NotVote","history.go(-1)",1);
	}
	$re=DoVote($r,$vote);
	$sql=$empire->query("update {$dbtbpre}enewsinfovote set votetext='".addslashes($re['votetext'])."',voteip='".$re['voteip']."',votenum=votenum+".$re['votetotal']." where pubid='$pubid'".do_dblimit_upone());
	if($sql)
	{
...
function DoVote($r,$vote){
	...
	$re['votetext']=dgdbe_rpstr($new_votetext);
	$re['voteip']=addslashes(eckDbStrlen_char(dgdbe_rpstr($r['voteip']),1));
	return $re;
}
...
function AddInfoPfen($add){
	...
	$sql=$empire->query("update {$dbtbpre}ecms_".$class_r[$classid]['tbname']." set infopfen=infopfen+$fen,infopfennum=infopfennum+1 where id='$id' and classid='$classid'".do_dblimit_upone());
	if($sql)
	{
		esetcookie('lastforfenid',$classid.'n'.$id,time()+30*24*3600);	//最后发布
		if($doajax==1)
		{
			$nr=$empire->fetch1("select infopfen,infopfennum from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where id='$id' and classid='$classid'".do_dblimit_one());
