...
//show
function EmpireCMSPlayVideo(vtype,furl,width,height,autostart,baseurl,vimg,vloop){
	var filetype;
	var cfiletype;
	var playstr='';
	...
	if(vtype=='auto'||vtype=='')
	{
		...
		cfiletype='|'+filetype+'|';
		if(filetypehtml5video.indexOf(cfiletype)!=-1)//html5video
		{
			playstr=EmpireCMSShowHtml5video(furl,width,height,autostart,baseurl,vimg,vloop);
		}
		else if(filetypehtml5audio.indexOf(cfiletype)!=-1)//html5audio
		{
			playstr=EmpireCMSShowHtml5audio(furl,width,height,autostart,baseurl,vimg,vloop);
		}
		else if(filetypejwplayer.indexOf(cfiletype)!=-1)//jwplayer
		{
	...
	{
		if(vtype=='html5video')//html5video
		{
			playstr=EmpireCMSShowHtml5video(furl,width,height,autostart,baseurl,vimg,vloop);
		}
		...
		else if(vtype=='jwplayer')//jwplayer
		{
			playstr=EmpireCMSShowJwplayer(furl,width,height,autostart,baseurl);
		}
...
//html5video
function EmpireCMSShowHtml5video(furl,width,height,autostart,baseurl,vimg,vloop){
	var str='';
	var addauto='';
	var addimg='';
	var addloop='';
	...
	str='<video id="ecmsvideoid" width="'+width+'" height="'+height+'" controls="controls"'+addauto+addloop+addimg+'><source src="'+furl+'"></source> Your browser is not supported </video>';
	return str;
}

//audio
function EmpireCMSShowHtml5audio(furl,width,height,autostart,baseurl,vimg,vloop){
	var str='';
	var addauto='';
	var addimg='';
	var addloop='';
	...
	str='<audio id="ecmsaudioid" src="'+furl+'" controls="controls"'+addauto+addloop+'> Your browser is not supported </audio>';
	return str;
}
