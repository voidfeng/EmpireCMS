...
function MemberConnect_CheckOpenid($apptype,$openid){
	...
	$mcr=$empire->fetch1("select id,userid from {$dbtbpre}enewsmember_connect where apptype='$apptype' and bindkey='$openid'".do_dblimit_one());
	return $mcr;
}
...
function MemberConnect_UpdateBindLogin($id){
	...
	$id=(int)$id;
	$lasttime=time();
	$empire->query("update {$dbtbpre}enewsmember_connect set loginnum=loginnum+1,lasttime='$lasttime' where id='$id'".do_dblimit_upone());
}
...
function MemberConnect_CheckReBind($apptype,$userid){
	...
	$userid=(int)$userid;
	$apptype=RepPostVar($apptype);
	$num=$empire->gettotal("select count(*) as total from {$dbtbpre}enewsmember_connect where userid='$userid' and apptype='$apptype'".do_dblimit_cone());
	if($num)
	{
