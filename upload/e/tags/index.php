...
if(!$tagid&&!is_array($_GET['tagname'])&&@strstr($_GET['tagname'],'etagid'))
{
...
else
{
	...
	$tagr=$empire->fetch1("select tagid,num,tagtitle,tagkey,tagdes,fclast from {$dbtbpre}enewstags where tagname='$tagname'".do_dblimit_one());
	if(!$tagr['tagid'])
	{
		printerror('HaveNotTags','',1);
	}
...
if(empty($tempid))
{
	$tempid=(int)$public_r['tagstempid'];
}
else
{
