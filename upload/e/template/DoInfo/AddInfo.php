...
<script src="../../c/ecachemod/emodjs/qpostinfo.js"></script>
<?=$loadeditorjs?>
<form name="add" method="POST" enctype="multipart/form-data" action="ecms.php" onsubmit="return EmpireCMSQInfoPostFun(document.add,'<?=$mid?>');">
  <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="tableborder">
          <tr class="header"> 
			...
			{
			?>
              <input type="hidden" name="fztid" id="fztid" value="<?=$fztid?>">
              <input type="hidden" name="fzid" id="fzid" value="<?=$fzid?>">
              <input type="hidden" name="fzcid" id="fzcid" value="<?=$fzcid?>">
			<?php
			}
			?>
			  </td>
