...
//配置查询自定义字段列表,逗号开头，多个用逗号格开，格式“ui.字段名”，例如：,ui.userpic
$useraddf='';

//分页SQL
$query='select '.eReturnSelectMemberF('userid,username,email,registertime,groupid,phno,upic','u.').$useraddf.' from '.eReturnMemberTable().' u'.$add." order by u.".egetmf('userid')." desc".do_dblimit($line,$offset);
...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="tableborder">
  <form name="memberform" method="get" action="index.php">
	...
	while($r=$empire->fetch($sql))
	{
		...
		$userpic=eMember_UpicReturnUrl($r['userid'],$r['upic']);
	?>
    <tr bgcolor="#FFFFFF"> 
      <td><div align="center"> 
          <?=$r['userid']?>
