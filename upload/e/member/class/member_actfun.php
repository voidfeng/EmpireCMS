...
function QMReturnCheckThePass($ckuserid,$ckpass,$cktime,$authstr,$ecms=1){
	...
	$time=time();
	$pr=$empire->fetch1("select ".$f." from {$dbtbpre}enewspublic".do_dblimit_one());
	$ar=explode('||',$authstr);
...
function SendGetPasswordEmail($add){
	...
	$ur=$empire->fetch1("select ".eReturnSelectMemberF('userid,username,email')." from ".eReturnMemberTable()." where ".egetmf('username')."='$username'".do_dblimit_one());
	$useremail=$ur['email'];
	...
	//发送邮件
	$pr=$empire->fetch1("select getpasstext,getpasstitle from {$dbtbpre}enewspublic".do_dblimit_one());
...
function CheckGetPassword($add,$ecms=1){
	...
	$ur=$empire->fetch1("select ".eReturnSelectMemberF('userid,username,checked,groupid')." from ".eReturnMemberTable()." where ".egetmf('userid')."='".$r['id']."'".do_dblimit_one());
	if(empty($ur['userid']))
	{
		printerror('GPErrorPass',$public_r['newsurl'],1);
	}
	$addur=$empire->fetch1("select authstr from {$dbtbpre}enewsmemberpub where userid='".$r['id']."'".do_dblimit_one());
	if(!$addur['authstr'])
	{
		printerror('GPErrorPass',$public_r['newsurl'],1);
	}
...
function DoGetPassword($add){
	...
	$renewpassword=RepPostVar($add['renewpassword']);
...
function SendActUserEmail($userid,$username,$email){
	...
	//发送邮件
	$pr=$empire->fetch1("select acttext,acttitle from {$dbtbpre}enewspublic".do_dblimit_one());
	@include(ECMS_PATH.'e/class/SendEmail.inc.php');
...
function DoRegSend($add){
	...
	//密码
	$ur=$empire->fetch1("select ".eReturnSelectMemberF('userid,salt,password')." from ".eReturnMemberTable()." where ".egetmf('username')."='$username'".do_dblimit_one());
	if(!$ur['userid'])
	{
	...
	$addr=$empire->fetch1("select userid,authstr from {$dbtbpre}enewsmemberpub where userid='".$r['userid']."'".do_dblimit_one());
