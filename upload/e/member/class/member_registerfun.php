...
function eCheckIpRegTime($ip,$time){
	...
	$ip=RepPostVar($ip);
	$uaddr=$empire->fetch1("select userid from {$dbtbpre}enewsmemberadd where regip='$ip' order by userid desc".do_dblimit_one());
	if(empty($uaddr['userid']))
	{
		return '';
	}
	$ur=$empire->fetch1("select ".eReturnSelectMemberF('userid,registertime')." from ".eReturnMemberTable()." where ".egetmf('userid')."='".$uaddr['userid']."'".do_dblimit_one());
	if(empty($ur['userid']))
	{
		return '';
	}
...
function register($add){
	global $empire,$dbtbpre,$public_r,$ecms_config,$level_r;
	//关闭注册
	...
	{
		if('dg'.$level_r[$groupid]['regps']!='dg'.$add['mg_regps'])
		{
			printerror('FailMgSetPs','',1);
		}
	}
	//IP
	$regip=egetip();
	...
	$sql=$empire->updatesql("insert into ".eReturnMemberTable()."(".eReturnInsertMemberF('username,password,rnd,email,registertime,groupid,userfen,userdate,money,zgroupid,havemsg,checked,salt,userkey,isot,phno,upic,mustdo,rndt').") values('$username','$password','$rnd','$email','$registertime','$groupid','".$public_r['reggetfen']."','0','0','0','0','$checked','$salt','$userkey','1','$phno',0,0,'$rndt');","ins");
	//取得userid
	...
	if(!$addr['userid'])
	{
		...
		$sql1=$empire->updatesql("insert into {$dbtbpre}enewsmemberadd(userid,spacestyleid,regip,lasttime,lastip,loginnum,regipport,lastipport,eipf".$member_r[0].") values('$userid','$spacestyleid','$regip','$lasttime','$regip','1','$regipport','$regipport','$eipf'".$member_r[1].");","ins");
	}
	//更新附件
	...
	if($sql)
	{
		...
		$set6=esetcookie("mlrndt",$rndt,$logincookie);
		//验证符
		...
		{
			$location=CheckCanUseDm($returnurl);
		}
