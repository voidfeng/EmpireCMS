...
function AddMsg($add){
	...
	$user=islogin();
	$title=dgdbe_rpstr(trim($add['title']));
	$to_username=RepPostVar(trim($add['to_username']));
	$msgtext=dgdbe_rpstr($add['msgtext']);
	...
	//接收方是否存在
	$r=$empire->fetch1("select ".eReturnSelectMemberF('userid,groupid,havemsg')." from ".eReturnMemberTable()." where ".egetmf('username')."='$to_username'".do_dblimit_one());
	if(!$r['userid'])
	{
	...
	$newhavemsg=eReturnSetHavemsg($r['havemsg'],0);
	$usql=$empire->query("update ".eReturnMemberTable()." set ".egetmf('havemsg')."='$newhavemsg' where ".egetmf('username')."='$to_username'".do_dblimit_upone());
	if($sql)
	{
...
function DelMsg($mid){
	...
	if($sql)
	{
		$num=$empire->gettotal("select count(*) as total from {$dbtbpre}enewsqmsg where to_username='".$user['username']."' and haveread=0".do_dblimit_cone());
		if(!$num)
		{
