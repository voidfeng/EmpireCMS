...
function admin_EditMember($add,$logininid,$loginin){
	...
	$do_delupic=(int)$add['do_delupic'];
	$oldupic=(int)$add['oldupic'];
	...
	{
		$num=$empire->gettotal("select count(*) as total from ".eReturnMemberTable()." where ".egetmf('username')."='".$add['username']."' and ".egetmf('userid')."<>".$add['userid']."".do_dblimit_cone());
	...
	$isern=(int)$add['isern'];
	$spacename=dgdbe_rpstr($add['spacename']);
	$spacegg=dgdbe_rpstr($add['spacegg']);
	//验证附加表必填项
	...
	//头像
	if($do_delupic&&$oldupic)
	{
		eMember_UpicDelFile($add['userid'],$oldupic);
		$add1.=",".egetmf('upic')."=0";
	}

	$sql=$empire->query("update ".eReturnMemberTable()." set ".egetmf('email')."='".$add['email']."',".egetmf('phno')."='".$phno."',".egetmf('groupid')."='".$add['groupid']."',".egetmf('userfen')."='".$add['userfen']."',".egetmf('money')."='".$add['money']."',".egetmf('userdate')."='".$userdate."',".egetmf('zgroupid')."='".$add['zgroupid']."',".egetmf('checked')."='".$add['checked']."',".egetmf('ingid')."='".$add['ingid']."',".egetmf('isern')."='".$isern."'".$add1." where ".egetmf('userid')."='".$add['userid']."'");

	//更改用户名
	if($add['oldusername']<>$add['username'])
	{
		//手机
		$empire->query("update {$dbtbpre}enewsmemberphno set uname='$dousername' where uid='".$add['userid']."'");
		//短消息
...
function admin_DelMember($userid,$loginuserid,$loginusername){
	...
	$r=$empire->fetch1("select ".eReturnSelectMemberF('username,groupid,upic')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid'");
	if(empty($r['username']))
	{
		printerror("NotDelMemberid","history.go(-1)");
	}
	...
	//手机
	$del=$empire->query("delete from {$dbtbpre}enewsmemberphno where uid='".$userid."'");
	//删除附加表
	...
	//头像
	eMember_UpicDelFile($userid,$r['upic']);
    if($sql)
	{
...
function admin_DelMember_all($userid,$logininid,$loginin){
	global $empire,$dbtbpre;
    CheckLevel($logininid,$loginin,$classid,"member");//验证权限
	$userid=eCheckEmptyArray($userid);
    $count=count($userid);
    if(!$count)
	{
		 printerror("NotDelMemberid","history.go(-1)");
	...
	for($i=0;$i<$count;$i++)
	{
		$euid=(int)$userid[$i];
		//删除短消息
		$ur=$empire->fetch1("select ".eReturnSelectMemberF('username,groupid,upic')." from ".eReturnMemberTable()." where ".egetmf('userid')."='".$euid."'");
		...
		$del=$empire->query("delete from {$dbtbpre}enewsqmsg where to_username='".$dousername."'");
		//头像
		eMember_UpicDelFile($euid,$ur['upic']);
		//集合
		$inid.=$dh.$euid;
		$dh=",";
    }
	...
	//手机
	$del=$empire->query("delete from {$dbtbpre}enewsmemberphno where uid in (".$inid.")");
	//删除收藏
...
function admin_DoCheckMember_all($add,$logininid,$loginin){
	global $empire,$dbtbpre;
    CheckLevel($logininid,$loginin,$classid,"member");//验证权限
	$userid=eCheckEmptyArray($add['userid']);
...
function admin_ClearMember($add,$logininid,$loginin){
	...
	if($phno)
	{
		$where.=" and ".egetmf('phno')." like '%$phno%'";
	}
	if($enduserid)
	{
	...
	$sql=$empire->query("select ".eReturnSelectMemberF('userid,username,groupid,upic')." from ".eReturnMemberTable()." where ".$where);
	$dh='';
	$inid='';
	while($r=$empire->fetch($sql))
	{
		...
		//头像
		eMember_UpicDelFile($euid,$r['upic']);
		//集合
		$inid.=$dh.$euid;
		$dh=',';
    }
