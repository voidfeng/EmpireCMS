...
function AddFriend($add){
	...
	$num=$empire->gettotal("select count(*) as total from ".eReturnMemberTable()." where ".egetmf('username')."='$fname'".do_dblimit_cone());
	if(!$num)
	{
		printerror("NotFriendUsername","",1);
	}
	//重复提交
	$num=$empire->gettotal("select count(*) as total from {$dbtbpre}enewshy where fname='$fname' and userid='".$user_r['userid']."'".do_dblimit_cone());
	if($num)
	{
		printerror("ReAddFriend","",1);
	}
	$cid=(int)$add['cid'];
	$fsay=dgdbe_rpstr($add['fsay']);
	...
	if($sql)
	{
		printerror("AddFriendSuccess","../member/friend/?cid=$fcid",1);
	}
	else
	{
...
function EditFriend($add){
	...
	if($sql)
	{
		printerror("EditFriendSuccess","../member/friend/?cid=$fcid",1);
	}
	else
	{
		printerror("DbError","",1);
...
function DelFriend($add){
	...
	if($sql)
	{
		printerror("DelFriendSuccess","../member/friend/?cid=$fcid",1);
	}
	else
	{
		printerror("DbError","",1);
...
function AddFriendClass($add){
	...
	//是否登陆
	$user_r=islogin();
	$cname=dgdbe_rpstr($add['cname']);
	$sql=$empire->query("insert into {$dbtbpre}enewshyclass(cname,userid) values('$cname','".$user_r['userid']."');");
	if($sql)
	{
...
function EditFriendClass($add){
	...
	$user_r=islogin();
	$cname=dgdbe_rpstr($add['cname']);
	$sql=$empire->query("update {$dbtbpre}enewshyclass set cname='$cname' where cid='$cid' and userid='".$user_r['userid']."'");
	if($sql)
	{
