...
function DelFava_All($favaid){
	...
	//是否登陆
	$user_r=islogin();
	$favaid=eCheckEmptyArray($favaid);
	$count=count($favaid);
	if(empty($count))
...
function AddFavaClass($add){
	...
	$sql=$empire->query("insert into {$dbtbpre}enewsfavaclass(cname,userid) values('".$cname."','".$user_r['userid']."');");
	if($sql)
	{
...
function EditFavaClass($add){
	...
	$user_r=islogin();
	$cname=dgdbe_rpstr($add['cname']);
	$sql=$empire->query("update {$dbtbpre}enewsfavaclass set cname='$cname' where cid='$cid' and userid='".$user_r['userid']."'");
	if($sql)
	{
