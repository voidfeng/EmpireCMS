...
function ReturnDoMemberF($fid,$add,$mr,$ecms=0,$username='',$admin=0){
	...
	$fid=(int)$fid;
	$pr=$empire->fetch1("select openmembertranimg,memberimgsize,memberimgtype,openmembertranfile,memberfilesize,memberfiletype from {$dbtbpre}enewspublic".do_dblimit_one());
	...
	if($tranf)
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			{
				if(!strstr($pr['memberimgtype'],"|".$tffiletype."|"))
				{
					printerror("NotQTranFiletype","",1);
				}
			...
			{
				if(!strstr($pr['memberfiletype'],"|".$tffiletype."|"))
				{
					printerror("NotQTranFiletype","",1);
				}
			}
