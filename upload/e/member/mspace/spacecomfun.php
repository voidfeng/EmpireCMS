...
function ChangeSpaceStyle($add){
	...
	$sql=$empire->query("update {$dbtbpre}enewsmemberadd set spacestyleid='$styleid' where userid='".$user_r['userid']."'".do_dblimit_upone());
	if($sql)
	{
...
function DoSetSpace($add){
	...
	$sql=$empire->query("update {$dbtbpre}enewsmemberadd set spacename='$spacename',spacegg='$spacegg' where userid='".$user_r['userid']."'".do_dblimit_upone());
	if($sql)
	{
