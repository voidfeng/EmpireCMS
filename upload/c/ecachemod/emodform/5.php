...
<a onclick="window.open('ecmseditor/FileMain.php?type=1&classid=<?=$classid?>&infoid=<?=$id?>&filepass=<?=$filepass?>&sinfo=1&doing=1&field=titlepic<?=$ecms_hashur['ehref']?>','','width=700,height=550,scrollbars=yes');" title="选择已上传的图片"><img src="../../e/data/images/changeimg.gif" border="0" align="absbottom"></a> 
  ...
  <tr> 
    ...
    <td bgcolor=ffffff><select name="movietype" id="movietype"><option value="港台影视"<?=$r['movietype']=="港台影视"?' selected':''?>>港台影视</option><option value="海外影视"<?=$r['movietype']=="海外影视"?' selected':''?>>海外影视</option><option value="大陆影视"<?=$r['movietype']=="大陆影视"?' selected':''?>>大陆影视</option><option value="日韩影视"<?=$r['movietype']=="日韩影视"?' selected':''?>>日韩影视</option></select>，推荐等级：<select name="star" id="star"><option value="1"<?=$r['star']=="1"?' selected':''?>>1星</option><option value="2"<?=$r['star']=="2"||$ecmsfirstpost==1?' selected':''?>>2星</option><option value="3"<?=$r['star']=="3"?' selected':''?>>3星</option><option value="4"<?=$r['star']=="4"?' selected':''?>>4星</option><option value="5"<?=$r['star']=="5"?' selected':''?>>5星</option></select>，扣除点数：<input name="moviefen" type="text" id="moviefen" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'moviefen',stripSlashes($r['moviefen']))?>" size="">
</td>
  </tr>
  <tr> 
    <td width=16% height=25 bgcolor=ffffff>出品公司</td>
    <td bgcolor=ffffff><input name="company" type="text" id="company" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'company',stripSlashes($r['company']))?>" size="">
，出品时间：<input name="movietime" type="text" id="movietime" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'movietime',stripSlashes($r['movietime']))?>" size="">
</td>
  </tr>
  <tr> 
    <td width=16% height=25 bgcolor=ffffff>主演</td>
    <td bgcolor=ffffff><input name="player" type="text" id="player" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'player',stripSlashes($r['player']))?>" size="">
，导演&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;：<input name="playadmin" type="text" id="playadmin" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'playadmin',stripSlashes($r['playadmin']))?>" size="">
</td>
  </tr>
  <tr> 
    <td width=16% height=25 bgcolor=ffffff>影片文件</td>
    <td bgcolor=ffffff>影片格式：<input name="filetype" type="text" id="filetype" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'filetype',stripSlashes($r['filetype']))?>" size="8">
<select name="select2" onchange="document.add.filetype.value=this.value">
      ...
      </select>，影片大小：<input name="filesize" type="text" id="filesize" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'filesize',stripSlashes($r['filesize']))?>" size="12">
<select name="select" onchange="document.add.filesize.value+=this.value">
  ...
  <tr> 
    ...
    <td bgcolor=ffffff>带宽要求：<input name="playdk" type="text" id="playdk" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'playdk',stripSlashes($r['playdk']))?>" size="">
，片长：<input name="playtime" type="text" id="playtime" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'playtime',stripSlashes($r['playtime']))?>" size="">
</td>
  </tr>
  <tr> 
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr> 
	...
	if($r['downpath'])
	{
		$r['downpath']=ehtmlspecialchars(stripSlashes($r['downpath']));
		//下载地址
		$j=0;
		$d_record=explode("\r\n",$r['downpath']);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr> 
	...
	if($r['onlinepath'])
	{
		$r['onlinepath']=ehtmlspecialchars(stripSlashes($r['onlinepath']));
		$j=0;
		$od_record=explode("\r\n",$r['onlinepath']);
  ...
  <tr> 
    ...
    <td bgcolor=ffffff><textarea name="moviesay" cols="80" rows="10" id="moviesay"><?=$ecmsfirstpost==1?"":DoRehValue($modid,'moviesay',stripSlashes($r['moviesay']))?></textarea>
</td>
  </tr>
</table>
