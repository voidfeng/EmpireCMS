...
<tr> 
  ...
  <tr>
    ...
    <td bgcolor=ffffff><input name="productno" type="text" id="productno" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'productno',stripSlashes($r['productno']))?>" size="60">
</td>
  </tr>
  <tr>
    <td width=16% height=25 bgcolor=ffffff>品牌</td>
    <td bgcolor=ffffff><input name="pbrand" type="text" id="pbrand" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'pbrand',stripSlashes($r['pbrand']))?>" size="60">
</td>
  </tr>
  <tr>
    <td width=16% height=25 bgcolor=ffffff>简单描述</td>
    <td bgcolor=ffffff><textarea name="intro" cols="80" rows="10" id="intro"><?=$ecmsfirstpost==1?"":DoRehValue($modid,'intro',stripSlashes($r['intro']))?></textarea>
</td>
  </tr>
  <tr>
    <td width=16% height=25 bgcolor=ffffff>计量单位</td>
    <td bgcolor=ffffff><input name="unit" type="text" id="unit" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'unit',stripSlashes($r['unit']))?>" size="60">
</td>
  </tr>
  <tr>
    <td width=16% height=25 bgcolor=ffffff>单位重量</td>
    <td bgcolor=ffffff><input name="weight" type="text" id="weight" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'weight',stripSlashes($r['weight']))?>" size="60">
</td>
  </tr>
  <tr>
    <td width=16% height=25 bgcolor=ffffff>市场价格</td>
    <td bgcolor=ffffff><input name="tprice" type="text" id="tprice" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'tprice',stripSlashes($r['tprice']))?>" size="60">
元</td>
  </tr>
  <tr>
    <td width=16% height=25 bgcolor=ffffff>购买价格</td>
    <td bgcolor=ffffff><input name="price" type="text" id="price" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'price',stripSlashes($r['price']))?>" size="60">
元</td>
  </tr>
  <tr>
    <td width=16% height=25 bgcolor=ffffff>积分购买</td>
    <td bgcolor=ffffff><input name="buyfen" type="text" id="buyfen" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'buyfen',stripSlashes($r['buyfen']))?>" size="60">
点</td>
  </tr>
  <tr>
    <td width=16% height=25 bgcolor=ffffff>库存</td>
    <td bgcolor=ffffff><input name="pmaxnum" type="text" id="pmaxnum" value="<?=$ecmsfirstpost==1?"100":DoRehValue($modid,'pmaxnum',stripSlashes($r['pmaxnum']))?>" size="60">
</td>
  </tr>
  <tr>
...
<a onclick="window.open('ecmseditor/FileMain.php?type=1&classid=<?=$classid?>&infoid=<?=$id?>&filepass=<?=$filepass?>&sinfo=1&doing=1&field=titlepic<?=$ecms_hashur['ehref']?>','','width=700,height=550,scrollbars=yes');" title="选择已上传的图片"><img src="../../e/data/images/changeimg.gif" border="0" align="absbottom"></a> 
  ...
  <tr>
    ...
    <td bgcolor=ffffff><input name="productpic" type="text" id="productpic" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'productpic',stripSlashes($r['productpic']))?>" size="60">
...
<div style="background-color:#D0D0D0"><?=ECMS_ShowEditorVar("newstext",$ecmsfirstpost==1&&$usersldo==0?"":DoRehValue($modid,'newstext',stripSlashes($r['newstext'])),"Default","","300","100%")?>
