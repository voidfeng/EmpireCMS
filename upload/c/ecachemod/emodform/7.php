...
</td></tr><tr><td width='16%' height=25 bgcolor='ffffff'>副标题</td><td bgcolor='ffffff'><input name="ftitle" type="text" id="ftitle" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'ftitle',stripSlashes($r['ftitle']))?>" size="60">
...
</td></tr><tr><td width='16%' height=25 bgcolor='ffffff'>内容简介</td><td bgcolor='ffffff'><textarea name="smalltext" cols="80" rows="10" id="smalltext"><?=$ecmsfirstpost==1?"":DoRehValue($modid,'smalltext',stripSlashes($r['smalltext']))?></textarea>
...
?>
<input type=text name=writer value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'writer',stripSlashes($r['writer']))?>" size=""> 
        <select name="w_id" id="w_id" onchange="document.add.writer.value=document.add.w_id.value">
          <option>选择作者</option>
		  <?=$w_class?>
        </select>
<input type="button" name="wbutton" value="增加作者" onclick="window.open('NewsSys/writer.php?<?=$ecms_hashur['ehref']?>&addwritername='+document.add.writer.value);"></td></tr><tr><td width='16%' height=25 bgcolor='ffffff'>信息来源</td><td bgcolor='ffffff'><?php
$b_class='';
...
?>
<input type="text" name="befrom" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'befrom',stripSlashes($r['befrom']))?>" size=""> 
        <select name="befrom_id" id="befromid" onchange="document.add.befrom.value=document.add.befrom_id.value">
          <option>选择信息来源</option>
          <?=$b_class?>
        </select>
<input type="button" name="wbutton" value="增加来源" onclick="window.open('NewsSys/BeFrom.php?<?=$ecms_hashur['ehref']?>&addsitename='+document.add.befrom.value);"></td></tr><tr><td height=25 colspan=2 bgcolor='ffffff'><div align=left>新闻正文(*)</div></td></tr></table><div style="background-color:#D0D0D0"><?=ECMS_ShowEditorVar("newstext",$ecmsfirstpost==1&&$usersldo==0?"":DoRehValue($modid,'newstext',stripSlashes($r['newstext'])),"Default","","300","100%")?>
