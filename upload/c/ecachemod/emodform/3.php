...
<select name="select" onchange="document.add.filesize.value+=this.value">
  ...
  <tr> 
    ...
    <td bgcolor=ffffff>图片尺寸：<input name="picsize" type="text" id="picsize" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'picsize',stripSlashes($r['picsize']))?>" size="">
，图片分辨率：<input name="picfbl" type="text" id="picfbl" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'picfbl',stripSlashes($r['picfbl']))?>" size="">
</td>
  </tr>
  <tr> 
    <td width=16% height=25 bgcolor=ffffff>来源</td>
    <td bgcolor=ffffff><input name="picfrom" type="text" id="picfrom" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'picfrom',stripSlashes($r['picfrom']))?>" size="60">
</td>
  </tr>
  <tr> 
...
<a onclick="window.open('ecmseditor/FileMain.php?type=1&classid=<?=$classid?>&infoid=<?=$id?>&filepass=<?=$filepass?>&sinfo=1&doing=1&field=titlepic<?=$ecms_hashur['ehref']?>','','width=700,height=550,scrollbars=yes');" title="选择已上传的图片"><img src="../../e/data/images/changeimg.gif" border="0" align="absbottom"></a> 
  ...
  <tr> 
    ...
    <td bgcolor=ffffff><input name="picurl" type="text" id="picurl" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'picurl',stripSlashes($r['picurl']))?>" size="45">
...
?>
  ...
  <tr> 
    ...
    <td bgcolor=ffffff>每行显示条数：<input name="num" type="text" id="num" value="<?=$ecmsfirstpost==1?"3":DoRehValue($modid,'num',stripSlashes($r['num']))?>" size="">
，缩图大小：<input name="width" type="text" id="width" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'width',stripSlashes($r['width']))?>" size="6">
*<input name="height" type="text" id="height" value="<?=$ecmsfirstpost==1?"":DoRehValue($modid,'height',stripSlashes($r['height']))?>" size="6">
<font color="#666666">(宽*高)</font></td>
  </tr>
  <tr> 
    <td width=16% height=25 bgcolor=ffffff>图片简介(*)</td>
    <td bgcolor=ffffff><textarea name="picsay" cols="80" rows="10" id="picsay"><?=$ecmsfirstpost==1?"":DoRehValue($modid,'picsay',stripSlashes($r['picsay']))?></textarea>
</td>
  </tr>
</table>