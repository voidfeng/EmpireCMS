...
//查询SQL
$query="select id,classid,isurl,titleurl,isqf,havehtml,istop,isgood,firsttitle,ismember,username,plnum,totaldown,onclick,newstime,truetime,lastdotime,titlefont,titlepic,title,efz".$queryaddf." from ".$infotb." where ".$yhadd."userid='".$user['userid']."' and ismember=1".$add." order by newstime desc".do_dblimit($line,$offset);
...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="tableborder">
  ...
  <?php
	while($r=$empire->fetch($sql))
	{
		...
		$liurl_edit='AddInfo.php?enews=MEditInfo&classid='.$r['classid'].'&id='.$r['id'].'&mid='.$mid.$addecmscheck;
		$liurl_del='ecms.php?enews=MDelInfo&classid='.$r['classid'].'&id='.$r['id'].'&mid='.$mid.$addecmscheck;
	?>
  <tr bgcolor="#FFFFFF" id=news<?=$r['id']?>> 
    ...
    <td height="25"><div align="center"><a href="<?=$liurl_edit?>">修改</a> | <a href="<?=$liurl_del?>" onclick="return confirm('确认要删除?');">删除</a> 
      </div></td>
  </tr>
  <?php
	}
